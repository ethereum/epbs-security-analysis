...
- **`parent_root`** — the hash-tree root of $B$'s parent beacon block (call it $P$). This is $B$'s choice: which beacon block to extend.
- **`bid.parent_block_hash`** — the `block_hash` of the execution payload $B$'s bid references as its parent. $P$'s bid itself carries two hash fields: `block_hash` (the hash of $P$'s own future execution payload) and `parent_block_hash` (the hash $P$ inherited as its execution parent). **Given $P$, $B$'s `bid.parent_block_hash` must equal one of these two values**, and which value it takes corresponds to $P$'s **payload status** — either **FULL** (the execution chain advances *through* $P$'s payload) or **EMPTY** (the execution chain *skips past* $P$, inheriting $P$'s own execution parent):
  - **`bid(P).block_hash`** → $B$ declares $P$ as **FULL**.
  - **`bid(P).parent_block_hash`** → $B$ declares $P$ as **EMPTY**.
...
The same constraint is enforced at the state-machine level by `process_execution_payload_bid`, which asserts `bid.parent_block_hash == state.latest_block_hash`. The field `state.latest_block_hash` carries the `block_hash` of the most recent FULL ancestor — so the two admissible values listed above (`bid(P).block_hash` and `bid(P).parent_block_hash`) are exactly the two values `state.latest_block_hash` can hold when $B$ is processed, depending on whether $P$'s payload effects were applied during $B$'s `process_parent_execution_payload` step.

*On the construction side:* when the slot-$N$ builder constructs $B$'s bid, it does not pick from the two-element set directly. It calls `prepare_execution_payload`, which simulates `apply_parent_execution_payload` on a copy of state when $P$'s payload is locally available — advancing `state.latest_block_hash` to `bid(P).block_hash`. If $P$'s payload is not locally available, the simulation does not run and `state.latest_block_hash` remains at `bid(P).parent_block_hash`. The builder then writes the resulting `state.latest_block_hash` into `bid.parent_block_hash`. So the bid-layer constraint (one of two values) is a *consequence* of the construction mechanism, not a direct choice. 

We prefer the bid-layer framing in the main text because it is directly inspectable on-chain — there are exactly two hashes in $P$'s bid, and $B$'s bid takes one of them. This avoids reasoning about the precise moment in the state transition at which `state.latest_block_hash` is updated, a subtlety addressed by [consensus-specs PR #5094](https://github.com/ethereum/consensus-specs/pull/5094) (which deferred execution-payload processing to the next block via `process_parent_execution_payload`). The bid-layer constraint holds regardless of those state-machine timing details.
