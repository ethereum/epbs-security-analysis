...
- *Trustless (`bid.value > 0`).* Assume there exists a canonical block $B'$ at slot $N{-}1$ that remains canonical forever, **S1** (synchrony), **S2** (β < 20%), **S3** (slot-$N$ PTC majority is honest, and honest PTC members wait to observe both the payload and the blob data before voting), and $B$ is timely (Definition in §9.1: $t + \Delta \leq T_{\mathrm{att}}$).
...
> **TL;DR.** This section describes the mechanism that implements **P6** (unconditional payment) and **P5** (builder withholding protection, trustless). The IOU is recorded at bid time (only when `bid.value > 0`); queueing for transfer happens via **Path A** (next block, when the builder reveals) or **Path B** (epoch boundary, gated by a 60% attestation quorum). Path A suppresses Path B, so the proposer is paid exactly once. End-to-end, the `BuilderPendingWithdrawal` is **eventually queued** under P6's attestation-inclusion hypothesis; the actual EL credit follows when a subsequent block's `apply_withdrawals` drains the queue (typically a few slots later under standard liveness). Path B's quorum is what makes P5 work: a builder who withholds in the absence of honest support is not charged.
>
...
> **Property P6: Unconditional payment.** For a block at slot N with `bid.value > 0` that is timely (under P6's preconditions: $B$ extends $B'$ at slot $N{-}1$ and $B'$ remains canonical forever), and given P6's attestation-inclusion hypothesis (all honest slot-$N$ attestations are included on the canonical chain before the start of epoch $\mathsf{epoch}(N)+2$), the proposer's payment is **eventually** queued via the Path A / Path B mechanism: at slot N+1 via Path A when a subsequent block declares the slot FULL on chain, or via Path B when a canonical block at slot $\geq$ start of epoch $\mathsf{epoch}(N)+2$ runs `process_epoch` and the 60% same-slot quorum is met (this is the natural case when the builder withheld). In the Path B case, the beacon block's same-slot attestation weight reaches the 60% quorum because timeliness + the canonicity precondition + S1 + S2 force at least $(1-\beta) W > 0.8 W$ of honest weight on the beacon block — accumulated into `payment.weight` only because the attestations are included on chain; see §9.3 P6 proof Case 1b. After the IOU is cleared by either path, no second payment is ever produced: `process_attestation` skips zero-`amount` entries and `process_builder_pending_payments` checks `weight ≥ quorum` on a zeroed entry, which fails. The payment is therefore single.
...
P6 promises that the proposer's `fee_recipient` is **eventually credited**. The protocol delivers this in two steps: *(i)* a `BuilderPendingWithdrawal` entry is appended to `state.builder_pending_withdrawals` via Path A or Path B (this is what the §9.3 proof establishes from S1 + S2 + the attestation-inclusion hypothesis); *(ii)* a later canonical block's `apply_withdrawals` drains that entry, debiting the builder's CL balance and crediting the proposer's `fee_recipient` at the EL. Step (ii) requires future canonical FULL blocks to be produced (this implicit queue-drain liveness piggybacks on the same Gasper liveness implicit in P6's inclusion hypothesis — see §9.3 P6 Part 2). Step (i) is bounded by the structural ring-buffer layout; step (ii)'s timing depends on chain state at settlement time (queue position, FULL/EMPTY mix of subsequent slots) and **cannot be precisely bounded by the protocol** — only "eventually". The rest of this drop-down works through the drain rate and best- and worst-case settlement timing.
...
**Bottom line.** P6 promises that the proposer's `fee_recipient` is eventually credited with the bid amount. The protocol delivers this in two steps: queueing (within $\sim 2$ epochs of slot $N$ under the attestation-inclusion hypothesis) and EL credit (once `apply_withdrawals` drains the IOU, rate-limited by the 15-per-FULL-block cap and depending on how many other proposers are queued ahead of $B$'s proposer plus the FULL/EMPTY mix of subsequent slots). Under standard liveness with a healthy chain, the gap between queueing and EL credit is small (one to a handful of slots); under heavy backlog or extended periods of missed/EMPTY slots, the gap can be much larger.
...
Both sub-cases require $\geq 32$ consecutive missed slots; under **S2**, the union-bound probability is $\leq 2\beta^{32} \approx 10^{-22}$. The §9.3 P6 statement reflects this through its attestation-inclusion hypothesis, which fails in the second sub-case above (no attestations are ever included).
...
*$B$ extends $B'$ (derived).* Suppose for contradiction that $B$ does not extend $B'$. Since $B'$ is canonical at slot $N{-}1$ by hypothesis, $B$ is then not in the subtree rooted at $B'$, i.e., $B$ sits on a non-canonical branch. Honest slot-$N$ validators' `get_head` at $T_{\mathrm{att}}$ therefore returns a node in the canonical subtree (in particular, not $B$), and they do not vote for $B$. The real attestation weight on $B$ is then bounded by Byzantine weight, $\leq \beta W < 20\% W < 40\% W$, contradicting **A1a** (ii). Hence $B$ extends $B'$.

*Honest weight on $B$.* Under timeliness ($B$ broadcast at $t$ with $t + \Delta \leq T_{\mathrm{att}}$) and **S1**, every honest slot-$N$ validator receives $B$ by $T_{\mathrm{att}}$. Combined with $B$ extending the canonical $B'$ and the absence of any visible equivocation at $T_{\mathrm{att}}$, every honest slot-$N$ validator's `get_head` returns $B$, so each votes for $B$. By **S2**, the honest weight on $B$ is $\geq (1-\beta) W > 80\% W$.

*Anti-reorg.* By the same arithmetic as P1's proof (§9.3 P1): $B$ has $\geq (1-\beta) W > 80\% W$ honest weight at slot $N+1$, while any boost-only competitor has at most $40\% W$ (boost) $+ \beta W < 60\% W$; no boost-only reorg succeeds. The remaining parent-level reorg vector is closed by "$B'$ remains canonical forever"; the missing-slot configuration is excluded because $B'$ exists at slot $N{-}1$. Hence $B$ remains in the canonical chain at every later time.
...
*Assumptions used:* **S1**, **S2**, **honest slot-$N$ proposer** (inherited via P1), P6's **attestation-inclusion hypothesis** (all honest slot-$N$ attestations are included on the canonical chain before the start of epoch $\mathsf{epoch}(N) + 2$; required for Path B Case 1b's quorum), **G-BidAdmit**, **G-Settle**, **G-PayAttest**, **G-PayEpoch**, **G-Solvency**. The configuration preconditions on $B$ ($B$ extends $B'$ at slot $N{-}1$, $B'$ remains canonical forever, `bid.value > 0`) feed canonicity into P1 and arm the IOU machinery. The queue → EL credit step relies on the standard withdrawal-pipeline liveness (FULL blocks continue to be produced); the §7 fold-out works through the timing.
...
**Scope of this subsection.** Of the six properties, we encode the four that are naturally expressible as trace-predicate implications: **P3** (data availability for chain inclusion), **P4 trustless** (builder revealing protection, trustless case), **P5 trustless** (builder withholding protection, trustless case), and **P6** (unconditional payment). P1 (Block safety) and P2 (Payload execution deadline) are not encoded here — P1 is a fork-choice claim already covered by the §6 fork-choice machinery, and P2 is a structural observation about handler dependencies (§9.3's P2 proof enumerates the relevant handlers directly). The **non-trustless** cases of P4 and P5 are parametric in a confirmation rule $R$ and its assumption set $\Sigma_R$ (§5 Phase 3); the trustless predicates below would specialise to non-trustless by substituting the confirmation-rule check for the A1a (i)–(iii) conditions, but a single parametric encoding is more legibly left to the formal-treatment companion.
...
**The admissible traces** are exactly those consistent with the §9.1–§9.2 assumptions: every honest actor follows the spec handlers; the network delivers within $\Delta < T_{\mathrm{att}}$; per-committee Byzantine weight is below β = 20%; PTC Byzantine weight is below 50%; etc. A claim like "P6 holds" means: *for every admissible trace, `P6(trace, B) == True` for every canonical block `B`.*
...
# and Phase 3 read and write, plus the observations A1a cautious-reveal
...
class BuilderState:
    ...
    observed_attestations:  Sequence[Attestation]                # for A1a(ii) — >=40% real weight check
    observed_equivocations: Set[Tuple[ValidatorIndex, Slot]]     # for A1a(iii) — equivocation visibility
...
# @Upon builder handlers (submit_bid, reveal_payload) plus Assumption A1a
...
# There exists a canonical block B' at slot B.slot - 1 that remains in the
# canonical chain of every honest validator at every step of the trace.
# This is the strengthening canonicity hypothesis embedded in P1, P4
# (trustless), and P6 — see §4 and the discussion below.  Under this
# hypothesis, "B extends B'" is DERIVED in each property's proof (automatic
# for an honest proposer in P1/P6; derived from A1a (ii) plus B' canonicity
# in P4 trustless — see §9.3 P4 trustless proof, "B extends B' (derived)").
def canonical_b_prime_remains_canonical(trace: Trace, B: BeaconBlock) -> bool: ...
...
# True iff a strict majority of the slot's PTC committee are honest AND
# every honest PTC member that votes before T_ptc waits to observe both
# the payload and the blob data before voting (S3 in §9.1).
def s3_holds_at_slot(trace: Trace, slot: Slot) -> bool: ...
...
# True iff B's broadcast event in the trace satisfies t + DELTA <= T_att
# relative to its own slot — i.e., B is timely in the §9.1 sense (Definition
# of timely block). Used by P4 trustless to formalise the "B is timely"
# hypothesis from §4.
def is_timely(trace: Trace, B: BeaconBlock) -> bool: ...

# P6's attestation-inclusion hypothesis (§4): every honest slot-N attestation
# for B is included on the canonical chain before the start of epoch
# epoch(N)+2. Required for Path B's quorum derivation in the §9.3 P6 proof
# (Case 1b).
def all_honest_slot_n_attestations_included_before_epoch_n_plus_2(
    trace: Trace, B: BeaconBlock
) -> bool: ...
...
**P3 — Data availability for chain inclusion.** *(Narrative: §4 P3.)*

<details>
<summary><b>Pseudocode definition</b> (click to expand)</summary>

```python
# P3: Data availability for chain inclusion
#
# Hypothesis: at some (time t*, honest validator v*), h belongs to the
#   payload hash chain of v*'s canonical chain (§3 sense).
...
#   == h, has passed verify_execution_payload_envelope, and its blob data
#   passed is_data_available. Co-temporal: 'whenever h is on chain at v,
#   it is available at v' (not a settled-by-time-t statement).
#
# Assumes only S2 (giving an honest super-majority); the §9.3 P3 proof
# routes through on_block's parent-FULL assertion, not through PTC votes.

def P3(trace: Trace, h: Hash32) -> bool:
    return implies(P3_hypothesis(trace, h), P3_conclusion(trace, h))


def P3_hypothesis(trace: Trace, h: Hash32) -> bool:
    for s in trace:
        for _, store_v in s.validator_stores.items():
            chain_v = honest_canonical(store_v)
            if full_block_committing_to(chain_v, h) is not None:
                return True
    return False


def P3_conclusion(trace: Trace, h: Hash32) -> bool:
    for s in trace:
        for _, store_v in s.validator_stores.items():
            chain_v = honest_canonical(store_v)
...
**P4 (trustless) — Builder revealing protection, trustless case.** *(Narrative: §4 P4 trustless.)*

<details>
<summary><b>Pseudocode definition</b> (click to expand)</summary>

```python
# P4 trustless: Builder revealing protection (trustless case, bid.value > 0)
#
# Hypothesis: the builder named in bid(B) is honest, broadcast an envelope
#   binding to bid(B).block_hash before slot N+1 begins under Assumption
#   A1a (§5 Phase 3); S3 holds at slot N (honest PTC majority + honest
#   members wait for both signals); B is timely; AND the strengthening
#   hypothesis holds — a canonical block B' at slot N-1 remains canonical
#   forever. ("B extends B'" is DERIVED inside the §9.3 P4 trustless proof
#   from A1a (ii) + B' canonicity, not assumed.)
# Conclusion: from the start of slot N+2 onward, every honest validator's
#   canonical chain contains B and declares B FULL.

def P4_trustless(trace: Trace, B: BeaconBlock) -> bool:
    return implies(P4_trustless_hypothesis(trace, B),
                   P4_trustless_conclusion(trace, B))


def P4_trustless_hypothesis(trace: Trace, B: BeaconBlock) -> bool:
    bid = B.body.signed_execution_payload_bid.message
    N = B.slot
    if not (bid.value > 0):
        return False                          # trustless case only
    ...
    return (honest_reveal
            and is_timely(trace, B)
            and s3_holds_at_slot(trace, N)
            and canonical_b_prime_remains_canonical(trace, B))


def P4_trustless_conclusion(trace: Trace, B: BeaconBlock) -> bool:
    # Why slot N+2 (not N+1): block_status(chain, B) is undefined when B is
    # the head. By slot N+2, the slot-(N+1) successor has landed on the
    # canonical chain of every honest v under S1+S2, giving B a successor
    # that pins block_status(B) to FULL.
...
**P5 (trustless) — Builder withholding protection, trustless case.** *(Narrative: §4 P5 trustless.)*

<details>
<summary><b>Pseudocode definition</b> (click to expand)</summary>

```python
# P5 trustless: Builder withholding protection (trustless case, bid.value > 0)
#
...
#   start).
# Conclusion: no BuilderPendingWithdrawal carrying
#   (bid.value, bid.builder_index) ever appears in any honest validator's
#   state for the head, from the epoch-(e+1) boundary forward — the
#   BuilderPendingPayment is discarded by process_builder_pending_payments
#   rather than paid out (or cleared by process_proposer_slashing in the
#   equivocation sub-case).

def P5_trustless(trace: Trace, B: BeaconBlock) -> bool:
    return implies(P5_trustless_hypothesis(trace, B),
                   P5_trustless_conclusion(trace, B))


def P5_trustless_hypothesis(trace: Trace, B: BeaconBlock) -> bool:
    bid = B.body.signed_execution_payload_bid.message
    N = B.slot
    if not (bid.value > 0):
        return False                          # trustless case only
...
def P5_trustless_conclusion(trace: Trace, B: BeaconBlock) -> bool:
    bid = B.body.signed_execution_payload_bid.message
    N = B.slot
    e = N // SLOTS_PER_EPOCH
    # Bound the scan to the 2-epoch settlement window. Outside this window,
    # any matching BPW must come from a different bid by the same builder
    # and is not evidence about THIS bid's settlement.
...
**P6 — Unconditional payment to the proposer.** *(Narrative: §4 P6.)*

<details>
<summary><b>Pseudocode definition</b> (click to expand)</summary>

```python
# P6: Unconditional payment to proposer
#
# Hypothesis: B at slot N has bid.value > 0, is proposed by an honest
#   proposer, and the strengthening canonicity hypothesis holds — a
#   canonical block B' at slot N-1 remains canonical forever. Plus P6's
#   attestation-inclusion hypothesis: all honest slot-N attestations are
#   included on the canonical chain before the start of epoch
#   epoch(N) + 2. ("B extends B'" is automatic for an honest proposer.)
# Conclusion: a BuilderPendingWithdrawal carrying
#   (bid.value, bid.fee_recipient, bid.builder_index) is appended to
#   state.builder_pending_withdrawals at SOME step in the settlement
#   window — either at slot N+1 via Path A (settle_builder_payment when
#   a successor declares FULL) or at the first canonical block at slot
#   >= start of epoch epoch(N)+2 via Path B
#   (process_builder_pending_payments under the 60% quorum).

def P6(trace: Trace, B: BeaconBlock) -> bool:
    return implies(P6_hypothesis(trace, B), P6_conclusion(trace, B))


def P6_hypothesis(trace: Trace, B: BeaconBlock) -> bool:
    bid = B.body.signed_execution_payload_bid.message
    N = B.slot
    if not (bid.value > 0):
        return False                          # P6 covers value-bearing bids; bid.value = 0 is vacuous
    proposer = proposer_for_slot(trace, N)
    if not is_honest(proposer, trace):
        return False
    if not canonical_b_prime_remains_canonical(trace, B):
        return False
    # Attestation-inclusion hypothesis: every honest slot-N attestation for B
    # is included on the canonical chain before the start of epoch(N)+2.
    # (Required for Path B Case 1b's quorum; cf. §9.3 P6.)
    if not all_honest_slot_n_attestations_included_before_epoch_n_plus_2(trace, B):
        return False
    return True


def P6_conclusion(trace: Trace, B: BeaconBlock) -> bool:
    bid = B.body.signed_execution_payload_bid.message
    fee_recipient = bid.fee_recipient   # ExecutionPayloadBid.fee_recipient — destination set by process_execution_payload_bid for BuilderPendingWithdrawal
    N = B.slot
    e = N // SLOTS_PER_EPOCH
    t_window_start = N * SECONDS_PER_SLOT
    t_window_end = (e + 2) * SLOTS_PER_EPOCH * SECONDS_PER_SLOT + DELTA
    ...
    # appended (Path A at slot N+1, or Path B at the first canonical block
    # at slot >= start of epoch e+2) and then drained out of
    # state.builder_pending_withdrawals before the deadline.
    honest_validators = {
        v_idx
...
> **Note — the strengthening canonicity hypothesis in P4 trustless and P6.** The predicate `canonical_b_prime_remains_canonical(trace, B)` is the formal restatement of the §4 hypothesis "there exists a canonical block $B'$ at slot $N{-}1$ that remains canonical forever". It closes a reorg vector that the cautious-reveal threshold of A1a does not address on its own: a Byzantine slot-$N$ proposer publishes $B$ late enough to fragment honest attestations so that 40% see $B$ in time and vote for $B$ while the rest don't and vote for an earlier head on a competing branch. Without the strengthening, a slot-$N{+}1$ proposer's `get_head` walks to the heavier (alternative) branch and reorgs $B$ — even though the builder followed A1a (≥40% real weight + no equivocation visible). Forcing $B'$ to remain canonical excludes the alternative branch from honest fork-choice consideration, so the slot-$N{+}1$ fork-choice can only extend $B'$. The fact that $B$ extends $B'$ is **derived** inside each property's proof (automatic for an honest proposer in P1/P6; derived from A1a (ii) plus $B'$'s canonicity in P4 trustless — see §9.3 P4 trustless proof, "$B$ extends $B'$ (derived)"), so it does not appear as a separate predicate.
>
> *Open work.* The strengthening is itself stronger than the rest of the S/A/G framework. We are working on weaker formulations of P4 trustless and P6 — likely by tightening the cautious-reveal threshold in A1a to a value that absorbs the worst-case slot-$N$ split-vote attack, or by adding an explicit "no competing block of greater real weight at $t_{\mathrm{rev}}$" precondition to the builder's reveal decision. Until that work lands, the predicates above carry the strengthening as an explicit hypothesis.
