...
- *Trustless (`bid.value > 0`).* Assume **S1** (synchrony), **S2** (β < 20%), and the proof-specific side condition for the withholding subcase: **I_payweight** for the no-weight subcase, **I_slash** for the equivocation-driven subcase where a canonical IOU for this builder bid remains (§9.1).
...
The remainder of this document shows how the protocol's algorithms enforce P1–P6 plus P_valid. §5 Phase 3 presents the cautious-reveal protocols an honest builder can follow in each case. §8 walks through the adversarial scenarios in the trustless case. §9 (work-in-progress) catalogues a self-contained set of assumptions and gives proof sketches that, combined with the code in §5–§7, justify the seven properties.
...
*Each G-assumption pins down the behaviour of an internal spec function. Each is given as a pseudocode definition that captures the assumption's content for the purposes of proving the Category B and Category C properties (P4, P5, P6): pedagogical pseudocode rather than the full spec body. Category A (P1, P2, P3, P_valid) cites few or none of these; each proof sketch in §9.3 lists the assumptions it uses.*
...
The argument is valid **given** the stated assumptions. The sketches are ordered to match §4's categorisation: Category A (P1, P2, P3, P_valid; payment-trustlessness-independent), Category B (P4, P5; payment-trustlessness-dependent, each with two cases), Category C (P6; trustless-only).
...
**Scope of this subsection.** Of the seven properties, we encode the five that are naturally expressible as trace-predicate implications or invariants: **P3** (data availability for chain inclusion), **P_valid** (chain validity), **P4 trustless** (builder revealing protection, trustless case), **P5 trustless** (builder withholding protection, trustless case), and **P6** (unconditional payment). P1 (Block safety) is a fork-choice claim already covered by the §6 fork-choice machinery, and P2 (Payload execution deadline) is a structural observation about handler dependencies (§9.3's P2 proof enumerates the relevant handlers directly). The **non-trustless** cases of P4 and P5 are parametric in a confirmation rule $R$ and its assumption set $\Sigma_R$ (§5 Phase 3); the trustless predicates below would specialise to non-trustless by replacing the A1a/canonicity side of the hypothesis with the chosen confirmation-rule predicate.

**Why traces, not snapshots.** A distributed protocol has no single "system state": every validator has its own `Store`, and the network delivers messages with bounded delay. Properties that quantify over time, such as "the bid is eventually credited" or "whenever `h` is on chain, the payload is available", cannot be expressed as predicates on a single state. We model the system as a **trace**: a sequence of honest-actor broadcast events, each paired with the resulting per-validator state. Properties are quantified over traces.

Byzantine actors do not produce events in the trace: only honest broadcasts do. Byzantine effects appear indirectly, either as state changes at honest recipients under synchrony or as the absence of an honest broadcast where one would normally appear. This is the cleanest shape for a verifier: honest behaviour is explicit; the adversary's freedom shows up in which traces are admissible.

**Admissible traces** are exactly those consistent with the assumptions named by the property being checked: every honest actor follows the spec handlers; S1/S2/S3 hold where the property cites them; and proof-specific side conditions such as **L_verify**, **L_successor**, **I_payweight**, **I_slash**, and **L_withdraw** hold where the corresponding predicate includes them. A claim like "P6 holds" means: for every trace satisfying P6's hypothesis-side assumptions, `P6(trace, B) == True` for every block `B` named by the predicate.

The trace types are defined below; the five property predicates follow.

<details>
<summary><b>Trace, actor, event, and helper types</b> (click to expand)</summary>

```python
# Actor as a variant type. Each variant carries the spec's role-specific
# index type, so the type system enforces the role-index pairing.
...
# Messages broadcast by honest actors on the gossip network. DataColumnSidecar
# represents the PeerDAS blob-data sidecars; a concrete model may split this
# into one event per sidecar or a bundle event for all sidecars needed by a bid.
Message = Union[
    SignedBeaconBlock,
...
# A builder's local state: what submit_bid / reveal_payload in §5 Phase 0 and
# Phase 3 read and write, plus the observations A1a cautious reveal consults.
@dataclass
class BuilderState:
    stored_payload: Optional[ExecutionPayload]
    stored_requests: Optional[ExecutionRequests]
    observed_blocks: Mapping[Root, SignedBeaconBlock]
    observed_attestations: Sequence[Attestation]
    observed_equivocations: Set[Tuple[ValidatorIndex, Slot]]


# A single honest-actor broadcast. Byzantine broadcasts are outside the trace;
# their delivered effects appear in later honest stores.
@dataclass
class Event:
    time: float
    actor: Actor
...
# A trace step: the broadcast event plus the resulting honest local states.
@dataclass
class TraceStep:
    event: Event
    validator_stores: Mapping[ValidatorIndex, Store]
    builder_states: Mapping[BuilderIndex, BuilderState]


# Mathematical sequence of trace steps. Liveness predicates below are meant over
# complete executions: either infinite traces, or finite traces only when the
# execution has terminated with no pending liveness obligation.
...
def bid_of(block: BeaconBlock) -> ExecutionPayloadBid: ...
def has_gloas_bid(block: BeaconBlock) -> bool: ...
def parent_in_chain(chain: Sequence[BeaconBlock], block: BeaconBlock) -> Optional[BeaconBlock]: ...
def is_data_available_at(store: Store, root: Root) -> bool: ...


# Structural assumptions from §9.1. The slot-specific variants are used when a
# property names one slot; the whole-trace form is used for properties over an
# arbitrary payload hash.
def s1_holds(trace: Trace) -> bool: ...
def s2_holds(trace: Trace) -> bool: ...
def s2_holds_for_slot(trace: Trace, slot: Slot) -> bool: ...


# P1-style honest proposer condition used by P6: the scheduled proposer for
# B.slot is honest, broadcasts B as its slot block, B is timely, and B extends
# the canonical slot-(N-1) block from the strengthening hypothesis.
def honest_proposer_published_p1_block(trace: Trace, B: BeaconBlock) -> bool: ...


# Transition-level instrumentation. BuilderPendingWithdrawal does not carry a
# slot or bid identifier, so a formal trace should record append/debit/credit
# events with the originating bid. Snapshot scans by (builder, amount) are not
# precise enough when the same builder has multiple equal-value bids.
def builder_pending_withdrawal_queued_for_bid(trace: Trace, B: BeaconBlock) -> bool: ...
def builder_pending_withdrawal_queued_exactly_once_for_bid(trace: Trace, B: BeaconBlock) -> bool: ...
def builder_balance_debited_for_bid(trace: Trace, B: BeaconBlock) -> bool: ...
def builder_balance_debited_exactly_once_for_bid(trace: Trace, B: BeaconBlock) -> bool: ...
def proposer_fee_recipient_credited_for_bid(trace: Trace, B: BeaconBlock) -> bool: ...
def proposer_fee_recipient_credited_exactly_once_for_bid(trace: Trace, B: BeaconBlock) -> bool: ...


# P3 import-time witness. There exists an honest validator and a trace step at
# which that validator has a canonical FULL-declaring successor for the block
# committing to h, and its local store contains the verified payload and
# node-local blob-data availability witness required by on_block's parent-FULL
# assertion.
def payload_availability_witness_for_hash(trace: Trace, h: Hash32) -> bool: ...


# There exists a canonical block B' at slot B.slot - 1 that remains in the
# canonical chain of every honest validator at every step of the trace. This is
# the strengthening canonicity hypothesis embedded in P1, P4 trustless, and P6.
...
# S3 at the named slot: honest-online PTC majority, and honest online PTC
# members that vote before T_ptc wait until both local signals are positive.
def s3_holds_at_slot(trace: Trace, slot: Slot) -> bool: ...


# Liveness / inclusion side conditions from §9.1.
def l_verify_holds_for_reveal(trace: Trace, B: BeaconBlock) -> bool: ...
def l_successor_holds_after_full_decision(trace: Trace, B: BeaconBlock) -> bool: ...
def p5_withholding_side_conditions_hold(trace: Trace, B: BeaconBlock) -> bool: ...
def l_withdraw_holds_for_payment(trace: Trace, B: BeaconBlock) -> bool: ...


# P6's attestation-inclusion hypothesis: every online honest slot-N
# attestation for B is included on the canonical chain before the start of
# epoch(N)+2.
def all_online_honest_slot_n_attestations_included_before_epoch_n_plus_2(
    trace: Trace, B: BeaconBlock
) -> bool: ...


# Returns the FULL non-head canonical block B* whose bid commits to h, if any.
# This is the witness for the §3 payload-hash-chain characterization.
def full_block_committing_to(
    chain: Sequence[BeaconBlock], h: Hash32
) -> Optional[BeaconBlock]: ...


def block_is_on_payload_hash_chain(chain: Sequence[BeaconBlock], B: BeaconBlock) -> bool:
    return full_block_committing_to(chain, bid_of(B).block_hash) == B


def is_timely(trace: Trace, B: BeaconBlock) -> bool: ...


def builder_revealed_payload_and_blob_sidecars(trace: Trace, B: BeaconBlock) -> bool:
    """
    True iff the honest builder named in bid(B) broadcasts the matching
    SignedExecutionPayloadEnvelope and the associated blob sidecars within the
    reveal window used by the applicable cautious-reveal protocol.
    """
    ...


def builder_revealed_under_A1a(trace: Trace, B: BeaconBlock) -> bool:
    """
    True iff the honest builder named in bid(B) broadcasts the matching payload
    envelope and blob sidecars, and at reveal time A1a's three cautious-reveal
    checks hold: the reveal-time window, >= 40% real attestation weight for B,
    and no visible proposer equivocation for B's slot.
    """
    ...


def builder_withheld_under_A1a(trace: Trace, B: BeaconBlock) -> bool:
    """
    True iff the honest builder named in bid(B) follows A1a and therefore does
    not reveal for B in one of the P5-covered withholding subcases: the
    no-weight subcase where A1a's attestation-weight check fails, or the
    equivocation-driven subcase where A1a's no-equivocation check fails.
    """
    ...


def eventually_always(trace: Trace, predicate: Callable[[TraceStep], bool]) -> bool:
    for i in range(len(trace)):
        if all(predicate(s) for s in trace[i:]):
            return True
    return False
...
#
# Hypothesis: S2 holds and, at some honest validator view, h belongs to the
#   payload hash chain of that view's canonical chain (§3 sense).
# Conclusion: the trace contains an honest-node import witness: some honest node
#   successfully processed a canonical FULL-declaring successor, and at that
#   node the corresponding payload was verified and blob-data availability
#   passed.
...
def P3_hypothesis(trace: Trace, h: Hash32) -> bool:
    if not s2_holds(trace):
        return False
    for s in trace:
        for store_v in s.validator_stores.values():
...
def P3_conclusion(trace: Trace, h: Hash32) -> bool:
    return payload_availability_witness_for_hash(trace, h)
```

</details>

**P_valid — Chain validity.** *(Narrative: §4 P_valid.)*

<details>
<summary><b>Pseudocode definition</b> (click to expand)</summary>

```python
# P_valid: under S2, every canonical bid chooses exactly one of the two
# execution-parent hashes already committed by its parent bid. No third
# parent_block_hash is admissible on an honest canonical chain.

def P_valid(trace: Trace) -> bool:
    return implies(P_valid_hypothesis(trace), P_valid_conclusion(trace))


def P_valid_hypothesis(trace: Trace) -> bool:
    return s2_holds(trace)


def P_valid_conclusion(trace: Trace) -> bool:
    for s in trace:
        for store_v in s.validator_stores.values():
            chain_v = honest_canonical(store_v)
            for B in chain_v:
                if not has_gloas_bid(B):
                    continue
                parent = parent_in_chain(chain_v, B)
                if parent is None or not has_gloas_bid(parent):
                    continue

                bid = bid_of(B)
                parent_bid = bid_of(parent)
                allowed = {
                    parent_bid.block_hash,         # parent declared FULL
                    parent_bid.parent_block_hash,  # parent declared EMPTY
                }
                if bid.parent_block_hash not in allowed:
                    return False
    return True
```
...
```python
# P4 trustless: if an honest trustless builder reveals under A1a, and the P4
# structural/liveness assumptions hold, bid(B).block_hash eventually appears on
# the payload hash chain of every honest canonical view.
...
def P4_trustless_hypothesis(trace: Trace, B: BeaconBlock) -> bool:
    ...
    return (
        bid.value > 0
        and is_honest(builder_actor, trace)
        and builder_revealed_under_A1a(trace, B)
        and s1_holds(trace)
        and s2_holds_for_slot(trace, N)
        and is_timely(trace, B)
        and canonical_b_prime_remains_canonical(trace, B)
        and s3_holds_at_slot(trace, N)
        and l_verify_holds_for_reveal(trace, B)
        and l_successor_holds_after_full_decision(trace, B)
    )


def P4_trustless_conclusion(trace: Trace, B: BeaconBlock) -> bool:
    def all_honest_views_have_B_full(s: TraceStep) -> bool:
        for store_v in s.validator_stores.values():
            chain_v = honest_canonical(store_v)
            if B not in chain_v:
                return False
            if block_status(chain_v, B) != FULL:
                return False
            if not block_is_on_payload_hash_chain(chain_v, B):
                return False
        return True

    return eventually_always(trace, all_honest_views_have_B_full)
```
...
```python
# P5 trustless: if an honest trustless builder withholds under A1a in one of
# the covered withholding subcases, and the matching P5 side condition from
# §9.1 holds, the protocol never queues or debits that bid's trustless payment.
...
def P5_trustless_hypothesis(trace: Trace, B: BeaconBlock) -> bool:
    ...
    return (
        bid.value > 0
        and is_honest(builder_actor, trace)
        and builder_withheld_under_A1a(trace, B)
        and s1_holds(trace)
        and s2_holds_for_slot(trace, B.slot)
        and p5_withholding_side_conditions_hold(trace, B)
    )


def P5_trustless_conclusion(trace: Trace, B: BeaconBlock) -> bool:
    return (
        not builder_pending_withdrawal_queued_for_bid(trace, B)
        and not builder_balance_debited_for_bid(trace, B)
    )
```
...
```python
# P6: for a value-bearing bid published by the honest slot proposer under the
# P6 canonicity, attestation-inclusion, and withdrawal-pipeline liveness
# assumptions, the proposer's fee_recipient is eventually credited exactly once
# with the bid amount.
...
def P6_hypothesis(trace: Trace, B: BeaconBlock) -> bool:
    ...
    return (
        bid.value > 0
        and honest_proposer_published_p1_block(trace, B)
        and s1_holds(trace)
        and s2_holds_for_slot(trace, B.slot)
        and canonical_b_prime_remains_canonical(trace, B)
        and all_online_honest_slot_n_attestations_included_before_epoch_n_plus_2(trace, B)
        and l_withdraw_holds_for_payment(trace, B)
    )


def P6_conclusion(trace: Trace, B: BeaconBlock) -> bool:
    return (
        builder_pending_withdrawal_queued_for_bid(trace, B)
        and builder_pending_withdrawal_queued_exactly_once_for_bid(trace, B)
        and builder_balance_debited_for_bid(trace, B)
        and builder_balance_debited_exactly_once_for_bid(trace, B)
        and proposer_fee_recipient_credited_for_bid(trace, B)
        and proposer_fee_recipient_credited_exactly_once_for_bid(trace, B)
    )
```

</details>

> **Note on transition instrumentation.** The predicates above deliberately use transition-level helpers such as `builder_pending_withdrawal_queued_for_bid` rather than scanning only the current `state.builder_pending_withdrawals` list. A queued withdrawal can be drained before a later snapshot, and the SSZ object does not carry a slot or bid identifier. A verifier should therefore tag append/debit/credit events with their originating bid in the trace model.
>
> **Note on the strengthening canonicity hypothesis in P4 trustless and P6.** The predicate `canonical_b_prime_remains_canonical(trace, B)` is the formal restatement of the §4 hypothesis "there exists a canonical block $`B'`$ at slot $`N{-}1`$ that remains canonical forever". It closes the parent-level reorg vector discussed in §4. The fact that $`B`$ extends $`B'`$ is derived inside each property's proof: automatic for an honest proposer in P1/P6, and derived from A1a (ii) plus $`B'`$'s canonicity in P4 trustless.
>
> *Open work.* The strengthening is itself stronger than the rest of the S/A/G framework. We are working on weaker formulations of P4 trustless and P6, likely by tightening the cautious-reveal threshold in A1a or by adding an explicit no-competing-block condition to the builder's reveal decision. Until that work lands, the predicates above carry the strengthening as an explicit hypothesis.

---

## 10. What comes next

This document makes externally observable claims organised in three categories (A: P1, P2, P3, P_valid always-on; B: P4, P5 payment-trustlessness-dependent, each with two cases; C: P6 trustless-only), traces their enforcement, surfaces every unresolved assumption, and gives machine-readable predicates for the properties that currently fit the trace-predicate shape. Each protocol-enforced property rests on at least one algorithmic assumption about an internal spec function (the G-prefix assumptions in §9.2). Several internal mechanisms (two-phase block processing, `store.payloads` gating the FULL node, same-slot payload-neutrality of the weight computation, witness-statement semantics of honest PTC voting, bid commitments being binding) are treated as *descriptions* in §3 and §5, not as Properties, because they are not directly verifiable from network messages alone.
