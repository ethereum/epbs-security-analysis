...
**Spec version.** This document targets [`ethereum/consensus-specs`](https://github.com/ethereum/consensus-specs) at commit [`bb858984`](https://github.com/ethereum/consensus-specs/commit/bb85898493a38d117a40f8004d8c2befb735e412), Gloas fork: [`specs/gloas/`](https://github.com/ethereum/consensus-specs/tree/bb85898493a38d117a40f8004d8c2befb735e412/specs/gloas); inline `file:line` references are to that commit. The Gloas specs are work-in-progress and may differ from the EIP-7732 draft summary.
...
- **Distinct duty.** The PTC casts a **payload timeliness attestation** at 75% of the slot, reporting two independent signals: `payload_present` (whether the member observed the `SignedExecutionPayloadEnvelope` arrive) and `blob_data_available` (whether the member's sampled blob data columns arrived and passed KZG verification). Both are binary observations, never validity judgments. The spec object is a `PayloadAttestationMessage` ([`beacon-chain.md`](https://github.com/ethereum/consensus-specs/blob/bb85898493a38d117a40f8004d8c2befb735e412/specs/gloas/beacon-chain.md)); we follow the PBS literature and use the shorthand **witness statement** (or **witness vote**), since the PTC is *witnessing* payload and blob-data arrival, not judging execution validity. Phase 4 (§5) details the two-signal mechanics.
...
We've referred to "the builder's bid" loosely; we now make the object concrete. The bid is the `ExecutionPayloadBid` SSZ container, wrapped in `SignedExecutionPayloadBid` for transport. Spec source: [`beacon-chain.md`](https://github.com/ethereum/consensus-specs/blob/bb85898493a38d117a40f8004d8c2befb735e412/specs/gloas/beacon-chain.md) §`ExecutionPayloadBid`. The fields:
...
- **No solvency lock.** `can_builder_cover_bid` ([beacon-chain.md:666](consensus-specs/specs/gloas/beacon-chain.md#L666)) checks the builder's stake against the sum of all outstanding `value` obligations. `execution_payment` is not in that sum. A builder can issue unlimited `execution_payment` promises across slots without the protocol noticing they've overcommitted.
...
The spec prescribes only the minimal honest rule: reveal when the block is timely and is the head of the local chain ([`builder.md`](https://github.com/ethereum/consensus-specs/blob/bb85898493a38d117a40f8004d8c2befb735e412/specs/gloas/builder.md)). The §4 property statements (P_rev, P_withhold) reference *cautious-reveal protocols* that strengthen this minimum. Two protocols, one per case; an honest builder picks the one that matches their bid's payment field:
...
- *Only FULL blocks drain.* `process_withdrawals` ([beacon-chain.md:1463-1489](consensus-specs/specs/gloas/beacon-chain.md#L1463-L1489)) early-returns when the parent block declared EMPTY (no execution payload was applied). EMPTY-declaring blocks do not drain the queue.
...
A third settlement path exists alongside Path A and Path B, hidden inside `apply_parent_execution_payload` ([beacon-chain.md:1232-1241](consensus-specs/specs/gloas/beacon-chain.md#L1232-L1241)):
