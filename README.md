...
*Proof.* By **G-BidAdmit**, processing block $N$ at every honest node creates exactly one `BuilderPendingPayment` IOU at `state.builder_pending_payments[SLOTS_PER_EPOCH + (slot % SLOTS_PER_EPOCH)]` — the upper-half slot of the 2-epoch ring buffer for the current epoch. The IOU's two fields ([beacon-chain.md:223](consensus-specs/specs/gloas/beacon-chain.md#L223)) are initialised as follows.
