...
Each lemma is proved from the spec alone — the function bodies and the enumeration of write-sites. The §9 properties cite these lemmas; the lemmas never cite a §9 property as a premise. Where a proof below names a property — e.g. "this precondition is used by P_pay Part 3" — it is pointing *forward* to show the clause is not idle (the minimality obligation), not borrowing a §9 result. The dependency therefore runs strictly spec → lemma → property, so the §9 proofs that rest on these lemmas are not circular.
