...
| `parent_remains_canonical(B)`, `parent_in_previous_slot(B)`                                       | Strengthening hypotheses introduced in §4.`parent_remains_canonical(B)` ("parent(B) remains in the canonical chain of every honest validator at every later time") is used by the §8.4 **P1 and P2** proof sketches — it closes the split-vote-at-slot-$N$ reorg gap left by the cautious-reveal threshold in **A6**. `parent_in_previous_slot(B)` ("$B$'s parent is at slot $B.\mathrm{slot} - 1$") is used by the §8.4 **P2** proof sketch only — it closes the late-prior-slot gap exploitable by a colluding slot-$N{-}1$ / slot-$N$ Byzantine pair, and covers all multi-missing-slot configurations uniformly (the assumption fails whenever any slot between parent($B$) and $B$ is missing). P1 does not need this strengthening because its own 60%-honest-quorum hypothesis on $B$ already excludes the late-prior-slot competitor. Neither is a spec function; see §4 for full motivation and open work to weaken them. |
...
*(1) `parent_remains_canonical(B)`* — parent(B) remains in the canonical chain of every honest validator at every later time. **Used by P1 and P2.** Closes a **split-vote-at-slot-N** gap: a Byzantine slot-N proposer publishing $B$ late enough to fragment honest attestations, so that some honest validators vote on a sibling branch of parent($B$) instead of on $B$.
...
**An honest builder can self-enforce `parent_in_previous_slot(B)`.** Under **S1** ($\Delta < T_{\mathrm{att}}$), an honest builder for slot $N$ observes the beacon-block gossip layer and knows, by bid-construction time, which of the preceding slots produced a block. If no slot-$N{-}1$ block has been received — and more generally, if any slot between the observed canonical head and $N$ is missing — then any slot-$N$ block $B$ the proposer publishes will necessarily be parented at an older ancestor, falsifying `parent_in_previous_slot(B)` and exposing a reveal to the late-prior-slot attack above. P2 simply does not apply in that configuration. **An honest builder can therefore refuse to bid for slot $N$ whenever slot $N{-}1$ is observably empty (and, by extension, whenever any earlier slot since the canonical head is missing).** This turns `parent_in_previous_slot(B)` from an external hypothesis into a precondition the builder self-enforces at bid time — without any protocol change. Refusing to bid is strictly safer than bidding into a configuration where P2 has a known attack vector: the builder loses the (forgone) MEV from that one slot but avoids paying the bid value while having its payload reorged.

The §8.4 proof sketches rely on these hypotheses explicitly: P1 relies on (1) only; P2 relies on both.
