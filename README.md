...
- *Trustless (`bid.value > 0`).* Assume $B$ extends a block $B'$ at slot $N{-}1$ that remains canonical forever, **S1** (synchrony), **S2** (β < 20%), **S3** (slot-$N$ PTC majority is honest, and honest PTC members wait to observe both the payload and the blob data before voting), and $B$ is timely (Definition in §9.1: $t + \Delta \leq T_{\mathrm{att}}$).
...
*Part (i) — canonicity.* The builder reveals only when A1a fires, in particular when conditions (i), (ii), (iii) all hold: $t_{\mathrm{rev}} \geq T_{\mathrm{att}} + \Delta$; `real_attestation_weight(store, B) ≥ 0.4 W`; and no proposer equivocation against `block.slot` is visible. By the §5 "Why check equivocation" two-case analysis: any equivocation $B''$ broadcast at $t_{\mathrm{eq}} \leq T_{\mathrm{att}}$ is delivered to the builder by $t_{\mathrm{eq}} + \Delta \leq T_{\mathrm{att}} + \Delta \leq t_{\mathrm{rev}}$, so A1a (iii) would fire and the builder would withhold — contradicting the reveal hypothesis. Hence any equivocation $B''$ at slot $N$ was broadcast at $t_{\mathrm{eq}} > T_{\mathrm{att}}$, after every honest slot-$N$ validator had already voted at $T_{\mathrm{att}}$. Under timeliness ($B$ broadcast at $t$ with $t + \Delta \leq T_{\mathrm{att}}$) and **S1**, every honest slot-$N$ validator receives $B$ by $T_{\mathrm{att}}$ and votes for $B$. By **S2**, the honest weight on $B$ is $\geq (1-\beta) W > 80\% W$ — comfortably above the 40% threshold of A1a (ii). By the hypothesis "$B$ extends $B'$ at slot $N{-}1$, $B'$ canonical forever", $B$ sits on the canonical branch at slot $N$. We now apply the same anti-reorg arithmetic as P1's proof (§9.3 P1): $B$ has $\geq (1-\beta) W > 80\% W$ honest weight at slot $N+1$, while any boost-only competitor has at most $40\%W$ (boost) $+ \beta W < 60\% W$; no boost-only reorg succeeds. The remaining reorg vectors at the parent level and at a missing slot $N-1$ are closed by the hypotheses "$B'$ remains canonical forever" and "$B$ extends $B'$ at slot $N{-}1$" respectively. Hence $B$ remains in the canonical chain at every later time. (P1 itself is stated under "honest proposer" rather than under "timely + non-equivocation"; here we derive the same anti-reorg conclusion directly from P4's hypothesis set, since A1a's reveal precondition supplies the non-equivocation fact P1 would otherwise inherit from "honest proposer".)
...
*Assumptions used (trustless case):* **S1**, **S2**, **S3** (honest slot-$N$ PTC majority, with honest members waiting to observe both the payload and the blob data before voting), **A1a** (honest builder follows the cautious-reveal protocol, including broadcasting the blob sidecars), **timeliness** ($t + \Delta \leq T_{\mathrm{att}}$), and the configuration hypothesis "$B$ extends $B'$ at slot $N{-}1$, $B'$ canonical forever"; plus the Phase 0 / 3 / 4 handler code and §6 fork-choice code. Part (i)'s anti-reorg argument re-derives the same conclusion that P1 (Block safety) reaches under "honest proposer"; here P4 supplies the equivalent fact (no-equivocation from A1a (iii) plus the explicit "$B$ extends $B'$" hypothesis) instead of inheriting it from P1's honest-proposer hypothesis.
