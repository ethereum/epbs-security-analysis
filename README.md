...
**Spec version.** This document base-targets [`ethereum/consensus-specs`](https://github.com/ethereum/consensus-specs) at commit [`8e19f82b`](https://github.com/ethereum/consensus-specs/commit/8e19f82b04d92712ff424fa5b95ac4333f88b7d6), Gloas fork: [`specs/gloas/`](https://github.com/ethereum/consensus-specs/tree/8e19f82b04d92712ff424fa5b95ac4333f88b7d6/specs/gloas); inline `file:line` references are to that base commit. Where later changes affect the payment/slashing machinery analysed here, the prose and shown function bodies reflect them and say so — specifically `consensus-specs` PRs **#5364** (pass only the signed bid to `process_execution_payload_bid`, changing its signature to `(state, signed_bid)`), **#5368** (record the bid's proposer via `get_beacon_proposer_index(state)`), and **#5365** (clear a builder IOU on proposer slashing only when the slashed validator is the IOU's recorded proposer — added in response to this analysis; see Lemma A.6). The Gloas specs are work-in-progress and may differ from the EIP-7732 draft summary.
...
- **Distinct duty.** The PTC casts a **payload timeliness attestation** at 75% of the slot, reporting two independent signals: `payload_present` (whether the member observed the `SignedExecutionPayloadEnvelope` arrive) and `blob_data_available` (whether the member's sampled blob data columns arrived and passed KZG verification). Both are binary observations, never validity judgments. The spec object is a `PayloadAttestationMessage` ([`beacon-chain.md`](https://github.com/ethereum/consensus-specs/blob/8e19f82b04d92712ff424fa5b95ac4333f88b7d6/specs/gloas/beacon-chain.md)); we follow the PBS literature and use the shorthand **witness statement** (or **witness vote**), since the PTC is *witnessing* payload and blob-data arrival, not judging execution validity. Phase 4 (§5) details the two-signal mechanics.
...
We've referred to "the builder's bid" loosely; we now make the object concrete. The bid is the `ExecutionPayloadBid` SSZ container, wrapped in `SignedExecutionPayloadBid` for transport. Spec source: [`beacon-chain.md`](https://github.com/ethereum/consensus-specs/blob/8e19f82b04d92712ff424fa5b95ac4333f88b7d6/specs/gloas/beacon-chain.md) §`ExecutionPayloadBid`. The fields:
...
The spec prescribes only the minimal honest rule: reveal when the block is timely and is the head of the local chain ([`builder.md`](https://github.com/ethereum/consensus-specs/blob/8e19f82b04d92712ff424fa5b95ac4333f88b7d6/specs/gloas/builder.md)). The §4 property statements (P_rev, P_withhold) reference *cautious-reveal protocols* that strengthen this minimum. Two protocols, one per case; an honest builder picks the one that matches their bid's payment field:
...
*Minimality (obligation 4).* The lemma claims exactly the proposer-bound clear, and the proposer-match guard is the load-bearing clause, used in opposite directions by the two §9 properties. P_withhold Case 2b uses the *positive* direction: when the slot-`N` proposer equivocates, the slashing's `header_1.proposer_index` equals the IOU's recorded `proposer_index`, so the clear fires and the withholding builder is not charged. P_pay uses the *converse*: an honest, non-equivocating proposer produces no valid slashing of itself, and a foreign validator's slashing fails the guard, so an honest proposer's IOU is **never** cleared by a slashing — the exact guarantee P_pay's negative cite needs, and now an honest statement rather than an over-claim. An empty/default entry (`proposer_index = 0`, `amount = 0`) is matched only by a slashing of validator index 0, and clearing it is a harmless no-op (it is already zero). **History (the gap this lemma's guard closes).** Before `consensus-specs` PR #5365 the clear was *slot-keyed*: it zeroed slot `N`'s entry on any valid slashing carrying `header_1.slot == N`, with no proposer match. The four-phase analysis behind this appendix found that a foreign slashable validator `V ≠ P` could sign two distinct headers for slot `N` (valid equivocation evidence for `V`, never the slot's proposer) and thereby clear honest proposer `P`'s live IOU — denying `P` the payment whenever no FULL settlement preempted it (P_pay's Case B path). That is exactly the over-clear obligation 4 exists to catch; it was escalated and fixed upstream by the `proposer_index` guard proved here. The guard's correctness is *entangled* with Lemma A.2 (which establishes that `proposer_index` is the slot's real proposer) and with the `process_epoch`-before-`process_block` ordering (Lemma A.4's boundary). ∎
