...
- **"parent($B$) will remain canonical forever"** — The strengthening canonicity precondition embedded in P1, P4 (trustless case), and P6's statements (§4), where parent($B$) is the canonical block at slot $N{-}1$. Rules out a configuration where a Byzantine slot-$N$ proposer publishes $B$ late and fragments honest votes between $B$'s branch and a competing branch under an older common ancestor, causing parent($B$) to lose canonicity. The companion missing-slot configuration (a colluding slot-$N{-}1$ / slot-$N$ Byzantine pair exploiting a missing intervening slot) is excluded automatically: parent($B$)'s existence at slot $N{-}1$ leaves no missing slot to exploit. The fact that $B$ extends parent($B$) is derived from the other hypotheses in each property (automatic for an honest proposer in P1/P6; derived from A1a (ii) plus parent($B$)'s canonicity in P4 trustless). P4's non-trustless case substitutes the confirmation rule's $\Sigma_R$ (§5 Phase 3 A1b) for this hypothesis. Not a spec function; see §4 for full motivation and open work to weaken it.
...
**P1: Block safety.** Let $B$ be a block proposed in slot $N$ by an **honest proposer**. Assume parent($B$) is at slot $N{-}1$ and remains canonical forever, **S1** (synchrony) and **S2** (β < 20%). If $B$ includes a valid bid, then $B$ remains in the canonical chain at every later time.
...
*Assumptions used:* **S1**, **S2**, **honest slot-$N$ proposer** (subsumes timeliness and non-equivocation, see §4 P1 note); plus §6 `get_weight` / `get_head` code (boost-only-reorg arithmetic) and the Phase 2 attest handler (honest same-slot voting behaviour). The configuration preconditions on $B$ ($B$ extends parent($B$) at slot $N{-}1$ and parent($B$) remains canonical forever) supply the missing-slot exclusion and the parent-canonicity needed for the boost-only-reorg argument.
...
*Anti-reorg.* By the same arithmetic as P1's proof (§9.3 P1): $B$ has $\geq (1-\beta) W > 80\% W$ honest weight at slot $N+1$, while any boost-only competitor has at most $40\% W$ (boost) $+ \beta W < 60\% W$; no boost-only reorg succeeds. The remaining parent-level reorg vector is closed by "parent($B$) remains canonical forever"; the missing-slot configuration is excluded because parent($B$) exists at slot $N{-}1$. Hence $B$ remains in the canonical chain at every later time.
...
*Assumptions used:* **S1**, **S2**, **honest slot-$N$ proposer** (inherited via P1), P6's **attestation-inclusion hypothesis** (all honest slot-$N$ attestations are included on the canonical chain before the start of epoch $\mathsf{epoch}(N) + 2$; required for Path B Case 1b's quorum), **G-BidAdmit**, **G-Settle**, **G-PayAttest**, **G-PayEpoch**, **G-Solvency**. The configuration preconditions on $B$ ($B$ extends parent($B$) at slot $N{-}1$, parent($B$) remains canonical forever, `bid.value > 0`) feed canonicity into P1 and arm the IOU machinery. The queue → EL credit step relies on the standard withdrawal-pipeline liveness (FULL blocks continue to be produced); the §7 fold-out works through the timing.
