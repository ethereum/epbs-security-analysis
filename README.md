...
**(G-PayAttest)** *(proved as Lemma A.1, Appendix A.)* `process_attestation` selects the pending-payment entry for `attestation.data.slot` from the upper half of `builder_pending_payments` when the attestation target is in the current epoch, and from the lower half when the target is in the previous epoch. It increments that entry's `payment.weight` by `effective_balance(i)` exactly when (a) at least one new participation flag is set for validator `i`, (b) `is_attestation_same_slot(state, data)` holds, and (c) `payment.withdrawal.amount > 0`. No other code path increments `payment.weight`; settlement and slashing can only replace the whole entry with a fresh zero entry. **Precondition (b) is *head-binding*, not merely slot-keyed:** for `data.slot > 0`, `is_attestation_same_slot` returns True only when the attestation's `beacon_block_root` equals the canonical block at `data.slot` (and differs from the previous slot's), so an attestation credits the slot-`data.slot` entry *only if it votes for that slot's canonical block* — votes for a sibling, an older head, or a skipped slot credit nothing. (At the genesis slot `data.slot = 0` the helper short-circuits to True, but no IOU exists there, so precondition (c) credits nothing — see Lemma A.1.)

*Status — proved as Lemma A.1 (Appendix A).* The full body of `process_attestation` is not shown inline; §7 displays only the new ePBS payment-weight step. Appendix A discharges the three preconditions and the "no other path increments `payment.weight`" invariant against the spec — including a frame grep for every writer of `payment.weight` across the full inheritance hierarchy. These are the guarantees P_pay's Part 3 (no double-charge) and P_withhold's Case 1 (quorum unreachable) both cite.
...
- **Governed quantity** — the exact piece of state the lemma constrains, and the thing whose every write the frame obligation must enumerate. Naming it precisely — *which* field, on *which* entries, and *which kind* of write — is what makes "no other path" checkable. For Lemma A.1 it is *positive deltas to `payment.weight`*, so the zeroing that settlement and slashing perform falls outside the claim by construction (zeroing is not a positive delta).
- **Function of record** — the single spec function the lemma holds responsible for that effect. The trigger and effect obligations check that this function does what the lemma claims; the frame obligation checks that it is the *only* spec path that does. The grep exists to confirm no second, unnamed writer of the governed quantity has been overlooked.

### Lemma A.1 (G-PayAttest)

**Governed quantity.** `payment.weight` of any `state.builder_pending_payments[*]` entry — specifically, positive deltas to it. **Function of record:** `process_attestation` (`beacon-chain.md:1685`).
...
*Trigger (obligation 1).* The function asserts `data.target.epoch ∈ {previous, current}` and `data.target.epoch == compute_epoch_at_slot(data.slot)` (`beacon-chain.md:1687–1688`), fixing the two legal ring-buffer halves for `data.slot`. The half is then selected by `data.target.epoch == get_current_epoch(state)` — upper at `beacon-chain.md:1721`, lower at `beacon-chain.md:1725`. Inside the per-index loop over `get_attesting_indices(...)` (a deduplicated `Set[ValidatorIndex]`, `beacon-chain.md:1728`), the three preconditions are checked as a single conjunction (`beacon-chain.md:1746–1750`): (a) `will_set_new_flag`, set `True` only after a participation flag absent at `beacon-chain.md:1735` is added at `beacon-chain.md:1738` — a real state update, not a prediction; (b) `is_attestation_same_slot(state, data)`, whose body (`beacon-chain.md:500–511`) compares `data.beacon_block_root` against `get_block_root_at_slot(state, data.slot)`, which returns the canonical history entry `state.block_roots[...]` (`phase0/beacon-chain.md:1021–1026`) — a genuine head-bind, not a field-compare, and the `!= get_block_root_at_slot(state, data.slot - 1)` conjunct rejects skipped slots; (c) `payment.withdrawal.amount > 0`, a strict `>`.

*Effect (obligation 2).* When all three hold, the sole mutation is `payment.weight += state.validators[index].effective_balance` (`beacon-chain.md:1751`) — magnitude exactly the attester's effective balance, no multiplier. The mutated entry is written back to the same index it was read from (`beacon-chain.md:1762–1765`); this re-stores the enumerated mutation and introduces no second delta.

*Frame — no other path (obligation 3).* A grep for `.weight` across the full spec hierarchy returns exactly two sites: the quorum **read** `payment.weight >= quorum` (`beacon-chain.md:1077`) and the **write** above (`beacon-chain.md:1751`). No `payment.weight = …` field-assignment exists anywhere, so every other mutation of an entry is a whole-entry replacement, enumerated by grepping `builder_pending_payments` / `BuilderPendingPayment(`. The complete writer list is: bid-time IOU creation, which sets `weight=0` **explicitly** (`beacon-chain.md:1476–1484`, gated on `bid.value > 0`); settlement, which replaces the entry with `BuilderPendingPayment()` (`beacon-chain.md:935`); proposer-slashing, which does the same within the 2-epoch window (`beacon-chain.md:1820,1823`); the epoch-boundary ring shift, which relocates the existing upper-half weights into the lower half and zero-fills the new upper half without changing any weight (`beacon-chain.md:1080–1082`); and the fork-upgrade zero-fill (`fork.md:177`). Every no-argument `BuilderPendingPayment()` zeroes both `weight` and nested `withdrawal.amount` by SSZ default-value rules (`ssz/simple-serialize.md:125–148`; `Gwei` aliases `uint64`, `phase0/beacon-chain.md:169`); no constructor anywhere sets a non-zero weight. The pre-Gloas `process_attestation` bodies (`phase0`, `altair`, `deneb`, `electra`) predate `builder_pending_payments` and write no weight; Gloas fully overrides the function. Sibling fork `heze` and feature branch `_features/eip8148` are not Gloas ancestors (Gloas upgrades from `fulu.BeaconState`, `fork.md:120–123`) and never execute under the Gloas transition. Thus the only spec site adding a positive quantity to any `payment.weight` is `beacon-chain.md:1751`; the "no other path" clause is earned. Single-shot follows from precondition (a): a second attestation from the same validator for the same slot sets no new flag, so no second credit accrues — the basis for P_pay Part 3's once-per-validator accounting.

*Minimality (obligation 4).* All three preconditions are enforced (not assumed) and each is used downstream: (a) by P_pay Part 3 (once per validator); (b)'s head-binding by P_withhold Case 1 (the derived `< 0.40 W` honest bound) and P_pay Case B (only votes *for* the canonical block credit); (c) by P_pay Part 3 (settlement zeroes `amount`, so no further weight accrues) and P_withhold's `bid.value = 0` case. The effect is not over-claimed. The one boundary case: `is_attestation_same_slot` short-circuits to `True` at `data.slot = 0` (`beacon-chain.md:504–505`), so at the genesis slot precondition (b) is not head-binding. This is vacuous and breaks no §9 step: a positive IOU is created only by `process_execution_payload_bid` with `bid.slot == block.slot` (`beacon-chain.md:1467–1468`), and a processed block has `block.slot > state.latest_block_header.slot` after `process_slots` advances `state.slot` strictly (`phase0/beacon-chain.md:1370–1380,1396–1403,1886–1890`); genesis installs the latest header without processing a slot-0 block (`phase0/beacon-chain.md:1308–1315`). So the slot-0 entry's `amount` stays `0` and precondition (c) credits nothing. ∎
