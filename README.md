...
**Spec version.** This document targets [`ethereum/consensus-specs`](https://github.com/ethereum/consensus-specs) at commit [`aff96a12`](https://github.com/ethereum/consensus-specs/commit/aff96a12c33192324d07339681f2c4dcc339ea13), Gloas fork: [`specs/gloas/`](https://github.com/ethereum/consensus-specs/tree/aff96a12c33192324d07339681f2c4dcc339ea13/specs/gloas); inline spec-file references are to that commit. The Gloas specs are work-in-progress and may differ from the EIP-7732 draft summary.
...
- **Registration.** Builders register by submitting an execution-layer `BuilderDepositRequest` (EIP-8282), processed by `process_builder_deposit_request`. The request's `withdrawal_credentials` encodes the builder version in its first byte and the withdrawal execution address in its last 20 bytes; payload builders use `PAYLOAD_BUILDER_VERSION = 0`, which `process_execution_payload_bid` asserts on every bid. Processing assigns a `BuilderIndex`, and the builder becomes active once their deposit is finalized. (A one-time fork-time path, `onboard_builders_from_pending_deposits`, also onboards builders with `0x03`-credentialed pending deposits already in the validator-deposit queue at the Gloas fork; post-fork, `BuilderDepositRequest` is the only path.)
...
class ExecutionRequests(Container):
    ...
    builder_deposits: List[BuilderDepositRequest, MAX_BUILDER_DEPOSIT_REQUESTS_PER_PAYLOAD]  # EIP-8282
    builder_exits: List[BuilderExitRequest, MAX_BUILDER_EXIT_REQUESTS_PER_PAYLOAD]            # EIP-8282
```
...
We've referred to "the builder's bid" loosely; we now make the object concrete. The bid is the `ExecutionPayloadBid` SSZ container, wrapped in `SignedExecutionPayloadBid` for transport. Spec source: [`beacon-chain.md`](https://github.com/ethereum/consensus-specs/blob/aff96a12c33192324d07339681f2c4dcc339ea13/specs/gloas/beacon-chain.md) §`ExecutionPayloadBid`. The fields:
...
The spec prescribes only the minimal honest rule: reveal when the block is timely and is the head of the local chain ([`builder.md`](https://github.com/ethereum/consensus-specs/blob/aff96a12c33192324d07339681f2c4dcc339ea13/specs/gloas/builder.md)). The §4 property statements (P_rev, P_withhold) reference *cautious-reveal protocols* that strengthen this minimum. Two protocols, one per case; an honest builder picks the one that matches their bid's payment field:
...
- **A deposit to an exited, swept builder re-arms the delay.** A deposit to an existing builder credits its balance (`process_builder_deposit_request`, `beacon-chain.md`). If that builder had exited and been fully swept (`balance == 0`), the deposit *also* resets `withdrawable_epoch = current_epoch + MIN_BUILDER_WITHDRAWABILITY_DELAY` (`beacon-chain.md`) — roughly **36 days** (`MIN_BUILDER_WITHDRAWABILITY_DELAY = 8,192` epochs, `beacon-chain.md`).

Together these close the obvious recycling loop. The sweep zeroes a builder's balance; refilling it does not put it straight back into the sweep, because the top-up pushes `withdrawable_epoch` ~36 days into the future. An adversary therefore cannot cycle the same stake through the sweep to keep the shared budget saturated: sustaining a full builder sweep would require a continuous supply of *freshly aged* builders — new capital held idle for the full delay — not recycled funds. And even during such an attempt the reserved validator-sweep slot bounds the worst case: honest validator withdrawals keep draining at least one per block.
...
- **(L_withdraw) Withdrawal-pipeline liveness (lifecycle-liveness family; P_pay).** Canonical FULL blocks with accepted execution payloads keep being produced: `process_withdrawals` runs its withdrawal stages only in a block whose predecessor's payload was applied (it returns early otherwise), so without an ongoing supply of such blocks every withdrawal queue and sweep freezes. In P_pay, two further clauses are used: the `builder_pending_withdrawals` list has spare capacity (is below `BUILDER_PENDING_WITHDRAWALS_LIMIT`) at each settlement append point; and after a `BuilderPendingWithdrawal` is queued, such blocks continue often enough to drain the builder-pending-withdrawal queue. The block that drains the entry also has its execution payload accepted by the EL, so the `Withdrawal` emitted for the proposer's `fee_recipient` is actually credited on the execution layer.
...
*Assumptions used:* **S1**, **S2** (applied at slot $`N{+}1`$ and, for the "forever" conclusion, at each later slot — the Gasper persistence step), **honest slot-$`N`$ proposer** (subsumes timeliness and non-equivocation, see §4 P_can note), and **L_parent** (only when $`B`$ declares its parent FULL — supplies admission of $`B`$ at every honest node despite the `on_block` parent-FULL assertion); plus §6 `get_weight` / `get_head` code (boost-only-reorg arithmetic) and the Phase 2 `attest` handler (honest same-slot voting behaviour). The configuration preconditions on $`B`$ ($`B`$ extends $`B'`$ at slot $`N{-}1`$ and $`B'`$ remains canonical forever) supply the missing-slot exclusion, the parent-canonicity needed for the boost-only-reorg argument, and — via the operational reading (§4) — the honest-head containment used at the attestation step. **S3 and S4 are not used:** admission routes through **L_parent**, not the PTC, so PTC honesty (**S3**) is irrelevant — a Byzantine PTC can at most drive the honest proposer to declare its parent EMPTY, which leaves $`B`$ admissible at every honest node and equally canonical; and **S4** is the global restrictive adversary bound (§4), never a per-proof premise — here **L_parent** already excludes the role-oblivious split S4 would otherwise have to constrain.
...
*Honest weight on $`B`$.* Under timeliness and **S1**, every online honest slot-$`N`$ validator receives $`B`$ by $`T_{\mathrm{att}}`$, and by *Admission of $`B`$* it admits $`B`$. Combined with $`B`$ extending the canonical $`B'`$ — whose subtree contains every online honest head at $`T_{\mathrm{att}}`$, by the hypothesis's operational reading (§4) — and the absence of any visible equivocation at $`T_{\mathrm{att}}`$, every online honest slot-$`N`$ validator's `get_head` returns $`B`$, so each votes for $`B`$. By **S2**, the honest online weight on $`B`$ is $`\geq (1-\beta) W > 80\% W`$.
