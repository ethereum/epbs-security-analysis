...
**P1: Block safety.** Let $B$ be a block at slot $N$ extending a block $B'$ at slot $N{-}1$ and $B'$ will remain canonical forever. Assume **S1** (synchrony), **S2** (β < 20%). If $B$ includes a valid bid and is timely (Definition in §9.1: $t + \Delta \leq T_{\mathrm{att}}$), then $B$ remains in the canonical chain at every later time.
...
**P4: Builder revealing protection.** There exists a protocol an honest builder can follow such that, if the builder reveals its payload at slot $N$ (call the block $B$), then `bid(B).block_hash` is in the payload hash chain of the canonical beacon chain (equivalently, $B$ is FULL on chain; and by P3, the corresponding payload and blob data are available and valid). Two cases:
...
> **Property P6: Unconditional payment.** For a block at slot N with `bid.value > 0` that is timely (under P6's preconditions: $B$ extends $B'$ at slot $N{-}1$ and $B'$ remains canonical forever), and given P6's two chain-progression hypotheses (a canonical block at slot $\geq$ start of epoch $e_N+2$ exists; all honest slot-$N$ attestations on $B$ are included on the canonical chain before that boundary), the proposer's payment is queued within at most two epochs of slot N: at slot N+1 via Path A when a subsequent block declares the slot FULL on chain, or at the end of epoch *e+1* via Path B when no subsequent block declares the slot FULL (the natural case: the builder withheld). In the Path B case, the beacon block's same-slot attestation weight reaches the 60% quorum because timeliness + the canonicity precondition + S1 + S2 force at least $(1-\beta) W > 0.8 W$ of honest weight on the beacon block — accumulated into `payment.weight` only because the attestations are included on chain, and faced with the quorum check only because a canonical block at slot $\geq$ start of epoch $e_N+2$ runs `process_epoch`; see §9.3 P6 proof Case 1b. After the IOU is cleared by either path, no second payment is ever produced: `process_attestation` skips zero-`amount` entries and `process_builder_pending_payments` checks `weight ≥ quorum` on a zeroed entry, which fails. The payment is therefore single and bounded in time.
...
P6's "queued within at most two epochs" timing bounds the moment the chain *records the commitment* — a `BuilderPendingWithdrawal` entry appended to `state.builder_pending_withdrawals` via Path A or Path B. The **EL credit** (the actual debit of the builder's CL balance and credit to the proposer's `fee_recipient` at the EL) follows later, via `process_withdrawals`, and its timing cannot be precisely bounded by the protocol.
...
*Part (i) — canonicity.* The builder reveals only when A1a fires, in particular when conditions (ii) and (iii) hold: `real_attestation_weight(store, B) ≥ 0.4 W` and no proposer equivocation against `block.slot` is visible. By **A1a** condition (iii), no such equivocation is visible at the builder by reveal time $t_{\mathrm{rev}} \geq T_{\mathrm{att}}$ (condition (i)); by **S1**, any equivocation visible to *some* honest node by $T_{\mathrm{att}}$ would have been visible to the builder by $t_{\mathrm{rev}}$, so no equivocation is visible to any honest slot-$N$ validator either. Under timeliness ($B$ broadcast at $t$ with $t + \Delta \leq T_{\mathrm{att}}$) and **S1**, every honest slot-$N$ validator receives $B$ by $T_{\mathrm{att}}$; combined with the absence of any visible sibling at slot $N$, every honest slot-$N$ validator votes for $B$. By **S2**, the honest weight on $B$ is $\geq (1-\beta) W > 80\% W$ — comfortably above the 40% threshold of A1a (ii) — and $B$ is the canonical head at slot $N$. Combined with the hypothesis that some canonical block $B'$ exists at slot $N{-}1$, $B$ must extend $B'$. P1's hypotheses ($B$ extends $B'$, $B'$ remains canonical forever, **S1**, **S2**, $B$ timely) are therefore all satisfied, and by **P1** (Block safety), $B$ remains in the canonical chain at every later time.
...
- *Case 1b (Path B; quorum gate at epoch boundary).* No subsequent canonical block declares the slot FULL. By **P1** (Block safety), $B$ stays canonical, so the IOU created at $B$'s processing time persists in state. By the timeliness hypothesis combined with **S1** and the Phase 2 attest handler, every honest slot-$N$ attester casts a same-slot attestation for $B$ at $T_{\mathrm{att}}$; by **S2** the honest weight on $B$ is at least $(1-\beta) W > 0.8 W$. By hypothesis (2), all such attestations are included on the canonical chain before the start of epoch $e_N + 2$ (consistent with the spec's attestation inclusion window, which permits slot-$N$ attestations to be included up to the end of epoch $e_N + 1$). By **G-PayAttest**, each inclusion increments `payment.weight` by the attester's effective balance, accumulating to $\geq (1-\beta) W > 0.8 W \geq$ `get_builder_payment_quorum_threshold(state)`. By hypothesis (1), a canonical block at slot $\geq$ start of epoch $e_N + 2$ exists; its processing invokes `process_slots`, crossing the epoch-$e_N + 1$ boundary and running `process_epoch`, which calls `process_builder_pending_payments`. By **G-PayEpoch**, the quorum check passes and the `BuilderPendingWithdrawal` is appended at that point.
...
*Assumptions used:* **S1**, **S2**, **timeliness** ($t + \Delta \leq T_{\mathrm{att}}$; used via **P1**), P6's hypothesis (1) (a canonical block at slot $\geq$ start of epoch $e_N+2$ exists; required for Path B Case 1b to fire the epoch boundary), P6's hypothesis (2) (attestation inclusion before start of epoch $e_N + 2$; required for Path B Case 1b's quorum), **G-BidAdmit**, **G-Settle**, **G-PayAttest**, **G-PayEpoch**, **G-Solvency**. The configuration preconditions on $B$ ($B$ extends $B'$ at slot $N{-}1$, $B'$ remains canonical forever, `bid.value > 0`) feed canonicity into P1 and arm the IOU machinery.
