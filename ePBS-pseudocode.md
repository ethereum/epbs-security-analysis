...
| A    | P2: Payload-independent consensus progress | Fork-choice advances past missed payloads regardless of FULL/EMPTY mix; the canonical chain extends through honest-majority slots                                                                                                                                                                            | **Lemma 8b** (standalone statement) + Algorithm 2 + Lemma 1 + Lemma 8 (FULL/EMPTY children always available; PENDING resolves to one of {FULL, EMPTY} at slot $N{+}1$) — the structural property that EMPTY is a *defined* outcome, not a chain-halt                                                                                                                          |
...
| C    | P6†: Unconditional payment to the proposer (trustless-only) | For a block at slot $N$ with `bid.value > 0` extending $B'$ at slot $N{-}1$ ($B'$ canonical forever), with $B$ timely, $B$'s slot-$N$ proposer not equivocating, and given P6's two **chain-progression hypotheses** (1) a canonical block at slot $\geq$ start of epoch $e_N+2$ exists; (2) all honest slot-$N$ attestations on $B$ are included on the canonical chain before that boundary, the proposer's `fee_recipient` is eventually credited with the bid amount; payment is single (no double-charge); builder cannot avoid paying | Lemma 11 (combining Lemmas 9 and 10 — settlement via Path A or Path B, with hypotheses (1) and (2) supplying the epoch-boundary fire and quorum weight respectively) + Lemma 12 (single payment — Path A clears Path B; Path C ⊥ Path B because no intermediate canonical block ⟹ no attestations on chain ⟹ Path B quorum fails first) + Lemma 12b (solvency precondition at bid admission) + Lemma 21 (index correctness) + 2-epoch bound via the Helper 14 ring buffer (upper-half write at bid time, lower-half quorum check one epoch later) |
...
**Lemma 8b** (Payload-independent consensus progress — formal proof of P2). *Under the per-committee Byzantine bound $\beta < 1/2$ and network synchrony (S1), the fork-choice rule advances past every slot regardless of whether the slot's payload was revealed or withheld, FULL or EMPTY. Concretely: for any block $B$ at slot $N$ that becomes canonical, `block_status(canonical, B) \in \{\mathrm{FULL}, \mathrm{EMPTY}\}$` is decided at the next canonical slot, and the canonical chain extends past $B$ without halting on payload-availability.*

This is the formal statement of **property P2 (Payload-independent consensus progress) from §4 of the overview**.

*Proof.* By Algorithm 2 (`get_node_children`), the fork-choice tree alternates `PENDING → \{FULL, EMPTY\} → PENDING → \{FULL, EMPTY\}`: every PENDING node has both a FULL and an EMPTY child available (the EMPTY child is unconditionally present; the FULL child exists iff `r ∈ store.payloads` per Lemma 1). At slot $N+1$, the slot-$N$ PENDING node is resolved by the tiebreaker (Algorithm 6 `should_extend_payload`) into either its FULL or EMPTY child, and the canonical chain extends through whichever child wins. If the slot-$N+1$ block was missed, Lemma 8 supplies the same FULL/EMPTY resolution via slot-$N+1$ committee attestations at slot $N+2$. In either case, fork-choice does *not* halt — it produces a defined FULL/EMPTY verdict for slot $N$ and continues. ∎

*Remark.* P2 is a *structural* property of the two-phase fork-choice design (consensus-layer transitions never block on execution-payload arrival): missed payloads degrade to EMPTY on chain rather than producing chain stalls. Together with the standard Gasper liveness properties of the underlying LMD-GHOST + FFG, this gives ePBS the same eventual-liveness guarantees as pre-ePBS consensus, with the addition that EMPTY is now a defined outcome rather than a chain stall.
...
**Case 2: No subsequent block declares FULL (Path A does not fire).** This includes three sub-cases: (a) the builder withholds entirely, (b) the builder reveals an invalid execution payload (rejected by `verify_execution_payload_envelope`, so `store.payloads` is never populated), or (c) the builder reveals a valid execution payload, but no subsequent block builds on B-FULL — this last sub-case is the PTC + proposer collusion scenario analyzed in detail by Lemma 17 below. In all three sub-cases, `settle_builder_payment` is never called, so the `BuilderPendingPayment` persists. Same-slot attesters accumulate `payment.weight` via `process_attestation` (Helper 15). By the same argument established in Case 1 — under S1, S2, **no slot-$N$ proposer equivocation** (the explicit lemma hypothesis, supplying the same fact that H7 (i) supplies in the trustless case where the builder is honest; note H7 itself is *not* a hypothesis of this lemma, since Case 2 sub-cases (a)–(c) need not feature an honest builder), H8, H9, and "timely" publication — every honest slot-$N$ attester votes for $B$, contributing their effective balance to $B$'s `payment.weight` *once their attestation is included on-chain*. By hypothesis (2) (attestation inclusion), all such honest attestations are included on the canonical chain before the start of epoch $e_N+2$, so by Helper 15 their effective balances accumulate into `payment.weight`. The accumulated weight is at least $(1-\beta) W_N > 0.8 W_N > 0.6 W_N$, meeting the 60% quorum threshold. By hypothesis (1) (chain liveness past epoch $e_N+1$), a canonical block at slot $\geq$ start of epoch $e_N+2$ exists; its processing invokes `process_slots`, crossing the epoch-$e_N+1$ boundary and running `process_epoch`, which calls `process_builder_pending_payments`. By Lemma 10, the quorum check passes and the `BuilderPendingWithdrawal` is appended at that point. The proposer gets paid.
