...
- `bid(B).value` (gwei): the **trustless** payment amount. The protocol enforces it: when `value > 0`, `process_execution_payload_bid` (Helper 8) records a `BuilderPendingPayment` IOU in `state.builder_pending_payments` and the unconditional-payment mechanism (Lemma 11, Path A or Path B) settles it.
- `bid(B).execution_payment` (gwei): the **non-trustless** payment amount, an advertised promise the protocol does **not** enforce. Helper 8 never reads `execution_payment`; no IOU is created from it; no Path A or Path B applies. The field is metadata: a builder advertises an off-protocol payment commitment, and the proposer chooses whether to believe it. Delivery happens through some channel outside the consensus layer (typically a transaction inside the revealed execution payload paying `bid.fee_recipient`).
...
**Definition 3c** (Timely block). A beacon block $`B`$ at slot $`N`$ is *timely* iff it is broadcast at a time $`t`$ satisfying $`t + \Delta \leq T_{\mathrm{att}}`$, where $`\Delta`$ is the synchrony delay (Assumption S1). By S1, every honest slot-$`N`$ validator receives a timely block before $`T_{\mathrm{att}}`$, hence before casting its slot-$`N`$ attestation. A non-timely block (broadcast at $`t > T_{\mathrm{att}} - \Delta`$) is delivered only partially to honest validators by $`T_{\mathrm{att}}`$: some receive it in time and vote for it, others do not and vote for the previous head. This split visibility enables the late-block reorg vector against blocks that fall outside this definition. Properties P_can, P_rev (trustless), and P_pay condition their conclusions on $`B`$ being timely in this sense (overview §4 and §9.1).

*Spec reference: pedagogical. Formalises the "timely publication" hypothesis the overview names in §9.1.*
...
A vote is a *same-slot vote* for block B if `slot = slot(B)`. Same-slot voters always set `payload_present = False` because, by Definition 1, the attestation phase (T_att) precedes the builder reveal window: attesters vote at 25% of the slot while the builder has until 75%. At the time of voting, the attester has no way to know whether the payload will arrive. This is formalized in the honest attester behavior (Section 1.2, Attester lines 9–10). (Votes with `slot < slot(B)` are invalid: `validate_on_attestation` asserts `block_slot <= attestation.data.slot`.)

A vote is a *non-same-slot vote* for block B if `slot > slot(B)`. This occurs when no block was proposed in the voter's assigned slot and their head is an earlier block. Non-same-slot voters observe the payload status and set `payload_present` accordingly (Section 1.2, Attester lines 11–12).
...
**Definition 15** (Reveal deadline). The *reveal deadline* for a slot N payload is `t_payload_due = slot_start(N) + get_payload_due_ms()`, parameterised by `PAYLOAD_DUE_BPS` (currently 7500, i.e., 75% of the slot, so `t_payload_due − slot_start(N) ≈ 9s`). A reveal locally observed by an honest PTC member strictly before `t_payload_due` is voted as `payload_present = True`; a reveal observed at or after `t_payload_due` is voted as `payload_present = False`, irrespective of whether `on_execution_payload_envelope` (Algorithm 8) later populates `store.payloads[r]`. The True-timeliness PTC tally for slot N is therefore determined by the set of envelopes observed before `t_payload_due`, frozen at that instant.
...
**Definition 16** (Option window). The *option window* for a slot N bid is the interval `[t_commit, t_payload_due)` whose length equals `(PAYLOAD_DUE_BPS / 10000) × SLOT_DURATION` (currently 9s). Within this window, the builder can decide to reveal (broadcasting a `SignedExecutionPayloadEnvelope`) or to withhold (broadcasting nothing). The decision is unilateral: no consensus message during the window can revoke the bid commitment recorded at `t_commit`. (Reveals that arrive *after* `t_payload_due` but before `t_ptc` may still cause `on_execution_payload_envelope` to populate `store.payloads[r]` at recipients, but they do not contribute to the True-timeliness PTC tally and therefore do not influence the slot-N+1 fork-choice tiebreaker via the PTC primary path — see Lemma 30.)

**Definition 17** (Withhold strategy). A builder *withholds* if, despite having a `BuilderPendingPayment` recorded for its bid at `t_commit`, it does not broadcast a valid `SignedExecutionPayloadEnvelope` before `t_payload_due`. By Algorithm 8, no `store.payloads[r]` entry exists at any honest node by `t_payload_due`, hence no FULL node is created in the fork-choice tree (Definition 6).
