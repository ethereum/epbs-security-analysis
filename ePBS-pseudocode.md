...
† P1 and P2 rely on strengthening hypotheses. **P1 relies on only Assumption H8** ($\mathit{parent}(B)$ remains canonical at every honest validator throughout the trace), which closes the split-vote-reorg gap that would otherwise let $\mathit{parent}(B)$ lose canonicity (and $B$'s IOU with it) under H7's 40% reveal threshold. The orthogonal late-prior-slot gap is already excluded by P1's 60%-honest-quorum hypothesis on $B$. **P2 relies on both H8 and Assumption H9** ($\mathit{parent}(B).\mathit{slot} = N - 1$ — no missing block immediately before $B$, with the assumption failing whenever any slot between $\mathit{parent}(B)$ and $B$ is missing). H9 closes the late-prior-slot gap exploitable by a colluding slot-$(N{-}1)$/slot-$N$ Byzantine pair (full attack trace in H9's statement above). P3 and P4 do not depend on either — they reason about quorum failure and post-admission data availability respectively, both independent of $B$'s canonicity and structural-slot relationships.
...
| (strengthening, P2 only)`parent_in_previous_slot(B)` — overview §4    | **Assumption H9** in §12.4 — $\mathit{parent}(B).\mathit{slot} = N - 1$ (no missing block immediately before $B$; covers all multi-missing configurations between $\mathit{parent}(B)$ and $B$). Used as an explicit hypothesis in Lemma 7 and Lemma 17, and transitively in Lemma 9's second claim. Lemma 11 (P1) does **not** rely on H9 — its 60%-honest-quorum hypothesis subsumes the late-prior-slot bound. Closes the late-prior-slot reorg gap exploitable by a colluding slot-$(N{-}1)$/slot-$N$ Byzantine pair; open work is to weaken it (see H9 statement). |
...
This is a **strengthening hypothesis** used alongside H8 by **Lemma 7 (the P2 proof) and Lemma 17 (the §7 "only successful attack" remark)** to close a residual gap orthogonal to the split-vote-at-slot-$N$ gap H8 addresses. Lemma 9's second claim — that the FULL declaration happens at slot $N{+}1$ under H7's 40% reveal threshold — also depends on H9 (it is used by Lemma 7's Part (a) chain). **Lemma 11 (the P1 proof) does *not* rely on H9**: P1's hypothesis is a $60\%$ honest quorum on $B$'s beacon block, which alone outweighs any late-prior-slot competitor (see Lemma 11's "Role of H8" paragraph). The gap H9 closes: a colluding Byzantine pair at slots $N{-}1$ and $N$ can exploit a missing slot $N{-}1$ to seat a competing sibling $B'$ that reorgs $B$ even under H7 and H8. Concretely:
...
*Remark.* H9 is parallel to H8 in form (a per-property strengthening hypothesis) but narrower in scope: Lemma 7, Lemma 9 (second claim), and Lemma 17 rely on it; Lemma 11 (P1) does not — its 60%-honest-quorum hypothesis is the substitute. Lemmas 16, 18b, 22 are also independent of H9 — quorum failure and post-admission DA do not reason about $B$'s structural slot relationship to its parent. The overview (§4) refers to this strengthening as `parent_in_previous_slot(B)`.
...
**Lemma 11** (Unconditional payment — formal proof of P1). *For any block at slot N with bid.value > 0 that is attested by honest validators with total effective balance meeting the quorum threshold, AND under the strengthening hypothesis H8 ($\mathit{parent}(B)$ remains canonical throughout the trace): the proposer's payment is queued regardless of whether the builder reveals the execution payload. **This lemma is the formal proof of P1 (Unconditional payment to the proposer) from §4 of the overview, supplying the "settlement happens" half of P1's claim. Combined with Lemma 12 (single payment) and the 2-epoch ring-buffer layout of `state.builder_pending_payments` (Helper 8 line 26 writes to the upper half; Helper 14 examines the lower half at the epoch boundary), it also supplies the "within at most two epochs" timing bound stated in P1.** Lemma 12b supplies the solvency precondition that the bid is admittable only if the balance covers it.* The hypothesis `bid.value > 0` automatically excludes self-builds, where Helper 8 line 6 forces `bid.value = 0` (so the proposer would only ever "owe itself" zero, and no IOU is recorded by Helper 8 lines 17–26 — see also Lemma 12b's non-self-build scoping). The lemma is therefore vacuous for self-builds; its content is the unconditional-payment guarantee for external-builder bids only.

*Role of H8 in this lemma (and why H9 is not needed).* The quorum-meeting hypothesis ("attested by honest validators with total effective balance meeting the quorum threshold") presupposes $B$ remains canonical long enough for honest attestations on $B$ to accumulate. A split-vote reorg of $\mathit{parent}(B)$ (where slot-$N{-}1$ honest attestations fragment between parent($B$) and a sibling-of-parent($B$)) would fragment honest weight away from $B$'s branch before the quorum can form; H8 rules out that scenario by fixing parent($B$) canonical. The other attack class — the late-prior-slot sibling reorg closed by H9 in Lemma 7 — does **not** threaten P1 because P1's 60%-honest-quorum hypothesis on $B$ is already strong enough to outweigh any such competitor on its own: a sibling at slot $N{-}1$ (parented at slot $N{-}2$) can collect at most the slot-$N$ honest weight not on $B$ (at most $80\% - 60\% = 20\%$ under $\beta < 20\%$) plus the Byzantine slot-$N$ budget ($< 20\%$) for a total $< 40\%$, while $B$'s subtree carries the full $\geq 60\%$ honest. Hence Lemma 11 needs H8 but not H9.
...
**Case 1: A subsequent block declares FULL (Path A fires).** Suppose the builder reveals via the cautious strategy of Assumption H7 and a honest slot-$N{+}1$ proposer is in place. By the lemma's 60%-honest-quorum hypothesis on $B$, $B$'s PENDING subtree carries $\geq 60\%\, W_N$ of slot-$N$ committee weight at slot $N{+}1$; by H8, parent($B$) is canonical, so the slot-$N{+}1$ proposer's `get_head` walks through parent($B$) and selects the heaviest child of parent($B$). Sibling candidates of $B$ are bounded: any slot-$N$ sibling is excluded by H7 condition (i) (no proposer equivocation at slot $N$); any sibling-of-parent($B$) at an earlier slot (parented above parent($B$) in the case of a missing slot $N{-}1$, $N{-}2$, etc.) can collect at most the slot-$N$ honest weight not on $B$ ($\leq 20\%\, W_N$ under $\beta < 20\%$) plus the slot-$N$ Byzantine budget ($< 20\%\, W_N$) — total $< 40\%\, W_N$, strictly less than $B$'s $\geq 60\%\, W_N$. So $B$ is canonical at slot $N{+}1$. The honest slot-$N{+}1$ proposer, observing $B$-FULL via `should_extend_payload(store, r)` (Algorithm 6, PTC primary path or fallback (c), depending on PTC state), declares `parentStatus = FULL` for $B$. By Lemma 9 (first claim, conditional on FULL declaration), `process_parent_execution_payload` invokes `settle_builder_payment`, which queues the payment and clears the pending entry. The proposer gets paid in the slot-$N{+}1$ block.
