...
† P1 and P2 additionally invoke the strengthening hypothesis **Assumption H8** — $\mathit{parent}(B)$ remains canonical at every honest validator throughout the trace. This closes the split-vote-reorg gap left by H7's cautious-reveal threshold at 40%: a Byzantine slot-$N$ proposer publishing $B$ late can fragment honest attestations between $B$ and a sibling branch of $\mathit{parent}(B)$, and an honest slot-$N+1$ proposer's `get_head` would then pick the heavier sibling branch and reorg $B$. H8 excludes this attack; see the assumption statement above for the open work to weaken it. P3 and P4 do not depend on H8 — they reason about quorum failure and post-admission data availability respectively, both independent of slot-$N$ canonicity.
...
| (strengthening, P1/P2 only) `parent_remains_canonical(B)` — overview §4 | **Assumption H8** in §12.4 — $\mathit{parent}(B)$ remains canonical at every honest validator throughout the trace. Invoked as an explicit hypothesis in Lemma 7, Lemma 9, Lemma 11, Lemma 17. Closes the split-vote-reorg gap left by H7's 40% threshold; open work is to weaken it (see H8 statement). |
...
*Remark.* H8 is parallel in spirit to S4 (per-property honest-actor hypotheses): like S4, it is invoked per-property rather than standing globally. Lemmas 7, 9, 11, 17 invoke H8 for the block under analysis; Lemmas 16, 18b, 22 do not — they are independent of slot-$N$ canonicity. The overview (§4) refers to this strengthening as `parent_remains_canonical(B)`.
...
*Scope of this argument.* The weight comparison above is local to $\mathit{parent}(B)$'s child level — it rules out competitors $B'$ that share $\mathit{parent}(B)$. A separate attack — a Byzantine slot-$N$ proposer publishing $B$ late (at $t > T_{\mathrm{att}} - \Delta$) so that some honest attesters fail to see $B$ by $T_{\mathrm{att}}$ and instead vote on a sibling branch of $\mathit{parent}(B)$ — is excluded by the strengthening hypothesis **Assumption H8** (parent($B$) remains canonical), invoked in this lemma's statement as hypothesis (3). Under H8, no honest fork-choice ever leaves $\mathit{parent}(B)$'s branch, so the only competitors to $B$ are siblings sharing $\mathit{parent}(B)$ — which are excluded by H7 condition (i) (no proposer equivocation at slot $N$) and by the weight arithmetic above.
...
*Role of H8 in this lemma.* The quorum-meeting hypothesis ("attested by honest validators with total effective balance meeting the quorum threshold") presupposes $B$ remains canonical long enough for honest attestations on $B$ to accumulate. A split-vote reorg of $\mathit{parent}(B)$ would fragment honest weight away from $B$'s branch before the quorum can form; H8 rules out that scenario. Without H8, Case 1 (Path A) of the proof body — which appeals to Lemma 9's anti-reorg argument — is the load-bearing dependency; Case 2 (Path B) is robust as long as the quorum can be reached, which itself reduces to H8 for the same reason. Both paths cite H8 transitively via Lemma 7 / Lemma 9.
...
*Why hypothesis (3) is formulated as "declares EMPTY" rather than "reorgs B."* Under H7 (cautious-reveal builder), the strengthening hypothesis H8 ($\mathit{parent}(B)$ stays canonical), and the network-model synchrony assumption, a slot-$N+1$ proposer cannot substitute a boost-only reorg of $B$ for the EMPTY declaration. By Lemma 7 Part (b) (which now relies on H8), every honest slot-$N$ validator has voted for $B$ by $T_{\mathrm{att}}$, and the resulting bound $\mathit{weight}((B.\mathit{root}, \mathrm{PENDING})) \geq (1 - \beta)\, W > 0.8\, W$ at slot $N+1$ strictly exceeds the boost-induced upper bound $\mathit{weight}((B'.\mathit{root}, \mathrm{PENDING})) \leq 0.4\, W + \beta\, W \leq 0.6\, W$. Any sibling-reorg block $B'$ at slot $N+1$ therefore cannot outweigh $B$ at the PENDING level (and a split-vote reorg higher up the chain is excluded by H8). The colluder is forced onto the alternative path of declaring $\mathit{parentStatus} = \mathrm{EMPTY}$ on $B$, which in turn requires the PTC primary path of `should_extend_payload` to fail — exactly hypothesis (2). The combined resource requirement (a bribed PTC majority *plus* a colluding next-slot proposer) is what makes the attack expensive to mount under balance-weighted PTC sampling (Lemma 26).*
