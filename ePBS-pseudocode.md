...
A vote is a *same-slot vote* for block B if `slot = slot(B)`. Same-slot voters always set `payload_present = False` because, by Definition 1, the attestation phase (T_att) precedes the builder reveal window: attesters vote at 25% of the slot while the builder has until 75%. At the time of voting, the attester has no way to know whether the payload will arrive. This is formalized in the honest attester behavior (Section 1.2, Attester lines 9–10). (Votes with `slot < slot(B)` are invalid: the spec asserts `vote.slot >= block.slot` in `is_supporting_vote`.)
...
| `(r, status)` tuple form                                                                      | Pedagogical shorthand for `ForkChoiceNode(root=r, payload_status=status)` (`fork-choice.md` line 109). The algorithms below alternate between the tuple form and the spec container form for compactness.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `FULL`, `EMPTY`, `PENDING`                                                                | Shorthand for the spec constants `PAYLOAD_STATUS_FULL`, `PAYLOAD_STATUS_EMPTY`, `PAYLOAD_STATUS_PENDING` (`fork-choice.md` lines 76–78).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `epoch(N)`                                                                                       | The epoch containing slot $N$, equal to `compute_epoch_at_slot(N) = N // SLOTS_PER_EPOCH`. We write $\mathsf{epoch}(N)$ in math notation and `epoch(N)` in prose. Some earlier drafts of this document used the shorthand $e_N$ for the same quantity; we no longer do. A bound variable named $e$ in a lemma (e.g., Lemma 12c's "exit initiated at epoch $e$") is a *local* epoch variable, unrelated to $\mathsf{epoch}(N)$.                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
The [overview document](https://github.com/ethereum/epbs-security-analysis/blob/master/README.md) (Section 4) lists **six** externally observable properties P1–P6 — claims an observer with full visibility of network messages and on-chain state can verify without inspecting any node's internal state — organised in three categories: **Category A** (payment-trustlessness-independent: P1, P2, P3), **Category B** (payment-trustlessness-dependent, each with two cases: P4, P5), **Category C** (trustless-payment-only: P6). The following table maps each overview property to the formal lemmas proved in this section.
...
| A    | P2: Payload execution deadline is the beginning of the next slot | Let $B$ be a block proposed in slot $N$ and $P$ the payload with `bid(B).block_hash = P.block_hash`. With the only possible exception of builders, the protocol does not require any other honest actor to complete the execution of $P$ before the beginning of slot $N+1$.                                          | **Lemma 8b** (standalone statement, enumerating slot-$\leq N$ honest actors — attesters, PTC, earlier-slot proposers — and verifying none requires $P$ executed) + structural support from Lemma H1 (same-slot attesters signal zero), Lemma H4 (PTC reports observation, not validity), Algorithm 8 (`on_execution_payload_envelope` is the only execution path and is not gated on slot-$\leq N$ actors)                                                                                                                          |
| A    | P3: Data availability for chain inclusion | If a payload hash$h$ is in the payload hash chain (Definition 4c) of the canonical beacon chain — equivalently, $h = \mathit{bid}(B).\mathit{block\_hash}$ for some FULL block $B$ — then the corresponding execution payload is available and valid, and its associated blob data is also available | Lemma 18b (DA confirmed ⟺ hash on chain), under honest super-majority + honest slot-N+1 proposer. P3 claims the (⇒) direction (hash on chain ⇒ DA confirmed); Lemma 18b's biconditional strengthens this                                                                                                                            |
| B    | P4†: Builder revealing protection (trustless case `bid.value > 0`) | Honest reveal under A1a ⇒$\mathit{bid}(B).\mathit{block\_hash}$ is in the payload hash chain (Definition 4c) of the canonical beacon chain — equivalently, $\mathit{block\_status}(\mathit{canonical}, B) = \text{FULL}$ — under honest PTC majority and honest slot-N+1 proposer                      | **Canonicity step**: Lemma 7c (≡ P1), equivalently Lemma 7 Part (b) (anti-reorg argument under β < 20% and PROPOSER_SCORE_BOOST = 40%) under H8 + H9. **FULL-on-chain step**: Lemma 7 Part (a) (FULL/EMPTY tiebreaker selects FULL via PTC primary path) + Lemma 9 second claim (honest slot-N+1 proposer's `should_build_on_full(store, head)` returns True under honest PTC, so the proposer declares `parentStatus = FULL` — Lemma 14c discharges the "no False-DA quorum" precondition) + Lemma 18 (honest builder's bid is consistent with the resulting `parentStatus = FULL` once the parent's execution payload is in the local store) + Lemma 19 (honest proposer's `parent_execution_requests` is consistent with `parentStatus = FULL`) — combined, deliver the "hash in payload hash chain" claim. The two-step structure (canonicity → FULL declaration) mirrors the non-trustless row below, which substitutes Lemma 17b for the canonicity step.     |
| B    | P4: Builder revealing protection (non-trustless case `bid.execution_payment > 0, bid.value = 0`) | Honest reveal under A1b ⇒$\mathit{bid}(B).\mathit{block\_hash}$ is in the payload hash chain — canonicity routed through the confirmation rule $R$ and its assumption set $\Sigma_R$ instead of P1                                                                                                          | **Canonicity step**: Lemma 17b (parametric in $\Sigma_R$ via A1b ≡ H7b's $R$-confirmation precondition). **FULL-on-chain step**: Lemma 7 Part (a) + Lemma 9 second claim + Lemma 18 + Lemma 19 — the FULL-declaration mechanism (honest slot-N+1 proposer's `should_build_on_full` returns True under honest PTC ⇒ proposer declares `parentStatus = FULL` ⇒ next block declares B FULL on chain) is **value-independent**: Lemma 9's second claim is stated under H7 + `bid.value > 0` but the mechanism it proves (honest PTC + honest slot-N+1 proposer ⇒ FULL declaration at slot N+1) does not depend on the bid's payment regime. Re-using it for the non-trustless case requires only substituting H7 → H7b in the hypothesis; the conclusion holds verbatim. The strengthening hypotheses H8/H9 are *not* needed because $\Sigma_R$ delivers stronger canonicity directly. |
| B    | P5: Builder withholding protection (trustless case `bid.value > 0`)        | Honest withhold ⇒ neither builder charged nor proposer paid; Byzantine alone cannot meet quorum                                                                                                                                                                                                             | **Cases 1 + 2a** (no-weight withhold; equivocation reorgs $B$ out): Lemma 16's quorum-failure argument — the 60% = 40% + 20% calibration ensures honest participation is required, and Byzantine contribution is bounded below 20% by S2 (cf. arithmetic in Lemma 16's proof, with Lemma 20 providing the dual reachability arithmetic). **Case 2b** ($B$ retains canonicity; equivocation-driven withhold): Lemma 22 part (iv) — `process_proposer_slashing` clears the `BuilderPendingPayment` before the epoch-boundary check. See *Remark on coverage of P5* after Lemma 16 for the unified Case 1 / 2a / 2b routing.                                  |
...
| C    | P6†: Unconditional payment to the proposer (trustless-only) | For a block at slot $N$ with `bid.value > 0` extending $B'$ at slot $N{-}1$ ($B'$ canonical forever), with $B$ timely, $B$'s slot-$N$ proposer not equivocating, and given P6's **attestation-inclusion hypothesis** (all honest slot-$N$ attestations on $B$ are included on the canonical chain before the start of epoch $\mathsf{epoch}(N)+2$), the proposer's `fee_recipient` is eventually credited with the bid amount; payment is single (no double-charge); builder cannot avoid paying | Lemma 11 (settlement via Path A or Path B — the inclusion hypothesis supplies the quorum weight; under standard Gasper liveness, a canonical block at slot $\geq$ start of epoch $\mathsf{epoch}(N)+2$ eventually fires `process_epoch`) + Lemma 12 (single payment — Path A clears Path B; Path C ⊥ Path B because no intermediate canonical block ⟹ no attestations on chain ⟹ Path B quorum fails first) + Lemma 12b (solvency precondition at bid admission) + Lemma 21 (index correctness) |
...
Several internal mechanisms and non-externally-observable facts underlie the six properties without being properties themselves. They are still proved formally here because the property proofs reference them:

| Internal mechanism                                      | Statement                                               | Proved by                                                                                                                                                                                           |
| ------------------------------------------------------- | ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PTC bounds proposer power                               | PTC overrides a single malicious proposer's EMPTY       | Lemma 14 + adversarial table below — feeds the proof of P4 (revealing protection) since the PTC primary path or fallback (c) is what surfaces the FULL declaration on chain after an honest reveal |
...
| S4 (per-property honesty hypotheses)                                      | Stated as explicit lemma hypotheses where used (e.g., "honest slot-N+1 proposer" in Lemma 7, Lemma 9, Lemma 18, Lemma 18b)                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| (strengthening, P1, P4-trustless, P6)`parent_remains_canonical(B)` — overview §4 | **Assumption H8** in §1.4 — $\mathit{parent}(B)$ remains canonical at every honest validator throughout the trace. Used as an explicit hypothesis in Lemma 7, Lemma 9, Lemma 11, Lemma 17. Closes the split-vote-reorg gap left by A1a's 40% threshold; open work is to weaken it (see H8 statement). P4's non-trustless case routes canonicity through $\Sigma_R$ instead and does **not** carry H8.                                                                                                                                                                                                                                                              |
| (strengthening, P1, P4-trustless, P6)`parent_in_previous_slot(B)` — overview §4  | **Assumption H9** in §1.4 — $\mathit{parent}(B).\mathit{slot} = N - 1$ (no missing block immediately before $B$; covers all multi-missing configurations between $\mathit{parent}(B)$ and $B$). Used as an explicit hypothesis in Lemma 7, Lemma 9 (second claim), Lemma 11, and Lemma 17. Closes the late-prior-slot reorg gap exploitable by a colluding slot-$(N{-}1)$/slot-$N$ Byzantine pair — without H9, $B$'s slot-$N$ attestation weight can be split between $B$ and a late-released sibling at slot $N{-}1$, so the 60% derivation in Lemma 11's Case 2 fails AND $B$ is reorged at slot $N{+}1$. Open work is to weaken it (see H9 statement). P4's non-trustless case does **not** carry H9 — $\Sigma_R$'s canonicity guarantee subsumes it. |
...
**Algorithmic assumptions.** The overview names **six** G-assumptions (payment + slashing machinery) — the fork-choice helpers (`get_node_children`, `get_weight`, `is_supporting_vote`, `on_block`'s parent-FULL assertion, and the `get_head` tiebreaker) live as plain code in the overview's §6 ("Phase 5 and Fork Choice") and are not carried as named assumptions there. The formal-treatment provides the full algorithm/helper code for every spec function the overview references; the six named G-assumptions are discharged below, and the formal-treatment's Algorithms 1–9 give the full fork-choice machinery the overview's §6 cites by function name:
...
| `get_weight` — zero-return rule for FULL/EMPTY of immediately previous slot                                       | Algorithm 5 (lines 3–4) + Lemma 4 (zero-return rule)                                                                                      |
| `is_supporting_vote` — same-slot exclusion; non-same-slot direct attribution by `payload_present`; descendant attribution by chain-structural `parentStatus` | Algorithm 4 Cases 1 and 2 (lines 4–13) + Lemma 2 (same-slot exclusion) + Lemma 5 (descendant attribution)                              |
...
This is a **strengthening hypothesis** used by Lemmas 7, 9, 11, and 17 to close a residual gap that the cautious-reveal threshold of H7 does not address on its own. The gap: a Byzantine slot-$N$ proposer can publish $B$ late (broadcast at $t > T_{\mathrm{att}} - \Delta$), fragmenting honest slot-$N$ attestations between $B$ and a sibling-of-parent($B$) branch. With $\geq 40\%$ of honest weight on $B$ and the rest on the sibling branch, the H7 reveal check still passes — but an honest slot-$N+1$ proposer's `get_head` walks to the heavier sibling branch and reorgs $B$. Forcing $\mathit{parent}(B)$ to remain canonical excludes the sibling branch from honest fork-choice consideration, so the slot-$N+1$ proposer can only extend $\mathit{parent}(B)$; under H7 condition (i) (no proposer equivocation at slot $N$), $B$ is the unique descendant of $\mathit{parent}(B)$ at slot $N$, so it is extended and not reorged.
...
*Remark.* H8 is parallel in spirit to S4 (per-property honest-actor hypotheses): like S4, it is applied per-property rather than standing globally. Lemmas 7, 9, 11, 17 rely on H8 for the block under analysis; Lemmas 16, 18b, 22 do not — they are independent of slot-$N$ canonicity. The overview (§4) refers to this strengthening as `parent_remains_canonical(B)`.
...
This is a **strengthening hypothesis** used alongside H8 by **Lemma 7 (the P4-trustless proof), Lemma 11 (the P6 proof), and Lemma 17 (the §8 "only successful attack" remark)** to close a residual gap orthogonal to the split-vote-at-slot-$N$ gap H8 addresses. Lemma 9's second claim — that the FULL declaration happens at slot $N{+}1$ under A1a's 40% reveal threshold — also depends on H9 (it is used by Lemma 7's Part (a) chain). For P6 in particular, the 60% same-slot quorum on $B$ that Path B presupposes is **derived** in Lemma 11's proof from "timely publication" + S1 + S2 + H8 + H9 — it is not assumed. Without H9, the derivation fails: the late-prior-slot attack splits slot-$N$ honest attestations between $B$ and a late-released sibling $B'$, capping $B$'s `payment.weight` below 60% AND reorging $B$ at slot $N{+}1$. Concretely:
...
*Scope of this argument.* The weight comparison above is local to $\mathit{parent}(B)$'s child level — it rules out competitors $B'$ that share $\mathit{parent}(B)$. Two further attack vectors are excluded by the strengthening hypotheses listed in this lemma's statement: (i) a Byzantine slot-$N$ proposer publishing $B$ late (at $t > T_{\mathrm{att}} - \Delta$) so that some honest attesters fail to see $B$ by $T_{\mathrm{att}}$ and instead vote on a sibling branch of $\mathit{parent}(B)$ — excluded by **Assumption H8** (hypothesis (3)); and (ii) a colluding slot-$(N{-}1)$/slot-$N$ Byzantine pair exploiting a missing slot $N{-}1$ to seat a sibling $B'$ at slot $N{-}1$ parented at the slot-$N{-}2$ head, which outweighs $B$ at slot $N{+}1$ once proposer boost resets — excluded by **Assumption H9** (hypothesis (4); see H9's statement for the full attack trace). Under H8, no honest fork-choice ever leaves $\mathit{parent}(B)$'s branch, so the only competitors to $B$ that survive are siblings sharing $\mathit{parent}(B)$ — and these are excluded by H7 condition (i) (no proposer equivocation at slot $N$) and by the weight arithmetic above. Under H9, the late-prior-slot path is closed because no sibling of $\mathit{parent}(B)$ at slot $N{-}1$ can be retroactively introduced without slot-$(N{-}1)$ proposer equivocation (a separately-deterred class, addressed by `process_proposer_slashing` — Helper 22).
...
*Remark on the structural mechanism.* The lemma is a consequence of the two-phase ePBS design: the consensus-layer state transition (`process_block`) operates on the bid hash (`bid(B).block_hash`) and does not invoke `verify_execution_payload_envelope` for $P$. Algorithm 8 (`on_execution_payload_envelope`) is the only path to executing $P$ in the consensus model, and it is gated on the envelope's arrival (which may be at any time before the slot-$N+1$ block's processing). The structural availability of FULL and EMPTY children in the fork-choice tree at every slot (Algorithm 2 + Lemma 1) ensures that fork-choice *also* advances past slot $N$ regardless of whether $P$ is executed — this is the older "consensus progress" framing of P2; the current overview P2 sharpens it to "no execution deadline before slot $N+1$".
...
**Case 1: A subsequent block declares FULL (Path A fires).** Suppose the builder reveals via the cautious strategy of Assumption H7 and an honest slot-$N{+}1$ proposer is in place. Under **S1** (every honest slot-$N$ validator receives $B$ by $T_{\mathrm{att}}$ from the lemma's "timely" hypothesis), **H7** condition (i) (no proposer equivocation at slot $N$, so $B$ is the unique slot-$N$ child of $\mathit{parent}(B)$), **H8** ($\mathit{parent}(B)$ stays canonical, so no sibling-of-$\mathit{parent}(B)$ pulls honest attesters off $B$'s branch), and **H9** (no missing slot at $N{-}1$, so no late-released sibling $B'$ competes with $B$ under a shared ancestor), every honest slot-$N$ attester runs fork-choice with $B$ as the unique extension of $\mathit{parent}(B)$ and votes for $B$. By **S2**, the honest weight on $B$ at slot $N{+}1$ is at least $(1-\beta) W_N > 0.8 W_N$. The slot-$N{+}1$ proposer's `get_head` walks through $\mathit{parent}(B)$ and selects $B$. Observing $B$-FULL via `should_extend_payload(store, r)` (Algorithm 6, PTC primary path or fallback (c), depending on PTC state) and `should_build_on_full` (Algorithm 6b, satisfied under honest PTC), the proposer declares `parentStatus = FULL` for $B$. By Lemma 9 (first claim, conditional on FULL declaration), `process_parent_execution_payload` invokes `settle_builder_payment`, which queues the payment and clears the pending entry. The proposer gets paid in the slot-$N{+}1$ block.

**Case 2: No subsequent block declares FULL (Path A does not fire).** This includes three sub-cases: (a) the builder withholds entirely, (b) the builder reveals an invalid execution payload (rejected by `verify_execution_payload_envelope`, so `store.payloads` is never populated), or (c) the builder reveals a valid execution payload, but no subsequent block builds on B-FULL — this last sub-case is the PTC + proposer collusion scenario analyzed in detail by Lemma 17 below. In all three sub-cases, `settle_builder_payment` is never called, so the `BuilderPendingPayment` persists. Same-slot attesters accumulate `payment.weight` via `process_attestation` (Helper 15). By the same argument established in Case 1 — under S1, S2, **no slot-$N$ proposer equivocation** (the explicit lemma hypothesis, supplying the same fact that H7 (i) supplies in the trustless case where the builder is honest; note H7 itself is *not* a hypothesis of this lemma, since Case 2 sub-cases (a)–(c) need not feature an honest builder), H8, H9, and "timely" publication — every honest slot-$N$ attester votes for $B$, contributing their effective balance to $B$'s `payment.weight` *once their attestation is included on-chain*. By the **attestation-inclusion hypothesis**, all such honest attestations are included on the canonical chain before the start of epoch $\mathsf{epoch}(N)+2$, so by Helper 15 their effective balances accumulate into `payment.weight`. The accumulated weight is at least $(1-\beta) W_N > 0.8 W_N > 0.6 W_N$, meeting the 60% quorum threshold. The first canonical block whose processing crosses the epoch-$\mathsf{epoch}(N)+1$ boundary (such a block exists eventually under standard Gasper liveness — itself implied by S1 + S2 + an honest proposer schedule, the same setting the inclusion hypothesis implicitly assumes) invokes `process_slots`, which runs `process_epoch`, which calls `process_builder_pending_payments`. By Lemma 10, the quorum check passes and the `BuilderPendingWithdrawal` is appended at that point. The proposer is eventually paid.
...
**Lemma 14c** (Honest PTC majority excludes False-quorums). *Let B be a block with root r at slot N and assume the standing β < 0.5 per-PTC bound holds at slot N (i.e., the honest PTC fraction strictly exceeds `PAYLOAD_TIMELY_THRESHOLD / PTC_SIZE = 256 / 512`). Then, if every honest PTC member assigned to slot N (i) locally observes a valid execution payload for B strictly before `t_payload_due` and (ii) locally observes the blob columns it sampled to be available (passing `is_data_available`), then:*

*(i) `payload_timeliness(store, r, timely=False)` returns False — no False-timeliness quorum exists;*

*(ii) `payload_data_availability(store, r, available=False)` returns False — no False-DA quorum exists.*

*Equivalently, both `payload_timeliness(store, r, timely=True)` and `payload_data_availability(store, r, available=True)` return True at every honest node by slot N+1.*

*Proof.* By Lemma H4, an honest PTC member that locally observes the payload before `t_payload_due` votes `payload_present = True`; one that locally observes its sampled blob columns votes `blob_data_available = True`. Under the hypothesis, every honest PTC member votes True on both signals. The honest fraction strictly exceeds `PAYLOAD_TIMELY_THRESHOLD / PTC_SIZE`, so at most `PTC_SIZE - (PAYLOAD_TIMELY_THRESHOLD + 1) = PTC_SIZE - 257 = 255` PTC members are Byzantine and may vote False on either signal. Helper 4 checks `sum(payload_timeliness_vote[r] is False) > PAYLOAD_TIMELY_THRESHOLD = 256`: at most 255 such votes exist, so the count is at most 255, which is not strictly greater than 256. Hence `payload_timeliness(store, r, timely=False)` returns False. The same arithmetic applies to Helper 5: at most 255 False votes on `blob_data_available`, below `DATA_AVAILABILITY_TIMELY_THRESHOLD = 256`. Hence `payload_data_availability(store, r, available=False)` returns False. Conversely, at least `PTC_SIZE - 255 = 257` True votes exist on each signal, strictly exceeding the threshold of 256, so `payload_timeliness(store, r, timely=True)` and `payload_data_availability(store, r, available=True)` both return True. ∎

*Remark.* Lemma 14c is a structural fact used at two distinct sites: (i) Lemma 7 Part (a) (`should_extend_payload`'s PTC primary path fires) and Lemma 9 (`should_build_on_full(store, head=(r, FULL))` returns True), both via the "no False-DA quorum" half; (ii) Lemma 14b's contrapositive — namely, that the False-DA-quorum attack requires S3 to be violated. Promoting the counting argument here means later proofs can cite Lemma 14c instead of re-deriving it from scratch.
...
- **Case 1 — no-weight withhold (A1a (ii) failed).** The honest builder withholds because strictly less than 40% real attestation weight is observed at reveal time. Covered by Lemma 16 above: `payment.weight` cannot reach the 60% quorum because honest contribution is bounded by the case hypothesis ($< 40\%$) and Byzantine contribution is bounded by **S2** ($< 20\%$); `process_builder_pending_payments` (Helper 14) discards the entry at the epoch boundary.

- **Case 2a — equivocation-driven withhold; equivocation reorgs $B$ out (A1a (iii) failed).** The builder withholds because a proposer equivocation is visible. Honest fork-choice does not retain $B$ on the canonical chain, so no honest same-slot attester votes for $B$. The argument of Case 1 applies verbatim: honest contribution to `payment.weight` is zero, Byzantine contribution is `< 20%` by **S2**, and Helper 14 discards the entry.

- **Case 2b — equivocation-driven withhold; $B$ retains canonicity (A1a (iii) failed).** The original block may still have $\geq 40\%$ honest weight, so Case 1's quorum argument does not apply directly. The slashing-inclusion path discharges the IOU instead, via Lemma 22 part (iv): by **S1**, the equivocation evidence (observed by the honest builder at reveal time) is gossipped network-wide within $\Delta$; the IOU resides in `builder_pending_payments` until the $\mathsf{epoch}(N)+1 \to \mathsf{epoch}(N)+2$ boundary, a window of at least 32 slots (Helper 22 only clears IOUs whose slot is in the current or previous epoch); under **S2** (β < 20% per balance-weighted committee, which governs the proposer schedule), at least one of those 32 slots has an honest proposer who includes the `ProposerSlashing`. (The probability that all 32 consecutive proposers are Byzantine under **S2** is at most $\beta^{32} \approx 10^{-22}$ — of the same order as the per-slot **S3** bound and treated the same way: the proof reads the existence of an honest proposer within the window off **S2** as a deterministic claim at the named window, on the same overwhelming-probability footing that **S3** is justified on; see §1.4 "Adversarial model" / overview §9.1.) `process_proposer_slashing` (Helper 22) then clears the `BuilderPendingPayment` entry entirely, overriding whatever weight had accumulated.

Together, Lemma 16 (Cases 1 and 2a, which reduce to the same quorum-failure argument) and Lemma 22 part (iv) (Case 2b) prove P5's trustless case across all A1a-driven withhold causes. The non-trustless case is degenerate as noted in Lemma 16's main statement.
...
*Why hypothesis (3) is formulated as "declares EMPTY" rather than "reorgs B."* Under H7 (cautious-reveal builder), the strengthening hypotheses H8 ($\mathit{parent}(B)$ stays canonical) and H9 ($\mathit{parent}(B).\mathit{slot} = N - 1$), and the network-model synchrony assumption, a slot-$N+1$ proposer cannot substitute a boost-only reorg of $B$ for the EMPTY declaration. By Lemma 7 Part (b) (which now relies on H8 and H9), every honest slot-$N$ validator has voted for $B$ by $T_{\mathrm{att}}$, and the resulting bound $\mathit{weight}((B.\mathit{root}, \mathrm{PENDING})) \geq (1 - \beta)\, W > 0.8\, W$ at slot $N+1$ strictly exceeds the boost-induced upper bound $\mathit{weight}((B'.\mathit{root}, \mathrm{PENDING})) \leq 0.4\, W + \beta\, W \leq 0.6\, W$. Any sibling-reorg block $B'$ at slot $N+1$ therefore cannot outweigh $B$ at the PENDING level; a split-vote reorg higher up the chain is excluded by H8; and a late-prior-slot sibling reorg via a missing slot $N - 1$ is excluded by H9. The colluder is forced onto the alternative path of declaring $\mathit{parentStatus} = \mathrm{EMPTY}$ on $B$, which in turn requires the PTC primary path of `should_extend_payload` to fail — exactly hypothesis (2). The combined resource requirement (a bribed PTC majority *plus* a colluding next-slot proposer) is what makes the attack expensive to mount under balance-weighted PTC sampling (Lemma 26).*
...
*Proof.* The honest builder constructs its bid by invoking the `submit_bid` handler (Section 1.2). Line 3 reads `parent_block_hash ← state.latest_block_hash`. The subtlety: at the time the slot-$N+1$ builder constructs its bid, no block has yet executed `apply_parent_execution_payload` for $B$'s execution payload — that helper runs *inside* `process_block` of the slot-$N+1$ block, which doesn't exist yet (the builder is constructing the bid for it). So `state.latest_block_hash` as held in `store.block_states[B.root]` has *not* been updated to `bid(B).block_hash` yet.
...
*P3 direction, direct proof (⇒: $B$ FULL on chain $\Rightarrow$ DA was confirmed).* (Re-deriving the same direction directly, complementing the contrapositive route above.) Suppose $B$ is FULL on chain: there exists a canonical block $C$ at some slot $M > N$ with $\mathit{bid}(C).\mathit{parent\_block\_hash} = \mathit{bid}(B).\mathit{block\_hash}$.

*Step 1 — B's direct canonical consensus child declared B FULL.* Let $B^*$ be the direct canonical consensus child of $B$ (i.e., `B*.parent_root = B.root` and $B^*$ is on the canonical chain — necessarily exists, since $C$ extends from $B$ in the consensus chain and there is a unique canonical descendant at every slot after $B$). We claim `bid(B*).parent_block_hash = bid(B).block_hash`, equivalently `parentStatus(B*) = FULL`. Suppose for contradiction that $B^*$ declared EMPTY for $B$. Then in $B^*$'s `process_block`, Helper 12c's empty-parent branch runs and `state.latest_block_hash` is *not* updated. Subsequent canonical descendants of $B^*$ each read `state.latest_execution_payload_bid` cached by their parent (Helper 8 line 27 stores the parent's own bid), and Helper 8 line 14 asserts each descendant's `bid.parent_block_hash == state.latest_block_hash`. Tracing forward from $B^*$: `state.latest_block_hash` never equals `bid(B).block_hash` on the canonical chain past $B^*$ (because no apply call ever sets it to that value — only $B^*$ would have been able to, and we assumed it did not). But then $C$'s admission via Helper 8 line 14 must fail: `bid(C).parent_block_hash = bid(B).block_hash` ≠ `state.latest_block_hash` at $C$'s processing time. Contradiction with $C$ canonical. Therefore `parentStatus(B*) = FULL`.
...
*(iv) Payment clearing via slashing:* If the `ProposerSlashing` evidence is included in a block within the 2-epoch `builder_pending_payments` window, `process_proposer_slashing` (Helper 22, lines 14--22) clears the `BuilderPendingPayment` for the equivocating slot. This erases the IOU entirely — even if the quorum was met, the builder is not charged. Under $\beta < 20\%$, honest proposers control $> 80\%$ of slots, so the slashing evidence is included within a few slots. ∎
