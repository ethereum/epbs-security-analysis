...
<details>
<summary><b>Concept → formal-locus map</b> (click to expand)</summary>
...
| Named assumptions:**S1** (synchrony $`\Delta < T_{\mathrm{att}}`$), **S2** ($`\beta < 20\%`$), **S3** (honest PTC majority); **A1a / A1b** (cautious reveal — trustless / non-trustless); **L_verify, L_successor, L_withdraw**; **I_payweight, I_slash**; **H7–H9** | §5 Phase 3 (A1a/A1b), §9.1 | §1.4 "Discharging the overview's assumptions"; Assumptions H7 / H7b / H8 / H9        |
...
A note on the PTC entry: the spec specifies `PAYLOAD_ATTESTATION_DUE_BPS` as a *deadline*, not as an exact firing time — an honest PTC member must broadcast its vote *within the first* 75% of the slot. The Section 1.2 PTC handler models the worst-case (deadline) firing for proof-obligation simplicity, but a rational/cautious member typically waits close to T_ptc to maximise information. See the timing-semantics commentary in Section 1.2 for details.
...
**Definition 3b** (ExecutionPayloadBid container — payment fields). The bid `bid(B)` carried inside block $`B`$ is an `ExecutionPayloadBid` SSZ container. Two of its fields determine the **payment-trustlessness regime** of $`B`$:

- `bid(B).value` (gwei): the **trustless** payment amount.
- `bid(B).execution_payment` (gwei): the **non-trustless** payment amount.

We say block $`B`$ uses **trustless payment** if `bid(B).value > 0`, and **non-trustless payment** if `bid(B).value = 0` and `bid(B).execution_payment > 0`. The two are mutually exclusive in honest bids: a builder commits to one channel per bid. The case `bid(B).value = 0` and `bid(B).execution_payment = 0` is the **self-build** branch (`bid(B).builder_index = BUILDER_INDEX_SELF_BUILD`, signature `bls.G2_POINT_AT_INFINITY`), where the proposer builds for itself and no payment is owed.

*Protocol note.* The protocol enforces `value`: when `value > 0`, `process_execution_payload_bid` (Helper 8) records a `BuilderPendingPayment` IOU in `state.builder_pending_payments`, and the unconditional-payment mechanism later settles it. The protocol does not enforce `execution_payment`: Helper 8 never reads it, no IOU is created from it, and no Path A or Path B applies. The field is metadata for an off-protocol payment commitment, and delivery happens through some channel outside the consensus layer (typically a transaction inside the revealed execution payload paying `bid.fee_recipient`).

*Gossip-layer rejection of non-zero `execution_payment`.* The public `execution_payload_bid` gossip topic carries a `REJECT` validation rule (`p2p-interface.md`): a `SignedExecutionPayloadBid` is dropped at the gossip layer if `bid.execution_payment != 0`. Non-trustless bids cannot reach the proposer via the canonical bid topic and must travel through some off-protocol transport (a relay-style HTTP service, a direct builder ↔ proposer channel, a side-protocol). The consensus-layer admission in Helper 8 does not check `execution_payment`, so once such a bid is in a beacon block by *some* path, the block processes normally. The spec does not specify what that path looks like.

*Spec reference: `ExecutionPayloadBid` in `beacon-chain.md`; gossip `REJECT` rule in `p2p-interface.md`.*

**Definition 3c** (Timely block). A beacon block $`B`$ at slot $`N`$ is *timely* iff it is broadcast at a time $`t`$ satisfying $`t + \Delta \leq T_{\mathrm{att}}`$, where $`\Delta`$ is the synchrony delay (Assumption S1).

Under S1, every honest slot-$`N`$ validator receives a timely block before $`T_{\mathrm{att}}`$, hence before casting its slot-$`N`$ attestation. A non-timely block (broadcast at $`t > T_{\mathrm{att}} - \Delta`$) is delivered only partially to honest validators by $`T_{\mathrm{att}}`$: some receive it in time and vote for it, others do not and vote for the previous head. This split visibility enables the late-block reorg vector against blocks that fall outside this definition.

*Use in properties:* P_can, P_rev (trustless), and P_pay assume $`B`$ is timely as defined here.

*Spec reference: pedagogical. Formalises the timely-publication hypothesis used by the canonicity and trustless-payment arguments.*

**Definition 4** (Parent payload status). For any non-genesis block $`B`$, the *parent payload status* of $`B`$ is:

- $`\mathrm{parentStatus}(B) = \mathrm{FULL}`$ if `bid(B).parent_block_hash == bid(parent(B)).block_hash`;
- $`\mathrm{parentStatus}(B) = \mathrm{EMPTY}`$ otherwise.

This comparison determines whether $`B`$ was built on the full or empty version of its parent. A valid post-Gloas block can point its execution parent at one of the two hashes exposed by the parent bid: `bid(parent(B)).block_hash` for FULL, or `bid(parent(B)).parent_block_hash` for EMPTY. Honest construction obtains the same choice from `should_build_on_full(store, head)`: when it returns True, construction uses the parent's FULL execution state; when it returns False, construction uses the parent's EMPTY variant. Local payload delivery is therefore necessary for building FULL, but for an immediately previous-slot head it is not sufficient if the current PTC scorecard has a negative quorum.

*Spec reference: `get_parent_payload_status` in `fork-choice.md`.*

**Definition 4b** (Block status — chain-relative). Write $`B \prec \mathit{chain}`$ to mean "$`B`$ is a non-head canonical block on $`\mathit{chain}`$" (i.e., $`B`$ is on $`\mathit{chain}`$ and $`B \neq \mathit{head}(\mathit{chain})`$), and let $`\mathit{child}(\mathit{chain}, B)`$ denote $`B`$'s unique canonical child on $`\mathit{chain}`$ (the block whose `parent_root` equals $`B`$'s hash-tree root). Then `block_status(chain, B)` is:

- **FULL** if $`B \prec \mathit{chain}`$ and `bid(child(chain, B)).parent_block_hash == bid(B).block_hash`;
- **EMPTY** if $`B \prec \mathit{chain}`$ and `bid(child(chain, B)).parent_block_hash != bid(B).block_hash`;
- **undefined** otherwise.
...
This is the chain-relative companion to Definition 4: for $`B \prec \mathit{chain}`$, `block_status(chain, B)` equals `parentStatus(child(chain, B))`. Definition 4 reads the FULL/EMPTY declaration from the child's perspective (intrinsic to $`B`$'s child); Definition 4b reads the same fact relative to a named chain.

**Equivalent form of EMPTY and the most-recent-FULL-ancestor invariant.** Let $`C = \mathit{child}(\mathit{chain}, B)`$. Then $`B`$ is EMPTY on chain iff `bid(C).parent_block_hash == bid(B).parent_block_hash` — equivalently, $`C`$ stamps the *same* execution-layer tip its parent stamped, because `state.latest_block_hash` did not advance across $`B`$. More generally, for any non-head canonical block $`B`$ on $`\mathit{chain}`$, `bid(B).parent_block_hash` equals the `block_hash` of the **most recent FULL ancestor** of $`B`$ on $`\mathit{chain}`$ (or genesis if none). A run of consecutive EMPTY blocks all stamp the same upstream FULL ancestor; the stamp updates only when an ancestor is FULL. This invariant follows from the propagation of `state.latest_block_hash` through `process_parent_execution_payload` (Helper 12c): the EL tip is updated exactly when a block declares its parent FULL, and remains pinned otherwise.

*Spec reference:* No direct spec function. This formalises the chain-relative on-chain meaning of FULL/EMPTY.

**Definition 4c** (Payload hash chain). The *payload hash chain* is the collection of `bid(B).block_hash` values committed by every FULL (in the sense of Definition 4b) block on the canonical chain, inheriting the canonical chain's order. Equivalently, a payload hash $`h`$ belongs to the payload hash chain if and only if there exists a non-head canonical block $`B^*`$ such that `bid(B*).block_hash` $`= h`$ AND `block_status(canonical, B*) = FULL`.
...
*Spec reference:* No direct spec function. This formalises the hash-level execution chain induced by canonical FULL blocks.
...
**Definition 5** (Fork-choice node). A *fork-choice node* is a pair `(r, s)` where `r` is a block root and `s` is a payload status. The node — not the block — is the primary fork-choice abstraction: `get_head` traverses nodes, `get_weight` scores nodes, and the spec's `get_supported_node` / `is_ancestor` composition decides which votes support which nodes. Up to three nodes can reference the same block B: `(r, PENDING)`, `(r, FULL)`, and `(r, EMPTY)`.
...
A vote is a *same-slot vote* for block B if `slot = slot(B)`. Same-slot voters set `payload_present = False`: by Definition 1, the attestation phase precedes the builder reveal window, so the attester has no payload observation to report at same-slot attestation time.

A vote is a *non-same-slot vote* for block B if `slot > slot(B)`. This occurs when no block was proposed in the voter's assigned slot and their head is an earlier block. Non-same-slot voters set `payload_present` according to the payload status they observe.

*Protocol note:* Honest attester behavior below enforces same-slot payload-neutrality and non-same-slot payload-status reporting. Votes with `slot < slot(B)` are invalid because `validate_on_attestation` asserts `block_slot <= attestation.data.slot`.
...
**Definition 9** (Ancestor). For a fork-choice node `(r, s0)` and a slot t with `t ≤ slot(B_r)`, the *ancestor of `(r, s0)` at slot t*, written `anc((r, s0), t)`, is the fork-choice node identifying which block at slot t is in B_r's chain and what its payload status is:

- If `slot(B_r) ≤ t`: return `(r, s0)`. The block B_r is at or before the queried slot; it is its own ancestor, so the input node's payload status is preserved.
...
- Let `v` be the fork-choice node supported by the vote itself: `(root, PENDING)` for a same-slot vote, and `(root, FULL)` or `(root, EMPTY)` according to `payload_present` for a later vote. Let `(r', s') = anc(v, slot(B_r))`.
- The vote supports `(r, s)` iff `r' = r` and `(s = PENDING` or `s = s')`.

*Spec reference:* Pedagogical predicate equivalent to the spec's `is_ancestor(store, get_supported_node(store, message), node)` test in `get_attestation_score`; see `get_supported_node`, `is_ancestor`, and `get_attestation_score` in `fork-choice.md`.

**Definition 11** (PTC quorums on payload presence and blob-data availability). For a block with root `r`, the PTC scorecards `payload_timeliness_vote[r]` and `payload_data_availability_vote[r]` give rise to four predicates. Each predicate first checks local payload verification. If `r ∉ store.payloads`, the positive query returns False and the negative query returns True:

1. **True-timeliness predicate** — `payload_timeliness(store, r, timely=True)` returns True iff `r ∈ store.payloads` AND more than `PAYLOAD_TIMELY_THRESHOLD` entries in `payload_timeliness_vote[r]` are True.
2. **False-timeliness predicate** — `payload_timeliness(store, r, timely=False)` returns True iff either `r ∉ store.payloads`, or `r ∈ store.payloads` AND more than `PAYLOAD_TIMELY_THRESHOLD` entries are False.
3. **True-DA predicate** — `payload_data_availability(store, r, available=True)` returns True iff `r ∈ store.payloads` AND more than `DATA_AVAILABILITY_TIMELY_THRESHOLD` entries in `payload_data_availability_vote[r]` are True.
4. **False-DA predicate** — `payload_data_availability(store, r, available=False)` returns True iff either `r ∉ store.payloads`, or `r ∈ store.payloads` AND more than `DATA_AVAILABILITY_TIMELY_THRESHOLD` entries are False.

The True predicates combine the local payload-verification gate with the network-wide delivery witness consumed by `should_extend_payload`'s primary path (Algorithm 6). The negative forms are consumed by `should_build_on_full` (Algorithm 6b): for an immediately previous-slot FULL head, a negative quorum on either payload timeliness or blob-data availability makes an honest proposer build on EMPTY.
...
- Otherwise: return `attest_weight(r, s) + boost_weight(r, s)`, where:
  ...
  - `boost_weight(r, s)` is the proposer boost score iff `should_apply_proposer_boost(store)` is true and `(r, s)` is an ancestor of the PENDING node for `store.proposer_boost_root`; otherwise it is 0.

*Spec reference: `get_weight`, `get_attestation_score`, `should_apply_proposer_boost` in `fork-choice.md`.*
...
*Regime A — block is not the immediately previous slot's block (`s = PENDING` or `slot(B_r) + 1 ≠ current_slot`).* The tiebreaker is the natural payload-status integer (Definition 2): EMPTY=0, FULL=1, PENDING=2. The PTC special case is not active here; this tiebreaker only matters under exact weight ties.
...
**A note on `# (Ax)` markers.** Inline comments of the form `# (A1)`, `# (A2)`, … `# (A6)` tag specific lines of the handlers below where an *honest-actor behaviour* fires. These are **formal-treatment-internal labels** for cross-referencing the lemmas in §1.4: A1 → Lemma H3 (bid-reveal consistency); A2 → Lemma H1 (same-slot attesters signal `data.index = 0`); A3 → Lemma H2 (non-same-slot attesters signal FULL/EMPTY consistently); A4 → Lemma H4 (PTC members report observation, not validity); A5 → Lemma H5 (honest builder withholds when block is not local head); A6 → Assumption H7 (cautious-reveal strengthening). The companion overview document carries **only one** of these as a named assumption — the cautious-reveal strengthening, which the overview calls **A1** (≡ our A6 ≡ Assumption H7 here); the other five honest-behaviour facts are read directly off the §5 Phase handlers in the overview, not named as assumptions there. The markers here are forward references to §1.4.

<details>
<summary><b>Pedagogical abstractions table (click to expand)</b></summary>

The handlers below use a small set of helper names that do not appear in the Gloas specs as named functions. Each one abstracts spec prose or a generic operation, for readability. The mapping is:
...
| `has_execution_payload_envelope(store, root)`                                                                 | Inline prose check in `validator.md` §"Constructing the `PayloadAttestationMessage`" ("If a previously seen `SignedExecutionPayloadEnvelope` references the block … and it was seen before `get_payload_due_ms()`"). This is an envelope-arrival observation for the PTC vote, not the stronger `is_payload_verified(store, root)` / `root in store.payloads` gate used by fork choice.                                                                                                                                                                                                                                                                                                    |
...
| `(r, status)` tuple form                                                                                      | Pedagogical shorthand for `ForkChoiceNode(root=r, payload_status=status)` in `fork-choice.md`. The algorithms below alternate between the tuple form and the spec container form for compactness.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `FULL`, `EMPTY`, `PENDING`                                                                                | Shorthand for the spec constants `PAYLOAD_STATUS_FULL`, `PAYLOAD_STATUS_EMPTY`, `PAYLOAD_STATUS_PENDING` (`fork-choice.md` lines 78–80).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `bid(B)`, `bid(B).block_hash`, `bid(B).parent_block_hash`, `bid(B).value`, `bid(B).execution_payment` | Math notation for the `ExecutionPayloadBid` carried inside `B.body.signed_execution_payload_bid.message`. Field accesses use the usual dot syntax (Definitions 3 and 3b).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `parentStatus(B)`, `block_status(chain, B)`                                                                 | The FULL/EMPTY classifiers introduced in Definitions 4 and 4b.`parentStatus(B)` reads $`B`$'s declaration about its parent (FULL iff `bid(B).parent_block_hash == bid(parent(B)).block_hash`). `block_status(chain, B)` reads the same fact from a fixed chain's perspective (FULL iff `bid(child(chain, B)).parent_block_hash == bid(B).block_hash`); undefined at the head.                                                                                                                                                                                                                                                                                                                  |
| "$`B'`$ will remain canonical forever"                                                                        | Strengthening canonicity hypothesis embedded in P_can, P_rev (trustless case), and P_pay's statements (overview §4), where$`B'`$ is $`B`$'s parent at slot $`N{-}1`$. Formalised as Assumption H8 + H9 in §1.4. Rules out reorg configurations against $`B`$ that would otherwise contaminate the trustless proofs. P_rev's non-trustless case substitutes the confirmation rule's $`\Sigma_R`$ for this hypothesis.                                                                                                                                                                                                                                                                         |
| `epoch(N)`                                                                                                    | The epoch containing slot$`N`$, equal to `compute_epoch_at_slot(N) = N // SLOTS_PER_EPOCH`. We write $`\mathsf{epoch}(N)`$ in math notation and `epoch(N)` in prose. A bound variable named $`e`$ in a lemma (e.g., Lemma 12c's "exit initiated at epoch $`e`$") is a *local* epoch variable, unrelated to $`\mathsf{epoch}(N)`$.                                                                                                                                                                                                                                                                                                                                                        |
...
**Fork-choice validation guard.** Votes entering `store.latest_messages` are subject to `validate_on_attestation`: `data.index` must be 0 or 1; if the attestation is same-slot, `data.index` must be 0; and if `data.index == 1`, the referenced payload must be locally verified. The formal vote model below assumes this validation has already run.

*Spec reference: attestation `data.index` construction in `validator.md`; `validate_on_attestation` in `fork-choice.md`.*
...
def propose(validator, slot, state, store):
    ...
    build_on_full = should_build_on_full(store, head)
    parent_bid = state.latest_execution_payload_bid
    expected_parent_hash = (
        parent_bid.block_hash if build_on_full else parent_bid.parent_block_hash
    )
    ...
    if bids and validator.wants_external_builder:
        ...
        assert selected_bid.message.parent_block_hash == expected_parent_hash
        ...
        bid = ExecutionPayloadBid(
            ...
            parent_block_hash=expected_parent_hash,
...
The helper `should_build_on_full(store, head)` fixes the proposer-visible execution parent for all three construction surfaces. If it returns True, an accepted bid must use `state.latest_execution_payload_bid.block_hash` as `bid.parent_block_hash`, `parent_execution_requests` is copied from `store.payloads[head.root].execution_requests`, and self-build payload construction uses the parent's FULL execution state. If it returns False, an accepted bid must use `state.latest_execution_payload_bid.parent_block_hash`, `parent_execution_requests` is empty, and self-build payload construction uses the parent's EMPTY variant.
...
The four lookup helpers below — `has_beacon_block_for_slot`, `get_beacon_block_root_for_slot`, `has_execution_payload_envelope`, `check_blob_data` — are pedagogical names for the corresponding inline checks in the spec prose at [`validator.md`](https://github.com/ethereum/consensus-specs/blob/e17a1389e6cee2f44e6b873e324ad4cb0172e341/specs/gloas/validator.md) ("Constructing a payload attestation message"). The spec uses `is_data_available(beacon_block_root)` for the blob-data check; the other three are described in prose rather than as named helpers. Importantly, `has_execution_payload_envelope(store, block_root)` is shorthand for "the node previously saw a `SignedExecutionPayloadEnvelope` for `block_root` *strictly before* `get_payload_due_ms()` milliseconds into the slot" — the payload-arrival deadline (Definition 1) is distinct from the PTC-vote deadline, and this PTC observation is weaker than `store.payloads[block_root]`.
...
**Spec timing semantics.** The `@Upon(time_in_slot == T_ptc ...)` clause above models the worst-case (most conservative) firing time. The spec ([`validator.md`](https://github.com/ethereum/consensus-specs/blob/e17a1389e6cee2f44e6b873e324ad4cb0172e341/specs/gloas/validator.md)) actually specifies a *deadline*, not an exact firing time: a PTC member must broadcast the `PayloadAttestationMessage` *within the first* `PAYLOAD_ATTESTATION_DUE_BPS` (= 75%, i.e., 9 seconds) of the slot. An honest member is therefore allowed to vote as soon as it has decided its vote (e.g., as soon as the payload is observed), but a *cautious* member waits close to the deadline to maximise information — voting earlier risks emitting `payload_present = False` and then having the payload arrive later. The proofs in Section 1.4 depend only on (a) the value the member commits to and (b) the deadline being met; they do not depend on when within `(0, T_ptc]` the broadcast actually happens. We therefore keep the simpler firing-at-deadline form in the handler above and rely on the fact that all proof obligations follow from the worst case.
...
def submit_bid(builder, upcoming_slot, state, store, execution_engine, proposer_preferences):
    # Resolve the same FULL/EMPTY parent view the proposer will enforce.
    head = get_head(store)
    parent_block_root = head.root
    parent_bid = state.latest_execution_payload_bid
    if should_build_on_full(store, head):
        state = copy(state)
        apply_parent_execution_payload(state, store.payloads[head.root].execution_requests)
        parent_block_hash = parent_bid.block_hash
    else:
        parent_block_hash = parent_bid.parent_block_hash
    ...
    payload, execution_requests, blob_kzg_commitments = execution_engine.build_payload(
        ...
        target_gas_limit=proposer_preferences.target_gas_limit,
    )
    ...
    bid = ExecutionPayloadBid(
        ...
        gas_limit=payload.gas_limit,                               # Must be target-compatible with preferences
...
def reveal_payload(builder, block, store, execution_engine):
    ...
    #      (i) T_att + Delta <= t_rev < T_payload_due - Delta
    #          (lower bound: closes the equivocation window;
    #           upper bound: reveal-timing safety so envelope reaches honest PTC
    #           members before get_payload_due_ms() under S1),
...
**`submit_bid`** fires when the builder observes a `SignedProposerPreferences` for an upcoming slot. The builder constructs the actual execution payload **before** submitting the bid — `bid.block_hash` is the hash of an already-built payload, not a promise to build one later. The builder must have the payload ready at bid time because `block_hash` is a binding commitment: when the builder later reveals the execution payload, `verify_execution_payload_envelope` (Helper 12, line 16) asserts that the revealed execution payload's hash matches the committed `block_hash`. The `fee_recipient` must match the proposer's preferences, and the bid's `gas_limit` must be compatible with the proposer's `target_gas_limit` via `is_gas_limit_target_compatible`; the gossip network enforces these builder/proposer preference rules. `bid.value` is the competitive dimension: the amount the builder offers to pay the proposer. The builder stores the payload locally for later reveal.

A subtlety in the first block: the builder cannot just read the unmodified `state.latest_block_hash` and hope it is the correct execution parent. At bid construction time, `apply_parent_execution_payload` for the current head has not yet run in a canonical successor; it will run inside `process_block` of the next block. The builder therefore follows the same `should_build_on_full(store, head)` branch enforced on the proposer side: when building on FULL, it locally simulates `apply_parent_execution_payload` on a copy of the state, which advances the execution head to `parent_bid.block_hash`; when building on EMPTY, it uses `parent_bid.parent_block_hash`. The network's eventual `process_block` performs the same deterministic state update when the successor is admitted.
...
- **Lines 5–6: Honest withholding (cautious-reveal strategy).** The spec ([`builder.md`](https://github.com/ethereum/consensus-specs/blob/e17a1389e6cee2f44e6b873e324ad4cb0172e341/specs/gloas/builder.md)) states the minimal rule: an honest builder MAY choose not to reveal if the block "was not timely and thus it is not the head of the builder's chain." This is the loose rule captured by Lemma H5. **The proofs in this document assume a strictly stronger strategy** — the **cautious-reveal** rule formalised as Assumption H7: an honest builder broadcasts the payload only after (i) $`T_{\mathrm{att}} + \Delta \leq t_{\mathrm{rev}} < T_{\mathrm{payload\_due}} - \Delta`$ — a lower bound (closes the equivocation window: any equivocation broadcast at $`t_{\mathrm{eq}} \leq T_{\mathrm{att}}`$ reaches the builder by $`t_{\mathrm{eq}} + \Delta \leq t_{\mathrm{rev}}`$ under S1) and an upper bound (reveal-timing safety: envelope reaches every honest PTC member before `get_payload_due_ms()` under S1) — (ii) ≥ `PROPOSER_SCORE_BOOST = 40%` of real attestation weight for the block has been observed, and (iii) no proposer equivocation by `block.proposer_index` for `block.slot` is visible. The 40% threshold is calibrated to the proposer boost so that, under β < 20%, the slot-N+1 proposer cannot reorg slot N via boost alone (Lemma 7 Part (b)). Together (i)'s lower bound and (iii) close the equivocation window: any equivocation that could split honest attesters at $`T_{\mathrm{att}}`$ has reached the builder by $`t_{\mathrm{rev}}`$, so (iii) catches it; any equivocation broadcast after $`T_{\mathrm{att}}`$ arrives too late to influence honest votes. If the builder withholds because it detected an equivocation, `process_proposer_slashing` clears the `BuilderPendingPayment` when the equivocation evidence is included on-chain, protecting the builder from epoch-boundary payment charges. Without H7 (under H5 alone), the proofs only deliver the weaker conclusion that the FULL outcome *can* prevail; with H7, they deliver the stronger conclusion that it *does* prevail at slot N+1 under an honest next-slot proposer (Lemma 9, second claim).
...
```python
def get_ancestor(store, node, target_slot):
    r, s = node
    block = store.blocks[r]
    if block.slot <= target_slot:
        return node                                                # Preserve the node's payload status
    parent = (block.parent_root, get_parent_payload_status(store, block))
    return get_ancestor(store, parent, target_slot)                # Read the child's declaration about its parent
```
...
If the node's block is at or before slot t (line 4), it is its own ancestor and the function returns the input node unchanged (line 5). For ordinary block-root queries this is usually a PENDING node; preserving the input status matters for `is_ancestor`, where the queried node may already be FULL or EMPTY.

Otherwise (lines 6–7), the function steps to the parent node, using the current block's `parentStatus` declaration as the parent's payload status, and recurses. The first recursive step that crosses the target slot therefore returns the block at or before the target slot together with what the first descendant past the target declared about it. No BeaconState is consulted; only block-level bid hashes are read.

The function is used by `is_ancestor` and by the pedagogical `is_supporting_vote` wrapper (Algorithm 4, Case 2) to determine which FULL/EMPTY branch a descendant block passed through at an earlier slot.

*Spec reference: `get_ancestor` in `fork-choice.md`.*

#### Algorithm 4: get_supported_node / is_supporting_vote

```python
def get_supported_node(store, vote):
    block = store.blocks[vote.root]
    if block.slot < vote.slot:
        payload_status = FULL if vote.payload_present else EMPTY
    else:
        payload_status = PENDING
    return (vote.root, payload_status)
...
def is_supporting_vote(store, node, vote):
    ...
    if vote.root == r:                                             # Case 1: vote for this block
        ...
        anc_root, anc_status = get_ancestor(store, get_supported_node(store, vote), block.slot)
        return anc_root == r and (s == PENDING or s == anc_status)
```

`get_supported_node` is the current spec helper: it turns a latest attestation message into the fork-choice node directly supported by that message. If the attestation is from the same slot as the voted-for block, the supported node is PENDING; if it is from a later slot, `payload_present` selects FULL or EMPTY.

`is_supporting_vote` is a pedagogical wrapper around the spec's `is_ancestor(store, get_supported_node(store, vote), node)` test. It answers the question: "does vote m count toward the weight of fork-choice node (r, s)?" A vote is a tuple `(slot, root, payload_present)` — it says "at this slot, I believe this block root is the head, and here is my payload opinion." The wrapper expands the spec predicate into two cases depending on whether the voter voted directly for block r, or for a descendant of r.

**Case 1** (lines 12–18): The voter voted for this exact block (`m.root = r`).

- Lines 13–14: If we are asking about the PENDING node, the vote always counts. Every vote for a block supports the block's existence, regardless of payload opinion.
- Line 15: Assert that the vote's slot is not before the block's slot (a vote cannot target a block from the future relative to the vote's own slot — this would be an invalid state).
- Lines 16–17: If we are asking about FULL or EMPTY, but the voter is a same-slot voter (`m.slot == slot(B)`), the vote does **not** count. This is the key ePBS rule: same-slot attesters cast their votes before the builder reveals (Definition 1), so they have no payload opinion and cannot influence the FULL/EMPTY decision.
- Line 18: If the voter is a non-same-slot voter (`m.slot > slot(B)`), they observed the payload status and signaled it via `payload_present`. The vote counts for FULL if `payload_present = True`, for EMPTY if `payload_present = False`.

**Case 2** (lines 19–21): The voter voted for a descendant of block r (`m.root ≠ r`). The voter did not vote for r directly, but their vote for a later block implies a position on r. Line 20 calls `get_ancestor` (Algorithm 3) on the node returned by `get_supported_node` to walk back from the voted-for node to r's slot and read which FULL/EMPTY branch the descendant chain passed through. Line 21 then checks: is r the correct ancestor, and does the branch match? If so, the vote supports this node. Note that Case 2 always supports PENDING nodes (the `s = PENDING` disjunct), because any descendant passing through r implies support for r's existence.

*Spec reference: `get_supported_node`, `is_ancestor`, and `get_attestation_score` in `fork-choice.md`. The `is_supporting_vote` wrapper is pedagogical and expands the same predicate for proof readability.*

#### Algorithm 5: get_attestation_score / get_weight

```python
def is_ancestor(store, node, ancestor):
    node_root, node_status = node
    ancestor_root, ancestor_status = ancestor
    ancestor_at_slot_root, ancestor_at_slot_status = get_ancestor(
        store, node, store.blocks[ancestor_root].slot
    )
    if ancestor_at_slot_root != ancestor_root:
        return False
    return ancestor_at_slot_status == ancestor_status or ancestor_status == PENDING


def is_previous_slot_payload_decision(store, node):
    r, s = node
    is_previous_slot = store.blocks[r].slot + 1 == get_current_slot(store)
    is_payload_decision = s in [EMPTY, FULL]
    return is_previous_slot and is_payload_decision


def get_attestation_score(store, node, state):
    score = 0
...
def is_head_weak(store, head_root):
    ...
    head_weight = get_attestation_score(store, (head_root, PENDING), justified_state)
...
def should_apply_proposer_boost(store):
...
def get_weight(store, node):
    ...
    proposer_score = 0
    proposer_boost_node = (store.proposer_boost_root, PENDING)
    if is_ancestor(store, proposer_boost_node, node):
        proposer_score = get_proposer_score(store)
    return attestation_score + proposer_score
```

This group computes the weight behind a fork-choice node `(r, s)`. The attestation score counts active, unslashed, non-equivocating validators whose latest vote supports the node. The proposer boost is applied only when `should_apply_proposer_boost` allows it, and then only to ancestors of the PENDING node carrying `store.proposer_boost_root`.

**Zero-return rule.** If the node is a FULL or EMPTY node for the immediately previous slot, `get_weight` returns 0 immediately via `is_previous_slot_payload_decision`. This delegates the FULL/EMPTY decision entirely to the tiebreaker (Definition 13), which uses PTC votes instead of attestation weight.

**Attestation score.** The score scans all active validators in the justified checkpoint state, not just the committee of the block's slot. The spec turns each latest message into a supported node with `get_supported_node`, then counts the vote for every ancestor node via `is_ancestor`. The pedagogical `is_supporting_vote` wrapper above expands the same relation into direct-vote and descendant-vote cases.

**Proposer boost.** The spec applies boost only when the boosted block's parent is old enough, strong enough, or weak-but-not-equivocated; if the boost is applied, `get_weight` adds it to ancestors of `(proposer_boost_root, PENDING)`. The `is_head_weak` helper includes equivocating validators from the head slot committees when computing the weak-head threshold, matching the current monotonic spec rule.

*Spec reference: `get_attestation_score`, `is_head_weak`, `should_apply_proposer_boost`, `get_weight` in `fork-choice.md`.*

#### Algorithm 5b: get_payload_status_tiebreaker

```python
def get_payload_status_tiebreaker(store, node):
    r, s = node
    if is_previous_slot_payload_decision(store, node):
        if s == EMPTY:
            return 1
        if should_extend_payload(store, r):
            return 2
        return 0
    else:
        return s
...
This helper is the third key in `get_head`'s child selection. Outside the immediately previous slot's FULL/EMPTY decision, it returns the natural payload-status integer. For the immediately previous slot, EMPTY gets priority 1; FULL gets priority 2 only if `should_extend_payload` returns True, otherwise FULL gets priority 0. This is the exact place where the PTC primary path and the fallback conditions of Algorithm 6 influence fork choice.

*Spec reference: `get_payload_status_tiebreaker` in `fork-choice.md`.*
...
These fallbacks make `get_head` permissive when the local node has verified the payload but the PTC primary path did not produce True-quorums on both signals. For the slot-N+1 proposer's own construction path, that permissive fork-choice result is filtered by `should_build_on_full` below: an immediately previous-slot FULL head is built on as EMPTY if the PTC has a negative quorum on either timeliness or blob-data availability.
...
This function is consulted by an honest proposer at block-construction time, on the head returned by `get_head(store)`. It imposes a stricter construction constraint than `should_extend_payload`: for an immediately previous-slot FULL head, a negative PTC quorum on either payload timeliness or blob-data availability makes the proposer build on the EMPTY parent state — even if `get_head` placed the head at FULL via `should_extend_payload`'s fallback paths, and even if the proposer's own local copy of the payload passes verification.
...
**Line 4: Older resolved heads.** If the head is FULL and not from the immediately previous slot, fork choice has already resolved that FULL branch by ordinary weight. The proposer builds on FULL.

**Lines 5–8: Negative PTC quorum checks.** For an immediately previous-slot FULL head, the proposer builds on EMPTY if either negative aggregate signal exists: `payload_data_availability(..., available=False)` or `payload_timeliness(..., timely=False)`. If neither negative signal exists, the proposer builds on FULL.
...
**Why both `should_extend_payload` and `should_build_on_full`.** `should_extend_payload` is the universal fork-choice tiebreaker — consumed by every node's `get_head` to choose between sibling `(B, FULL)` / `(B, EMPTY)` nodes. `should_build_on_full` is a proposer-side construction check that fires *after* `get_head` has returned, and it can override `get_head`'s FULL determination when a negative PTC quorum is present. The asymmetry: `should_extend_payload` uses PTC's **True**-quorums on both signals, permissive with fallbacks; `should_build_on_full` uses PTC's negative timeliness and DA predicates, strict and without fallbacks for the immediately previous slot. Under honest PTC majority (S3 in the overview), no negative quorum arises against an honestly revealed payload, so `should_build_on_full` returns True and the proposer is free to extend FULL.
...
#### Algorithm 6c: get_dependent_root

```python
def get_dependent_root(store, root):
    epoch = get_current_store_epoch(store)
    if epoch <= MIN_SEED_LOOKAHEAD:
        return Root()                                             # Genesis block parent
    node = (root, PENDING)
    dependent_slot = Slot(compute_start_slot_at_epoch(epoch - MIN_SEED_LOOKAHEAD) - 1)
    dependent_root, _ = get_ancestor(store, node, dependent_slot)
    return dependent_root
```

This helper computes the root used to decide whether a newly received block belongs to the same proposer-lookahead dependency as the current canonical head. It wraps the block root as a PENDING fork-choice node, walks back to the dependent slot, and returns the ancestor root at that slot.

The helper is load-bearing for proposer boost: a timely first block in a slot receives boost only if it has the same dependent root as the pre-insertion head. This prevents proposer boost from being assigned to a block that conflicts with the canonical chain's proposer-lookahead dependency.

*Spec reference: `get_dependent_root` in `fork-choice.md`.*

#### Algorithm 6d: record_block_timeliness

```python
def record_block_timeliness(store, root):
...
This helper records two independent timeliness bits for the block: one against the attestation deadline and one against the payload-attestation deadline. `update_proposer_boost_root` reads the attestation-timeliness bit; `should_apply_proposer_boost` later reads the PTC-timeliness bit when checking early equivocations around a weak previous-slot parent.

*Spec reference: `record_block_timeliness` in `fork-choice.md`.*

#### Algorithm 6e: update_proposer_boost_root

```python
def update_proposer_boost_root(store, head, root):
...
This helper assigns proposer boost to the newly received block `root` only when three conditions all hold: no block has already received proposer boost in the current slot, the new block arrived before the attestation deadline, and the new block has the same dependent root as the pre-insertion head passed in by `on_block`.

The dependent-root check is why Algorithm 7 computes `head = get_head(store)` before inserting the new block into the store. The boost decision compares the new block against the canonical chain as it existed immediately before the block arrived.

*Spec reference: `update_proposer_boost_root` in `fork-choice.md`.*
...
def on_block(store, signed_block):
    ...
    head = get_head(store)                                          # Head before inserting this block
    ...
    update_proposer_boost_root(store, head.root, root)
...
**Line 12:** Computes the pre-insertion head. The proposer-boost update compares the new block's dependent root against this head's dependent root, so the head must be read before the new block is added to the store.

**Lines 13–16:** Stores the block and its post-state in the store. Lines 15–16 initialize empty PTC vote arrays (`payload_timeliness_vote`, `payload_data_availability_vote`) for this block — these arrays will be filled later as PTC members broadcast their votes (Algorithm 9) or as the next block includes aggregated PTC attestations (line 17).

**Lines 17–21:** Processes included PTC attestations from the previous slot (`notify_ptc_messages`), records block timeliness, applies proposer boost using the pre-insertion head through Algorithm 6e, updates checkpoints, and eagerly computes the pulled-up unrealized justification/finality for the new block.
...
This function is called when the builder's execution payload arrives over the network (constructed and broadcast as described in Section 1.2, Builder Phase 2), separately from the beacon block. It verifies the payload and stores it — but does not apply execution-layer state effects. Those effects are applied later, when the next block's `process_parent_execution_payload` runs inside `process_block`.
...
This function is called when a PTC member's vote arrives (constructed as described in Section 1.2, PTC Member Behavior), either from the gossip network (during the current slot) or from a block that included aggregated PTC attestations from the previous slot. PTC votes do not carry attestation weight — they do not appear in `get_weight` (Algorithm 5). Instead, they populate the PTC scorecard (`payload_timeliness_vote`, `payload_data_availability_vote`) that feeds into `payload_timeliness` (Definition 11), which feeds into `should_extend_payload` (Algorithm 6), which feeds into the tiebreaker (Definition 13). The chain of influence is: PTC vote → scorecard → timeliness check → tiebreaker → FULL vs. EMPTY decision for the immediately previous slot.
...
This section establishes the formal properties of the ePBS fork-choice rule, organized in three layers. We begin with properties of the honest behavior specifications (Section 1.2) — things that follow directly from how honest actors construct their messages. We then prove structural properties of the fork-choice algorithms (Section 1.3) — invariants that hold regardless of who is honest. Finally, we combine both layers to prove what the fork-choice rule produces under honest behavior.

#### Property table

The [overview document](https://github.com/ethereum/epbs-security-analysis/blob/master/README.md) (Section 4) lists **seven** externally observable properties — claims an observer with full visibility of network messages and on-chain state can verify without inspecting any node's internal state — organised in three categories: **Category A** (payment-trustlessness-independent: P_can, P_exec, P_DA, P_valid), **Category B** (payment-trustlessness-dependent, each with two cases: P_rev, P_withhold), **Category C** (trustless-payment-only: P_pay). The following table maps each overview property to the formal lemmas proved in this section.

| Cat. | Property                                                                                                     | Statement                                                                                                                                                                                                                                                                                                                                                                                                            | Proved by                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---- | ------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A    | P_can†: Block safety                                                                                        | If$`B`$ at slot $`N`$ extends a block $`B'`$ at slot $`N{-}1`$, $`B'`$ remains canonical forever, $`B`$ is timely, $`B`$'s slot-$`N`$ proposer does not equivocate, and $`B`$ includes a valid bid, then $`B`$ remains canonical at every later time                                                                                                                                             | **Lemma 7c** (standalone statement) + Lemma 7 Part (b) (anti-reorg argument under β < 20% and PROPOSER_SCORE_BOOST = 40%, with strengthenings H8 + H9 closing the split-vote and late-prior-slot reorg vectors)                                                                                                                                                                                                                                                                                            |
| A    | P_exec: Payload execution deadline is the beginning of the next slot                                         | Let$`B`$ be a block proposed in slot $`N`$ and $`P`$ the payload with `bid(B).block_hash = P.block_hash`. With the only possible exception of builders, the protocol does not require any other honest actor to complete the execution of $`P`$ before the beginning of slot $`N+1`$.                                                                                                                    | **Lemma 8b** (standalone statement, enumerating slot-$`\leq N`$ honest actors — attesters, PTC, earlier-slot proposers — and verifying none requires $`P`$ executed) + structural support from Lemma H1 (same-slot attesters signal zero), Lemma H4 (PTC reports observation, not validity), Algorithm 8 (`on_execution_payload_envelope` is the local payload-import path into `store.payloads`, while state-changing execution effects are applied later by `apply_parent_execution_payload`) |
| A    | P_DA: Data availability for chain inclusion                                                                  | If a payload hash$`h`$ is in the payload hash chain (Definition 4c) of the canonical beacon chain — equivalently, $`h = \mathit{bid}(B).\mathit{block\_hash}`$ for some FULL block $`B`$ — then some honest node successfully processed a canonical FULL-declaring successor through `on_block`, and at that node the corresponding execution payload was verified and its blob data was locally available | **Lemma 18b** (P_DA import witness), under S2. The statement is existential: hash-on-chain implies an honest-node import witness with `store.payloads[B.root]`, not that every honest node currently stores the payload                                                                                                                                                                                                                                                                                   |
| A    | P_valid: Chain validity                                                                                      | For every canonical block$`B`$, `bid(B).parent_block_hash` equals either `bid(parent(B)).block_hash` or `bid(parent(B)).parent_block_hash`; no third execution-parent hash is admissible                                                                                                                                                                                                                     | **Lemma 18a** (chain validity), using Helper 8's `bid.parent_block_hash == state.latest_block_hash` assertion plus Helper 12c's FULL/EMPTY state update                                                                                                                                                                                                                                                                                                                                                   |
| B    | P_rev†: Builder revealing protection (trustless case `bid.value > 0`)                                     | Honest reveal under A1a ⇒$`\mathit{bid}(B).\mathit{block\_hash}`$ is in the payload hash chain (Definition 4c) of the canonical beacon chain — equivalently, $`\mathit{block\_status}(\mathit{canonical}, B) = \text{FULL}`$ — under honest PTC majority and successor liveness                                                                                                                               | **Canonicity step**: Lemma 7c (≡ P_can), equivalently Lemma 7 Part (b), under H8 + H9. **FULL-on-chain step**: Lemma 7 Part (a) (PTC primary path selects FULL) + Lemma 9 second claim (honest successor's `should_build_on_full(store, head)` returns True under honest PTC because no negative timeliness or DA quorum exists — Lemma 14c) + Lemma 18 + Lemma 19 + the successor-liveness condition that turns the FULL fork-choice decision into a canonical child                             |
| B    | P_rev: Builder revealing protection (non-trustless case `bid.execution_payment > 0, bid.value = 0`)        | Honest reveal under A1b ⇒$`\mathit{bid}(B).\mathit{block\_hash}`$ is in the payload hash chain — canonicity routed through the confirmation rule $`R`$ and its assumption set $`\Sigma_R`$ instead of P_can                                                                                                                                                                                                  | **Canonicity step**: Lemma 17b (parametric in $`\Sigma_R`$ via A1b ≡ H7b's $`R`$-confirmation precondition). **FULL-on-chain step**: the same value-independent FULL-declaration machinery as the trustless row, with H7b replacing H7 and with honest PTC majority plus successor liveness                                                                                                                                                                                                      |
| B    | P_withhold: Builder withholding protection (trustless case `bid.value > 0`)                                | Honest withhold under A1a in the covered no-weight or equivocation-driven subcases ⇒ the protocol does not queue or debit that bid's trustless payment                                                                                                                                                                                                                                                              | **Case 1** (no-weight withhold): Lemma 16's no-charge lemma, using the proof-specific **I_payweight** bridge because `process_attestation` is slot-keyed, not head-keyed. **Case 2a** (equivocation reorgs $`B`$ out and no canonical IOU remains): no canonical IOU exists to charge. **Case 2b** (canonical IOU remains): Lemma 22 part (iv), under **I_slash**, clears the IOU before the boundary check                                                                     |
| B    | P_withhold: Builder withholding protection (non-trustless case `bid.execution_payment > 0, bid.value = 0`) | Trivially true — no IOU is recorded for non-trustless bids, so the protocol's charging path is structurally absent                                                                                                                                                                                                                                                                                                  | Helper 8 (`process_execution_payload_bid`) lines 17–26 are gated on `bid.value > 0`; for `bid.value = 0` no `BuilderPendingPayment` is created, and `process_attestation` (Helper 15) skips zero-`amount` entries via the precondition `payment.withdrawal.amount > 0`                                                                                                                                                                                                                             |
| C    | P_pay†: Unconditional payment to the proposer (trustless-only)                                              | For a block at slot$`N`$ with `bid.value > 0` extending $`B'`$ at slot $`N{-}1`$ ($`B'`$ canonical forever), with $`B`$ timely, $`B`$'s slot-$`N`$ proposer not equivocating, P_pay's **attestation-inclusion hypothesis**, and **L_withdraw**, the proposer's `fee_recipient` is eventually credited with the bid amount; payment is single (no double-charge)                        | Lemma 11 (a `BuilderPendingWithdrawal` is queued via Path A or Path B) + Lemma 12 (single queueing) + Lemma 12b (solvency at admission) + Lemma 21 (index correctness) + Helper 13/Helper 21 under **L_withdraw** (the queued withdrawal is eventually drained by a canonical FULL block and credited at the EL)                                                                                                                                                                                          |

† P_can, P_rev (trustless), and P_pay all rely on **both** strengthening hypotheses **H8** ($`\mathit{parent}(B)`$ remains canonical at every honest validator throughout the trace) and **H9** ($`\mathit{parent}(B).\mathit{slot} = N - 1`$ — no missing block immediately before $`B`$, with the assumption failing whenever any slot between $`\mathit{parent}(B)`$ and $`B`$ is missing). The 60% same-slot quorum on $`B`$ that Path B presupposes is **derived** (not assumed) in Lemma 11's proof from "timely publication" + S1 + S2 + H8 + H9 + A1a (i); H8 closes the split-vote-reorg gap (a Byzantine slot-$`N`$ proposer fragmenting honest attestations between $`B`$ and a sibling branch of $`\mathit{parent}(B)`$); H9 closes the late-prior-slot gap (a colluding slot-$`(N{-}1)`$/slot-$`N`$ Byzantine pair seating a competing sibling $`B'`$ at the missing slot $`N{-}1`$). Without either strengthening, the 60% derivation fails — Path B's quorum is never met — AND $`B`$ may be reorged so Path A doesn't fire either. P_exec, P_DA, P_withhold do not depend on either strengthening — they reason about structural fork-choice progress (P_exec), post-admission data availability (P_DA), and quorum failure (P_withhold) respectively, all independent of $`B`$'s canonicity and structural-slot relationships. P_rev (non-trustless) substitutes the confirmation rule's $`\Sigma_R`$ for the H8/H9 strengthenings.

Several internal mechanisms and non-externally-observable facts underlie the seven properties without being properties themselves. They are still proved formally here because the property proofs reference them:

| Internal mechanism                                      | Statement                                                                                                       | Proved by                                                                                                                                                                                                                                                            |
| ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PTC bounds proposer power                               | PTC True/True overrides a single malicious proposer's EMPTY; negative PTC quorums constrain honest construction | Lemma 14 + Lemma 14b + adversarial table below — feeds the proof of P_rev, and explains the PTC-negative EMPTY path outside S3                                                                                                                                      |
| Builder solvency at bid time                            | Builder cannot overbid across concurrent slots                                                                  | Lemma 12b — precondition for P_pay (unconditional payment); without solvency, the on-chain withdrawal could not be honoured                                                                                                                                         |
| Bid commitments are binding                             | Revealed execution payload must match bid hash                                                                  | Lemma H3 — a behavioural fact about honest builders that the overview reads off Phase 0 / Phase 3 code by construction (the same `stored_payload` object is set at bid time and read at reveal time); proved here as a lemma. Feeds the proof of P_rev.           |
| Missed-slot FULL/EMPTY resolution                       | Attestation weight resolves when next slot is missed                                                            | Lemma 8 + Lemma 28 (empty-slot state preservation, stated in §1.5) — analysed as a corner case of the fork-choice rule in §6 (Phase 5 and Fork Choice) of the overview                                                                                            |
...
| Execution validity gates FULL-node existence (per node) | FULL node exists iff payload validated                                                                          | Lemma 1 — a per-node invariant that supports P_DA (data availability for chain inclusion) externally                                                                                                                                                                |
...
| PTC votes are witness statements (honest behavior)      | Honest PTC reports observation, not validity                                                                    | Lemma H4 — a behavioural fact about honest PTC members that the overview reads off Phase 4 `ptc_vote` code (which calls `has_execution_payload_envelope`, never the execution engine); proved here as a lemma. Not externally checkable from a single PTC vote. |

#### Discharging the overview's assumptions

The [overview document](https://github.com/ethereum/epbs-security-analysis/blob/master/README.md) (Section 9) states explicit assumptions in two categories — structural (S-prefix) and algorithmic (G-prefix) — that its proof sketches depend on, plus the Phase-3 cautious-reveal protocols A1a/A1b (which the overview treats as protocol, not as a separate assumption category). This document provides the code and proofs that discharge every assumption. Note that the overview's set of named assumptions is deliberately small: it carries **two** behavioural assumptions (A1a, the trustless cautious-reveal strengthening; A1b, the non-trustless cautious-reveal strengthening, parametric in a confirmation rule $`R`$ and its assumption set $`\Sigma_R`$) and **six** algorithmic G-assumptions (payment + slashing machinery), because every other honest-actor fact and every fork-choice helper is read off code that the overview itself shows in full (Phase handlers in overview §5 and §6, payment helpers in overview §6/§7). The tables below show the correspondence for both the named overview assumptions and the formal-treatment lemmas that prove the additional honest-behaviour facts the overview cites by phase-code reference.
...
| --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S1 (synchrony Δ < T_att)                                                                                                   | Stated explicitly in each lemma that needs it (e.g., Lemma 7 Part (b), Lemma 11)                                                                                                                                                                                                                                                                                                                                                                  |
| S2 (per-slot online-participation / adversarial-weight bound, β < 20%)                                                     | Derived from balance-weighted committee sampling plus online-participation assumptions: Lemmas 25–26 (concentration), used by Lemma 7, Lemma 16, Lemma 18a, Lemma 18b, Lemma 24                                                                                                                                                                                                                                                                  |
| S3 (PTC honest-online majority)                                                                                             | Used by Lemma 7 Part (a), Lemma 14, Lemma 14c — corresponds to the PTC `payload_timeliness_vote` / `payload_data_availability_vote` majority thresholds and the honest waiting discipline for both payload and blob-data observations                                                                                                                                                                                                        |
| Per-property honesty hypotheses — **S4** in this document (the overview states these inline in its §4 property statements, not as a named assumption)                                                                                        | Stated as explicit lemma hypotheses where used (e.g., "honest slot-N+1 proposer" in Lemma 7 and Lemma 9, or "honest node successfully processed the block" in Lemma 18b's import witness)                                                                                                                                                                                                                                                         |
| (strengthening, P_can, P_rev-trustless, P_pay) "$`B'`$ at slot $`N{-}1`$ will remain canonical forever" — overview §4 | Formalised as**H8 + H9** in §1.4, with $`B' = \mathit{parent}(B)`$: H8 says the parent remains canonical throughout the trace; H9 says the parent is exactly the slot-$`N{-}1`$ block, so the missing-slot configuration is excluded. Used as an explicit hypothesis in Lemma 7, Lemma 9 (second claim), and Lemma 11. P_rev's non-trustless case routes canonicity through $`\Sigma_R`$ instead and does **not** carry H8/H9. |

**Proof-specific inclusion/liveness side conditions** become explicit lemma hypotheses:

| Overview side condition | Standing use in this document                                                                                                                                                                                                                         |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| I_payweight             | Used by Lemma 16's P_withhold no-weight subcase. It supplies the bridge from "A1a's support-for-$`B`$ weight is < 40%" to "`payment.weight` cannot reach 60%," because Helper 15 accumulates payment weight by slot rather than by attested head. |
| I_slash                 | Used by Lemma 16's equivocation-driven P_withhold subcase and Lemma 22 part (iv). It supplies canonical inclusion and processing of a valid `ProposerSlashing` before the relevant `builder_pending_payments` boundary check.                     |
| L_verify                | Used by Lemma 7 / Lemma 9 for P_rev. It supplies local payload/DA/EL processing before the PTC and successor decisions after an honest reveal; S1 supplies delivery, not local verification time.                                                     |
| L_successor             | Used by P_rev's FULL-on-chain step. It turns the honest fork-choice preference for FULL into an actual canonical successor block declaring FULL.                                                                                                      |
| L_withdraw              | Used by P_pay after Lemma 11 queues a `BuilderPendingWithdrawal`. It supplies canonical FULL blocks with accepted execution payloads until the queued withdrawal is drained and credited on the execution layer.                                    |

**Behavioral assumptions** — the overview names two (A1a, trustless cautious-reveal; A1b, non-trustless cautious-reveal). They are discharged by Assumptions H7 and H7b below respectively; the formal treatment also proves additional honest-behaviour lemmas (H1–H6) that the overview reads directly off its §5 Phase handlers (the overview removed the corresponding named A-assumptions because they are restatements of phase code rather than non-derivable strengthenings):

| Overview assumption / honest-behaviour fact                                                                          | Discharged by                                                                                                                                                                                                                                                                                                                                              |
| -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A1a (honest builder reveals cautiously — trustless case `bid.value > 0`)**                                | **Assumption H7** — trustless cautious-reveal strategy: $`T_{\mathrm{att}} + \Delta \leq t_{\mathrm{rev}} < T_{\mathrm{payload\_due}} - \Delta`$ AND ≥ 40% real attestation weight observed AND no proposer equivocation, before broadcast. Used by Lemmas 7, 9, 11, 17 to deliver concrete anti-reorg / Path-A-fires guarantees under β < 20%. |
| **A1b (honest builder reveals cautiously — non-trustless case `bid.execution_payment > 0, bid.value = 0`)** | **Assumption H7b** — non-trustless cautious-reveal strategy: a chosen confirmation rule $`R`$ returns `confirmed` for $`B`$ under $`\Sigma_R`$ AND reveal-timing safety, before broadcast. Used by Lemma 17b (non-trustless P_rev canonicity step) parametric in $`R`$ and $`\Sigma_R`$.                                                |
| Same-slot attesters signal zero (read off Phase 2 attest code in overview)                                           | Lemma H1 — the `attest` handler's `if head_block.slot == slot` branch (with `head_block = store.blocks[head.root]`)                                                                                                                                                                                                                                 |
| Non-same-slot attesters signal consistently (read off Phase 2 attest code in overview)                               | Lemma H2 — the `attest` handler's `else` branch (lines 11–12)                                                                                                                                                                                                                                                                                        |
| Builder bid-reveal consistency (read off Phase 0/Phase 3 code in overview by construction)                           | Lemma H3 — the `submit_bid` handler stores the payload (line 14) and `reveal_payload` uses the same object (line 5)                                                                                                                                                                                                                                   |
| PTC reports observation, not validity (read off Phase 4 ptc_vote code in overview)                                   | Lemma H4 — the `ptc_vote` handler calls `has_execution_payload_envelope` (line 9), not the execution engine                                                                                                                                                                                                                                           |
| Honest builder withholding is conditional (read off Phase 3 reveal_payload code in overview)                         | Lemma H5 — the `reveal_payload` handler's only early-return path (lines 5–6, counting from `def`)                                                                                                                                                                                                                                                    |

**Algorithmic assumptions.** The overview names **six** G-assumptions (payment + slashing machinery) — the fork-choice helpers (`get_node_children`, `get_weight`, the vote-support relation, `on_block`'s parent-FULL assertion, and the `get_head` tiebreaker) live as plain code in the overview's §6 ("Phase 5 and Fork Choice") and are not carried as named assumptions there. The formal-treatment provides the full algorithm/helper code for every spec function the overview references; the six named G-assumptions are discharged below, and the formal-treatment's Algorithms 1–9 give the full fork-choice machinery the overview's §6 cites by function name:
...
**Fork-choice machinery referenced by the overview's §6 (plain code in the overview, full formal code here):**

| Overview function (cited by name in overview §6)                                                                                                                  | Formal-treatment code + supporting lemma                                                                                                                                                       |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `get_node_children` — FULL/EMPTY children for PENDING, PENDING children for FULL/EMPTY                                                                          | Algorithm 2 (lines 3–13) + Lemma 1 (FULL iff `store.payloads`) + Lemma 3 (tree alternation)                                                                                                 |
| `get_weight` — zero-return rule for FULL/EMPTY of immediately previous slot                                                                                     | Algorithm 5 + Lemma 4 (zero-return rule)                                                                                                                                                       |
| Vote-support relation — same-slot exclusion; non-same-slot direct attribution by `payload_present`; descendant attribution by chain-structural `parentStatus` | Spec helpers `get_supported_node` + `is_ancestor`, expanded by Algorithm 4's pedagogical `is_supporting_vote` wrapper + Lemma 2 (same-slot exclusion) + Lemma 5 (descendant attribution) |
| `get_head` — traversal consults `should_extend_payload` via tiebreaker only between children from `get_node_children`                                       | Algorithm 1 (lines 8–10):`max` over `children` only; Definition 13 maps to `should_extend_payload`                                                                                      |
| `on_block` — rejects a block declaring FULL ancestry without `store.payloads[parent_root]`                                                                    | Algorithm 7 (`on_block`) line 5 — the `is_payload_verified(store, block.parent_root)` hard assertion; cf. Lemma 13                                                                        |

#### Properties of honest behavior

The following properties follow directly from the `@Upon` handlers in Section 1.2. They do not depend on the fork-choice algorithms — they are consequences of how honest actors construct their messages. These properties serve as the formally verified assumptions that the fork-choice proofs rely on.
...
*Proof.* By the `attest` handler (Section 1.2, Attester), the branch guard checks `if head_block.slot == slot` (with `head_block = store.blocks[head.root]`). When this condition is true, the body unconditionally executes `data.index = 0`. No other branch is reachable in that arm. ∎
...
*Proof.* By the `attest` handler (Section 1.2, Attester), when `head_block.slot != slot` (the `else` branch, with `head_block = store.blocks[head.root]`), the body executes `data.index = 1 if head.payload_status == FULL else 0`. The signal is fully determined by `head.payload_status`. ∎
...
*Proof.* By the `ptc_vote` handler (Section 1.2, PTC), counting from `def ptc_vote(...)` as line 1, lines 8–11: `if has_execution_payload_envelope(store, block_root): data.payload_present = True; else: data.payload_present = False`. The function `has_execution_payload_envelope` checks whether the envelope was previously seen before the payload deadline — it does not invoke the execution engine or call `verify_execution_payload_envelope`. The handler contains no call to any execution validation function. ∎
...
*Proof.* By the `reveal_payload` handler (Section 1.2, Builder), counting from `def reveal_payload(...)` as line 1: line 5 checks `if not is_head_of_chain(block, store)` and line 6 returns without broadcasting. This is the only early-return path after the builder-index check (lines 3–4). If `is_head_of_chain(block, store)` is true, the function proceeds to construct, sign, and broadcast the payload (lines 7 onward). ∎

---

**Lemma H6** (Proposer preferences constrain builder bids). *A gossip-propagated builder bid can only be forwarded by the network if a matching SignedProposerPreferences has been broadcast, the bid's `fee_recipient` matches the preferences, and the bid's `gas_limit` is compatible with the preferences' `target_gas_limit` under `is_gas_limit_target_compatible`.*

This property establishes that builders cannot unilaterally choose the payment destination or an arbitrary gas limit — the destination must match the proposer preferences, and the gas limit must follow the target-compatible EIP-1559 transition rule. It follows from the gossip validation rules rather than the honest behavior handlers directly, but it constrains the inputs that the honest behavior code operates on.

*Proof.* By the gossip validation rules in `p2p-interface.md`: an `execution_payload_bid` message is `[IGNORE]`d if no matching `SignedProposerPreferences` for the bid's slot and dependent root has been seen, `[REJECT]`ed if `bid.fee_recipient != proposer_preferences.fee_recipient`, and ignored unless `is_gas_limit_target_compatible(parent_gas_limit, bid.gas_limit, proposer_preferences.target_gas_limit)` returns True. These rules are enforced by every honest node in the gossip network before forwarding. Consequently, any bid that reaches the proposer via gossip satisfies the fee-recipient equality and gas-limit compatibility constraints. (Off-protocol bids are not subject to these gossip rules.) ∎

---

**Assumption H7** (Honest builder reveals via the cautious strategy). *Throughout this assumption and below, a* **real attestation** *for a block $`B`$ means an `Attestation` (or `SingleAttestation`) signed by a validator in the slot committee for `slot(B)` with `data.beacon_block_root = B.root` — i.e., an actual validator-cast vote, as distinct from the proposer-boost score that `get_weight` may add through `should_apply_proposer_boost`.*

*An honest builder broadcasts the `SignedExecutionPayloadEnvelope` for a beacon block $`B`$ with bid `bid` only after the builder has observed (in its local store): (i) the reveal-time window $`T_{\mathrm{att}} + \Delta \leq t_{\mathrm{rev}} < T_{\mathrm{payload\_due}} - \Delta`$ holds — a lower bound (closes the equivocation window so that any equivocation $`B''`$ broadcast at $`t_{\mathrm{eq}} \leq T_{\mathrm{att}}`$ is delivered to the builder by $`t_{\mathrm{rev}}`$ under S1) and an upper bound (reveal-timing safety: envelope reaches every honest PTC member before `get_payload_due_ms()` under S1); (ii) at least `PROPOSER_SCORE_BOOST = 40%` of a slot committee's effective balance in real attestations supporting $`B`$; (iii) a single non-equivocating beacon block at slot $`\mathit{slot}(B)`$ containing `bid` (no proposer equivocation by $`B.\mathit{proposer\_index}`$ for that slot). Equivalently, an honest builder withholds whenever any condition fails.*

This is the **cautious-reveal** strategy: the builder waits until the beacon block has accumulated at least as much real attestation weight as a single proposer boost is worth, and until no proposer-side equivocation evidence has surfaced. The 40% threshold is calibrated to `PROPOSER_SCORE_BOOST = 40` so that, under the per-committee Byzantine bound β < 20% (Section 1.4 adversarial model), the slot-N+1 proposer cannot reorg slot N via boost alone: the boost grants at most 40% synthetic weight to a competing branch, but the committed branch has ≥ 40% real weight plus whatever further honest attestations accrue.
...
1. **Real-attestation observation:** the builder polls its local store for attestations supporting `block.root` and waits until the cumulative real attestation weight (Definition: real attestation = validator-cast vote, excluding proposer-boost score) reaches `PROPOSER_SCORE_BOOST = 40%` of a slot committee's effective balance.
...
3. **Delivery-window separator:** the builder waits until $`t_{\mathrm{rev}} \geq T_{\mathrm{att}} + \Delta`$ and reveals before $`t_{\mathrm{rev}} < T_{\mathrm{payload\_due}} - \Delta`$. The lower bound closes the equivocation window that Lemma 7 Part (b) relies on: any equivocation $`B''`$ broadcast at $`t_{\mathrm{eq}} \leq T_{\mathrm{att}}`$ is delivered to the builder by $`t_{\mathrm{eq}} + \Delta \leq T_{\mathrm{att}} + \Delta \leq t_{\mathrm{rev}}`$ under S1, so the equivocation-evidence check catches it; any equivocation broadcast at $`t_{\mathrm{eq}} > T_{\mathrm{att}}`$ arrives at honest validators after $`T_{\mathrm{att}}`$ and so does not influence honest slot-$`N`$ votes. The upper bound is reveal-timing safety: under S1, the envelope reaches every honest PTC member before `get_payload_due_ms()`, so the PTC observation vote can be True. Local DA and EL verification happen separately through `on_execution_payload_envelope`. Either bound failing causes the builder to withhold.

If any check fails before $`T_{\mathrm{ptc}}`$, the builder withholds (early-return path). The `reveal_payload` pseudocode in Section 1.2 captures the spec's minimal rule (`is_head_of_chain`) at lines 5–6; the three H7 checks are layered on top of that and constitute the strategy's defining content. Because H7 is an assumption, not a theorem, there is no formal proof — only the explicit statement of the strategy that honest builders are assumed to follow. Compare **A1** in the [overview](https://github.com/ethereum/epbs-security-analysis/blob/master/README.md) (§9.2), where this is stated explicitly as a non-normative strengthening (the sole behavioural assumption named in the overview). ∎

*Remark on dischargeability.* H7 cannot be discharged from the spec alone; it can only be discharged by (i) augmenting `builder.md` with the cautious-reveal rule, or (ii) showing empirically that production builder implementations adopt it. Either route would convert H7 from a behavioural assumption into a derivable lemma. Under the current spec, every Lemma in §1.4–§1.5 that cites H7 is conditional on builders adopting the cautious strategy; without it, those lemmas degrade to the weaker H5-only conclusions noted in the surrounding remarks.
...
*Naming note.* The overview (§5 Phase 3) labels this trustless cautious-reveal protocol **A1a**. Throughout this document we write **H7 ≡ A1a**, using the formal-treatment-internal label H7 in lemma proofs and citing A1a where the overview's framing is relevant.

---

**Assumption H7b** (Honest builder reveals via the **non-trustless** cautious strategy, ≡ overview **A1b**). *For a non-trustless bid (`bid.execution_payment > 0` and `bid.value = 0`), an honest builder broadcasts the `SignedExecutionPayloadEnvelope` for a beacon block $`B`$ with bid `bid` only after the builder has observed (in its local store): (i) $`B`$ has been confirmed by a chosen confirmation rule $`R`$ — i.e., $`R`$ returns `confirmed` for $`B`$ under the rule's assumption set $`\Sigma_R`$; (ii) the reveal-timing safety check passes (the broadcast can reach honest PTC members before $`T_{\mathrm{ptc}}`$ under synchrony $`\Delta`$). Equivalently, an honest non-trustless builder withholds whenever either condition fails.*

This is the **non-trustless cautious-reveal** strategy. Whereas H7 (≡ A1a) uses a builder-side 40%-real-weight observable and an equivocation-visibility check to anchor canonicity via P_can (Lemma 7 Part (b)), H7b routes canonicity through the **confirmation rule** $`R`$ directly. The rule is a parameter: any provably-secure confirmation rule (an algorithm that, under its own assumption set $`\Sigma_R`$, returns `confirmed` for $`B`$ only if $`B`$ remains canonical forever) is admissible.

*Why a confirmation rule replaces the 40%-threshold + H8/H9 strengthening.* In the trustless case, the builder's reveal carries protocol-side payment exposure (Path B can charge it if the beacon block is widely attested). The cautious-reveal threshold + H8 + H9 are calibrated to bound this exposure under β < 20%. In the non-trustless case, there is **no protocol-side charging path** (`process_execution_payload_bid` Helper 8 creates no IOU when `bid.value = 0`; cf. Lemma 11's scoping). The builder's only loss from a bad reveal is delivering an execution payload it isn't paid for off-protocol — a builder-side reputation/business decision, not a consensus-layer concern. So the cautious-reveal protocol in the non-trustless case is free to use a stronger, slower canonicity check (a full confirmation rule) without protocol-mandated calibration.

*Parametricity in $`\Sigma_R`$.* The non-trustless P_rev (Lemma 17b below) holds parametrically in $`R`$ and its $`\Sigma_R`$. Choosing the [Fast Confirmation Rule (FCR)](https://fastconfirm.it) pulls in $`\Sigma_{\mathrm{FCR}}`$ (synchrony + a Byzantine bound + a quorum predicate); choosing a more conservative rule pulls in stronger assumptions but slows confirmation down. The overview (§4) writes the P_rev non-trustless case statement parametrically in $`\Sigma_R`$ for this reason.

*Remark.* H7b does **not** carry the H8/H9 strengthening hypotheses — $`R`$'s confirmation guarantee under $`\Sigma_R`$ subsumes them. (The split-vote and late-prior-slot reorg vectors that H8/H9 close in the trustless case are themselves consequences of fork-choice ambiguity that any reasonable confirmation rule rejects.) ∎

---

**Lemma 17b** (Canonicity via $`\Sigma_R`$ — non-trustless P_rev canonicity step). *Let $`B`$ be a beacon block at slot $`N`$ with `bid.execution_payment > 0` and `bid.value = 0` (non-trustless bid), and suppose an honest builder reveals $`B`$'s execution payload following Assumption H7b with confirmation rule $`R`$. Then under $`\Sigma_R`$, $`B`$ remains canonical at every later time.*

*Proof.* By H7b condition (i), the builder reveals only when $`R`$ returns `confirmed` for $`B`$. By the definition of *provably secure under $`\Sigma_R`$*, every block confirmed by $`R`$ remains canonical at every later time under $`\Sigma_R`$. Hence $`B`$ stays canonical. ∎

This lemma supplies the canonicity step of the non-trustless P_rev proof (overview §4, Category B, P_rev non-trustless case). The remainder of the non-trustless P_rev argument — the FULL-on-chain machinery (PTC primary path or fallback (c)) — is the same as the trustless case (Lemma 7 Part (a) + Lemma 9 second claim + Lemma 18 + Lemma 19), reused with **Lemma 17b replacing "Lemma 7c (≡ P_can) + H8 + H9"** as the canonicity step. The two canonicity routes deliver the same content ($`B`$ stays canonical) under disjoint assumption sets: H8/H9 under S1+S2 (trustless) versus $`\Sigma_R`$ (non-trustless).

---

**Assumption H8** (Strengthening: parent block remains canonical). *Let $`B`$ be a beacon block at slot $`N`$ under analysis. Throughout the trace from slot $`N`$ onward, $`\mathit{parent}(B)`$ remains in the canonical chain of every honest validator.*

This is a **strengthening hypothesis** used by Lemmas 7, 9, and 11 to close a residual gap that the cautious-reveal threshold of H7 does not address on its own. The gap: a Byzantine slot-$`N`$ proposer can publish $`B`$ late (broadcast at $`t > T_{\mathrm{att}} - \Delta`$), fragmenting honest slot-$`N`$ attestations between $`B`$ and a sibling-of-parent($`B`$) branch. With $`\geq 40\%`$ of honest weight on $`B`$ and the rest on the sibling branch, the H7 reveal check still passes — but an honest slot-$`N+1`$ proposer's `get_head` walks to the heavier sibling branch and reorgs $`B`$. Forcing $`\mathit{parent}(B)`$ to remain canonical excludes the sibling branch from honest fork-choice consideration, so the slot-$`N+1`$ proposer can only extend $`\mathit{parent}(B)`$; under H7 condition (iii) (no proposer equivocation at slot $`N`$), $`B`$ is the unique descendant of $`\mathit{parent}(B)`$ at slot $`N`$, so it is extended and not reorged.

*Why this is an assumption.* H8 cannot be derived from H7 alone — H7's 40% threshold is calibrated against *boost-only* reorgs (a slot-$`N+1`$ proposer using only proposer boost to outweigh $`B`$), not against *split-vote* reorgs (a Byzantine slot-$`N`$ proposer fragmenting honest attestations across two branches of $`\mathit{parent}(B)`$). Closing the split-vote gap formally requires either (i) raising H7's threshold above the worst-case Byzantine + late-broadcast split (likely $`\geq 50\%`$, possibly $`\geq 60\%`$ to absorb worst-case Byzantine slot-$`N`$ attestations), or (ii) adding an explicit "block was timely" precondition to the cautious-reveal check. Both routes are open work; until one of them is adopted, the lemmas that need anti-reorg guarantees cite H8 as an explicit hypothesis.

*Remark.* H8 is parallel in spirit to S4 (per-property honest-actor hypotheses): like S4, it is applied per-property rather than standing globally. Lemmas 7, 9, and 11 rely on H8 for the block under analysis; Lemmas 16, 18b, 22 do not — they are independent of slot-$`N`$ canonicity. The current overview packages H8 together with H9 as the single strengthening "$`B'`$ at slot $`N{-}1`$ will remain canonical forever."

---

**Assumption H9** (Strengthening: parent block is at the immediately preceding slot). *Let $`B`$ be a beacon block at slot $`N`$ under analysis. Then $`\mathit{parent}(B).\mathit{slot} = N - 1`$ — there is no missing block immediately before $`B`$ on $`B`$'s chain. (Equivalently: $`B`$'s chain has no gap at slot $`N{-}1`$, and by extension every missing-slot configuration between $`\mathit{parent}(B)`$ and $`B`$ is excluded: if $`\mathit{parent}(B)`$ is at slot $`N{-}k`$ for any $`k > 1`$, the assumption fails.)*

This is a **strengthening hypothesis** used alongside H8 by **Lemma 7 (the P_rev-trustless proof), Lemma 9's second claim, and Lemma 11 (the P_pay proof)** to close a residual gap orthogonal to the split-vote-at-slot-$`N`$ gap H8 addresses. Lemma 9's second claim — that the FULL declaration happens at slot $`N{+}1`$ under A1a's 40% reveal threshold — depends on H9 through Lemma 7's Part (a) chain. For P_pay in particular, the 60% same-slot quorum on $`B`$ that Path B presupposes is **derived** in Lemma 11's proof from "timely publication" + S1 + S2 + H8 + H9 — it is not assumed. Without H9, the derivation fails: the late-prior-slot attack splits slot-$`N`$ honest attestations between $`B`$ and a late-released sibling $`B'`$, capping $`B`$'s `payment.weight` below 60% AND reorging $`B`$ at slot $`N{+}1`$. Concretely:

- The slot-$`N{-}1`$ proposer withholds; slot-$`N{-}1`$ attesters see no slot-$`N{-}1`$ block and vote for the slot-$`N{-}2`$ head $`H`$.
- The slot-$`N`$ proposer publishes $`B`$ (with $`\mathit{parent}(B) = H`$, skipping the empty slot) near $`T_{\mathrm{att}}`$, reaching one subset of slot-$`N`$ attesters; the slot-$`N{-}1`$ proposer simultaneously releases its withheld $`B'`$ (also parented at $`H`$, claiming slot $`N{-}1`$) to a different subset.
- The adversary tunes delivery so $`B`$ collects $`\geq 40\%`$ real attestation weight (clearing H7 condition (ii) — 20% adversary plus $`\sim`$20% honest on $`B`$) while $`B'`$ collects $`\sim 60\%`$ of slot-$`N`$ honest weight. The builder, seeing $`\geq 40\%`$ real weight on $`B`$ and no proposer equivocation against $`B`$'s proposer (H7 condition (i) — $`B`$ and $`B'`$ are signed by *different* proposers), reveals per H7.
- At slot $`N{+}1`$ the proposer boost on $`B`$ resets (`on_tick_per_slot` clears `proposer_boost_root`); fork-choice now compares the siblings $`B`$ ($`40\%`$) and $`B'`$ ($`\sim 60\%`$) below their common parent $`H`$. $`B'`$ wins, the slot-$`N{+}1`$ proposer extends $`B'`$, and $`B`$ is reorged.

H8 holds in this scenario ($`\mathit{parent}(B) = H`$ is canonical because $`H`$ is an ancestor of both $`B`$ and $`B'`$) but does not protect $`B`$ itself — that is what H9 supplies. With $`\mathit{parent}(B).\mathit{slot} = N - 1`$, no sibling at slot $`N - 1`$ parented at the slot-$`N{-}2`$ head can be introduced without slot-$`N{-}1`$ proposer equivocation (a separately-deterred class, addressed by `process_proposer_slashing`), so the late-prior-slot path is closed.

*Why this is an assumption.* H9 cannot be derived from H7 and H8 alone — H7's 40% threshold and H8's canonicity of $`\mathit{parent}(B)`$ jointly close the same-slot-$`N`$ publish and split-vote-of-parent gaps, but neither addresses the late-prior-slot sibling. Closing it formally would require either (i) raising H7's threshold so that a competing sibling parented above $`\mathit{parent}(B)`$ cannot be tuned to clear the threshold simultaneously with $`B`$ on a small honest subset, or (ii) adding an explicit "no competing block of greater real weight at $`t_{\mathrm{rev}}`$" precondition to H7. Both routes are open work; until one is adopted, the lemmas that need anti-reorg guarantees cite H9 as an explicit hypothesis alongside H8.

*Remark.* H9 is parallel to H8 in form (a per-property strengthening hypothesis) and parallel in scope: Lemma 7, Lemma 9 (second claim), and Lemma 11 (P_pay) rely on it. Lemmas 16, 18b, 22 are independent of H9 — quorum failure and post-admission DA do not reason about $`B`$'s structural slot relationship to its parent. The current overview packages H9 together with H8 as the single strengthening "$`B'`$ at slot $`N{-}1`$ will remain canonical forever."

---

#### Structural properties of the fork-choice algorithms

The central object of this section is `get_head` (Algorithm 1): the function that every node runs to determine the canonical chain. `get_head` traverses the fork-choice tree starting from the justified checkpoint, alternating at each block between two decisions — "was the execution payload delivered?" (the FULL/EMPTY split) and "which block comes next?" (the block-selection step). To make these decisions, it relies on `get_weight` (Algorithm 5) for attestation-based scoring, the `get_supported_node` / `is_ancestor` vote-support relation (expanded by Algorithm 4 for proof readability), `should_extend_payload` (Algorithm 6) for the PTC-based tiebreaker, and `get_node_children` (Algorithm 2) for determining what branches exist in the tree.
...
This lemma establishes that the FULL branch is gated entirely by local payload verification: the node must have blob data locally available and the execution engine must accept the payload. If the builder withholds the execution payload, or reveals an invalid execution payload that fails `verify_execution_payload_envelope`, then `store.payloads[r]` is never populated (Algorithm 8 line 7 is never reached), and the FULL child does not exist. This is the fundamental enforcement mechanism of ePBS: it is not PTC votes or attestation weight that prevents an invalid execution payload from being treated as delivered — it is the absence of the FULL branch in the tree.
...
This lemma establishes that same-slot attesters — who are required to set `data.index = 0` (Section 1.2, Attester lines 9–10) because they vote at 25% of the slot, before the builder reveals at up to 75% (Definition 1) — cannot influence the FULL/EMPTY weight for the block they attested to. Their votes are not "counted as EMPTY" — they are excluded entirely from both sides.

*Proof.* By Algorithm 4's expansion of the spec vote-support relation, when `vote.root == r` (Case 1): if `s == PENDING`, the vote supports PENDING. If `s ∈ {FULL, EMPTY}`: the wrapper asserts `vote.slot >= block.slot` (matching `validate_on_attestation`'s no-future-block guard). If `vote.slot == block.slot` (the same-slot case covered by this lemma), the wrapper returns False before reaching the payload-status check. Therefore, for any same-slot vote with `m.slot = slot(B)` and any `s ∈ {FULL, EMPTY}`, the spec predicate `is_ancestor(store, get_supported_node(store, m), (r, s))` is False. Since `get_attestation_score` in Algorithm 5 only adds a validator's balance when this predicate holds, the vote contributes zero attestation weight to both (r, FULL) and (r, EMPTY); the zero-return rule separately makes both weights 0 in the immediately previous-slot case. ∎

*Remark.* Same-slot votes do support (r, PENDING), which matters when competing blocks exist at the same slot (proposer equivocation). They also support ancestor nodes via Case 2, where `get_ancestor` determines the FULL/EMPTY attribution from the chain structure, not from the voter's `payload_present` field.
...
*Proof.* By Algorithm 5 (`get_weight`), the first branch checks `is_previous_slot_payload_decision(store, node)`. For node `(r, s)` with `s ∈ {FULL, EMPTY}` and `slot(B) + 1 = get_current_slot(store)`, this predicate is true. The function immediately returns `0`, before computing either the attestation score or any proposer-boost score. ∎
...
**Lemma 5** (Descendant vote attribution). *Let m = (slot, root, payload_present) be a vote where root ≠ r, and let B_r be the block with root r. If the vote-support predicate expanded by `is_supporting_vote(store, (r, s), m)` is True for s ∈ {FULL, EMPTY}, equivalently if the spec condition `is_ancestor(store, get_supported_node(store, m), (r, s))` is True, then the chain from the voted-for block back to B_r passes through the s branch at r. The attribution is determined by the chain of parentStatus declarations (Definition 4), not by m.payload_present.*
...
*Proof.* By Algorithm 4's expansion of the spec vote-support relation, when `vote.root ≠ r` (Case 2), the wrapper computes `(anc_root, anc_status) = get_ancestor(store, get_supported_node(store, vote), block.slot)` and returns `anc_root == r and (s == PENDING or s == anc_status)`. For `s ∈ {FULL, EMPTY}`, the disjunct `s = PENDING` is false, so the condition reduces to `anc_root == r and s == anc_status`. Therefore, the vote supports `(r, s)` only if `s == anc_status`, where `anc_status` is the payload status returned by `get_ancestor`.

If `vote.root` is not strictly after `slot(B_r)`, then `get_ancestor` returns a node with `anc_root != r` unless the vote is for B_r itself, which Case 2 excludes. Thus every actually supporting Case-2 vote is for a strict descendant of B_r. For such a descendant, Algorithm 3 first steps from the voted-for block to its parent and assigns the parent status from that child block's `parentStatus`; the voted-for node's own `vote_status` is not the status returned at B_r. Continuing this walk, `anc_status` is computed as `parentStatus(D)` where D is the first block in the chain from the voted-for block back to B_r's slot. By Definition 4, `parentStatus(D)` compares `bid(D).parent_block_hash` against `bid(parent(D)).block_hash` — block-level data that is fixed at block creation time. The field `vote.payload_present` can determine the status of the directly voted-for node, but it does not determine the FULL/EMPTY branch attributed to an earlier ancestor in Case 2. ∎
...
The structural properties (Lemmas 1–5) describe invariants of the algorithms themselves, independent of who is honest or dishonest. We now prove what the fork-choice rule produces when the relevant actors follow the protocol as specified in Section 1.2 (Honest Behavior). These properties require **assumptions** about honest behavior — stated explicitly in each lemma and grounded in the pseudocode of Section 1.2 — and they compose the structural lemmas to reason about end-to-end outcomes: given a specific scenario (builder reveals, builder withholds, slot is missed), what does `get_head` return?
...
This is the most basic safety property of ePBS: if the builder does not deliver (or honestly withholds — see Section 1.2, Builder Phase 2 lines 5–6), the protocol does not pretend the execution payload exists. The proof is a direct consequence of Lemma 1.
...
**Lemma 7** (Honest builder and honest PTC imply FULL, and the block is not reorged). *Let B be a block with root r at slot N. Assume: (1) the builder reveals a valid execution payload for B via the cautious strategy of Assumption H7, so that r ∈ store.payloads and B has accumulated at least 40% of a slot committee's effective balance in real attestations by reveal time; (2) a majority of PTC members are honest and observe the payload, so that more than PTC_SIZE / 2 entries in payload_timeliness_vote[r] are True and more than PTC_SIZE / 2 entries in payload_data_availability_vote[r] are True; (3) the strengthening hypothesis of Assumption H8 holds — $`\mathit{parent}(B)`$ remains canonical at every honest validator throughout the trace from slot $`N`$ onward; (4) the strengthening hypothesis of Assumption H9 holds — $`\mathit{parent}(B).\mathit{slot} = N - 1`$ (no missing block immediately before B). Then, when get_head is executed at slot N + 1 (i.e., current_slot = N + 1), the traversal selects (r, FULL) over (r, EMPTY), and B-FULL is canonical for every honest validator running get_head at slot N + 1 — no slot-N+1 proposer can reorg slot N via proposer boost alone (under β < 20%), no Byzantine slot-N proposer can reorg B via a split-vote reorg of $`\mathit{parent}(B)`$ (by H8), and no colluding slot-(N-1)/slot-N Byzantine pair can reorg B via a late-prior-slot sibling at slot N-1 (by H9).*

This is the honest-case correctness property: when the builder delivers via cautious reveal (Assumption H7 ≡ A1a, the trustless cautious-reveal protocol) and the PTC honestly reports it (Section 1.2, PTC lines 9–10), the fork-choice rule correctly picks the FULL path for the most recent block, AND the FULL outcome is robust against a malicious slot-N+1 proposer trying to reorg slot N with their own boost. **Parts (a) and (b) of this lemma together are the formal proof of property P_rev (Builder revealing protection, trustless case) from §4 of the overview** — Part (a) delivers the FULL declaration on chain at slot N+1 (the claim that the execution payload is *included* in the canonical chain), and Part (b) is the anti-reorg guarantee that the beacon block itself is not reorged out. Part (b) is also the formal proof of **property P_can (Block safety)** from §4 of the overview — block safety is exactly the anti-reorg guarantee for $`B`$ under H8 + H9 + timeliness + S1 + S2.
...
- *(no observable equivocation through $`t_{\mathrm{rev}}`$)* The builder's H7 check at reveal time $`t_{\mathrm{rev}} \geq T_{\mathrm{att}} + \Delta`$ confirms no equivocation has surfaced in the builder's view. Any equivocation $`B''`$ broadcast at $`t_{\mathrm{eq}} \leq T_{\mathrm{att}}`$ would have reached the builder by $`t_{\mathrm{eq}} + \Delta \leq T_{\mathrm{att}} + \Delta \leq t_{\mathrm{rev}}`$ under S1, so the builder would have observed it and withheld — contradicting the reveal hypothesis. Hence any equivocation $`B''`$ at slot $`N`$ was broadcast at $`t_{\mathrm{eq}} > T_{\mathrm{att}}`$, after every honest slot-$`N`$ validator had already voted at $`T_{\mathrm{att}}`$.
...
- *(honest behaviour, Section 1.2 `attest` handler + Lemma H1)* Each honest slot-N validator runs `get_head` at $`T_{\mathrm{att}}`$, sees B as the head (B is the only slot-N block in view), and casts a same-slot attestation with `data.beacon_block_root = B.root` (from the `attest` handler's unconditional `data.beacon_block_root = head.root` step) and `data.index = 0` (by Lemma H1, since the head is from the attester's own slot).

By Algorithm 4's Case 1 expansion, every such attestation supports the node $`(B.\mathit{root}, \mathrm{PENDING})`$. (By Lemma 2, these same attestations do *not* support $`(B.\mathit{root}, \mathrm{FULL})`$ or $`(B.\mathit{root}, \mathrm{EMPTY})`$ — but that is irrelevant to the present comparison, which is at the PENDING level.)
...
- **Proposer boost.** If $`B'`$ receives proposer boost, Algorithm 5 adds `get_proposer_score(store)` to ancestors of `(B'.root, PENDING)` whenever `should_apply_proposer_boost(store)` permits the boost. Since `(B'.root, PENDING)` is an ancestor of itself, the maximum boost contribution to $`B'`$ is `PROPOSER_SCORE_BOOST/100 · W = 0.4 W`. If `should_apply_proposer_boost` suppresses the boost, the contribution is 0 and the bound is even stronger.
- **Slot-N attestations.** No slot-N attestation supports the node `(B'.root, PENDING)`. We check both cases of Algorithm 4's vote-support expansion:
  ...
  - **Case 2** computes `(anc_root, _) = get_ancestor((B.root, PENDING), slot(B'))` and requires `anc_root == B'.root`. By Algorithm 3 lines 4–5, since $`\mathrm{slot}(B) = N \leq N+1 = \mathrm{slot}(B')`$, `get_ancestor` returns `(B.root, PENDING)` — i.e., `anc_root == B.root ≠ B'.root`. So Case 2 does not support `(B'.root, PENDING)` either.
...
*Scope of this argument.* The weight comparison above is local to $`\mathit{parent}(B)`$'s child level — it rules out competitors $`B'`$ that share $`\mathit{parent}(B)`$. Two further attack vectors are excluded by the strengthening hypotheses listed in this lemma's statement: (i) a Byzantine slot-$`N`$ proposer publishing $`B`$ late (at $`t > T_{\mathrm{att}} - \Delta`$) so that some honest attesters fail to see $`B`$ by $`T_{\mathrm{att}}`$ and instead vote on a sibling branch of $`\mathit{parent}(B)`$ — excluded by **Assumption H8** (hypothesis (3)); and (ii) a colluding slot-$`(N{-}1)`$/slot-$`N`$ Byzantine pair exploiting a missing slot $`N{-}1`$ to seat a sibling $`B'`$ at slot $`N{-}1`$ parented at the slot-$`N{-}2`$ head, which outweighs $`B`$ at slot $`N{+}1`$ once proposer boost resets — excluded by **Assumption H9** (hypothesis (4); see H9's statement for the full attack trace). Under H8, no honest fork-choice ever leaves $`\mathit{parent}(B)`$'s branch, so the only competitors to $`B`$ that survive are siblings sharing $`\mathit{parent}(B)`$ — and these are excluded by H7 condition (iii) (no proposer equivocation at slot $`N`$) and by the weight arithmetic above. Under H9, the late-prior-slot path is closed because no sibling of $`\mathit{parent}(B)`$ at slot $`N{-}1`$ can be retroactively introduced without slot-$`(N{-}1)`$ proposer equivocation (a separately-deterred class, addressed by `process_proposer_slashing` — Helper 22).

**Step 4: Persistence past $`T_{\mathrm{att}}`$ of slot N+1.**

After slot-N+1 attestations are cast, the gap can only widen or stay positive under $`\beta < 0.2`$. In what follows, the slot-N+1 committee contributes a fresh $`W_{N+1}`$ worth of weight; under the per-slot stability convention from line 1111 above (Part (b)'s opening), $`W_{N+1} \approx W_N = W`$, so we add a second $`W`$ on top of the slot-N contribution from $`(*)`$:

- Honest slot-N+1 attesters run `get_head`, observe (per Step 3) that $`(B.\mathit{root}, \mathrm{PENDING})`$ wins, traverse into B's branch, and attest on B's chain. Two cases: if no slot-N+1 block extends B, they attest for $`B`$ directly (Case 1, supporting $`(B.\mathit{root}, \mathrm{PENDING})`$); if an honest extending block exists at slot N+1, they attest for that block (Case 2, descendant attribution to $`(B.\mathit{root}, \mathrm{PENDING})`$ via the `s == PENDING` disjunct in Algorithm 4). Either way, the contribution to $`\mathit{weight}((B.\mathit{root}, \mathrm{PENDING}))`$ is at least $`(1 - \beta)\, W_{N+1}`$, layered on top of the slot-N contribution $`(1-\beta)\, W_N`$ from Step 1.
...
Inputs (i) and (iii) come from the network/adversary model directly. **Input (ii) is delivered by the combination of H7's "no proposer equivocation" precondition and synchrony.** Specifically: H7 requires the builder to observe no equivocation in its local view at reveal time $`t_{\mathrm{rev}} \geq T_{\mathrm{att}} + \Delta`$. Any equivocation broadcast at $`t_{\mathrm{eq}} \leq T_{\mathrm{att}}`$ would be delivered to the builder by $`t_{\mathrm{eq}} + \Delta \leq t_{\mathrm{rev}}`$ under S1, so the builder would have observed it; hence "no equivocation broadcast at $`t_{\mathrm{eq}} \leq T_{\mathrm{att}}`$", which by S1 implies "no equivocation visible to honest validators at $`T_{\mathrm{att}}`$". Hence (ii).
...
**Lemma 7c** (Block safety — formal proof of P_can). *Let $`B`$ be a beacon block at slot $`N`$ that extends a block $`B'`$ at slot $`N{-}1`$ with $`B'`$ remaining canonical forever (Assumptions H8 and H9, jointly capturing "$`B'`$ will remain canonical forever" in the overview's framing). Assume $`B`$ includes a valid bid, $`B`$ is timely (Definition 3c, $`t + \Delta \leq T_{\mathrm{att}}`$), **$`B`$'s slot-$`N`$ proposer does not equivocate** (no second block at slot $`N`$ signed by $`B.\mathit{proposer\_index}`$ is broadcast), and the structural assumptions **S1** (synchrony) and **S2** ($`\beta < 20\%`$). Then $`B`$ remains in the canonical chain at every later time.*

This is the formal statement of **property P_can (Block safety) from §4 of the overview**.

*Proof.* By Lemma 7 Part (b), under the same hypotheses (H7's *reveal-time and 40%-weight* conditions are not needed for P_can — those constrain only when the builder reveals; the *no-equivocation* condition that Lemma 7 Part (b) uses is supplied here as an explicit lemma hypothesis instead of being derived from H7's builder-side check), the slot-$`N{+}1`$ proposer's boost alone cannot outweigh $`B`$'s honest-attestation weight at slot $`N{+}1`$. Specifically: under timeliness + **S1**, every honest slot-$`N`$ validator receives $`B`$ by $`T_{\mathrm{att}}`$; under no-equivocation, no competing block at slot $`N`$ by $`B`$'s proposer is visible, so honest slot-$`N`$ validators vote uniquely for $`B`$; by **S2**, $`B`$'s honest weight at slot $`N{+}1`$ is $`\geq (1-\beta) W_N > 0.8 W_N`$, comfortably exceeding the worst-case competing weight of $`0.4 W_N + \beta W_N \leq 0.6 W_N`$ that boost + adversary can muster. The split-vote reorg vector is closed by H8 ($`\mathit{parent}(B)`$ stays canonical, so no sibling-of-$`\mathit{parent}(B)`$ branch competes); the late-prior-slot reorg vector is closed by H9 ($`\mathit{parent}(B).\mathit{slot} = N - 1`$, so no slot-$`(N{-}1)`$ sibling competes). Hence $`B`$ is the canonical head at slot $`N{+}1`$ at every honest node. Inductively, the same argument at every later slot $`N' \geq N + 1`$ — using the *same* H8/H9 strengthenings now applied at $`B`$'s descendant on the canonical chain rather than at $`B`$ — keeps $`B`$ on the canonical chain. ∎

*Remark on the no-equivocation hypothesis.* "Timely" (Definition 3c) is a property of a single block's broadcast time and does *not* preclude $`B`$'s proposer publishing a second timely block $`B''`$ at slot $`N`$. If the proposer equivocates and publishes both $`B`$ and $`B''`$ timely, honest slot-$`N`$ validators may split between them, breaking the $`(1-\beta) W_N`$ honest-weight bound for $`B`$ alone. The no-equivocation hypothesis is therefore load-bearing for P_can and must be stated explicitly. (In the P_rev-trustless case, the same no-equivocation fact is derived from H7 (i) — A1a's no-equivocation check by the cautious-reveal builder — under timeliness + S1; here, with no honest builder in scope, we name it as an independent hypothesis on the proposer's behaviour.)

*Remark.* P_can is the *Block safety* property: a timely, canonical-parent-extending, non-equivocating block stays canonical. It is the Category A counterpart of P_rev-trustless (which adds the FULL-on-chain claim from the PTC primary path); P_can only claims that the beacon block itself is not reorged. P_can does not require A1a's reveal-time or 40%-weight conditions — the builder's reveal behaviour is orthogonal to block safety; only the proposer's non-equivocation is needed.
...
*Proof.* At slot $`N + 2`$, `current_slot = N + 2`. Block $`B`$ has `slot(B) = N`, so `slot(B) + 1 = N + 1 ≠ N + 2 = current_slot`. The zero-return rule in Algorithm 5 does not apply, and `get_weight` computes the full attestation score for both `(r, FULL)` and `(r, EMPTY)`.

Consider the attesters from slot $`N + 1`$. Since no block was proposed in slot $`N + 1`$, these are non-same-slot attestations: their head is $`B`$ (from slot $`N`$), with `data.slot = N + 1 > slot(B) = N`. By Algorithm 4 Case 1, the same-slot exclusion does not fire, and the direct-vote branch attributes the vote by `payload_present`: True supports `(r, FULL)`, False supports `(r, EMPTY)`.
...
*Remark.* This lemma explains why `data.index` payload signaling exists for non-same-slot attestations (Section 1.2, Attester lines 11–12): it is the fallback mechanism for when PTC votes cannot influence the fork-choice because the immediately next slot was missed. Lemma 2 established that same-slot attesters are payload-neutral; this lemma shows that non-same-slot attesters fill the gap. The two mechanisms are complementary: PTC votes decide the payload status at the tiebreaker when the next slot is on time (Lemma 7), and attestation weight decides it when the next slot is missed (this lemma).

---

**Lemma 8b** (Payload execution deadline is the beginning of the next slot — formal proof of P_exec). *Let $`B`$ be a block proposed in slot $`N`$ and let $`P`$ be the payload associated with $`B`$ (i.e., `bid(B).block_hash = P.block_hash`). With the only possible exception of builders, the protocol does not require any other honest actor to complete the execution of $`P`$ before the beginning of slot $`N+1`$.*

This is the formal statement of **property P_exec (Payload execution deadline is the beginning of the next slot) from §4 of the overview**.
...
- **Slot-$`N`$ attesters** (Section 1.2, `attest` handler firing at $`T_{\mathrm{att}}`$ of slot $`N`$): by Definition 7 + Lemma H1, same-slot attesters set `data.index = 0` *unconditionally* — they take no position on payload status. Their `get_head` traversal reads only `store.blocks`, `store.block_states`, and `store.latest_messages` (Definition 8); none of these requires $`P`$ executed.
- **Slot-$`N`$ PTC members** (Section 1.2, `ptc_vote` handler firing at $`T_{\mathrm{ptc}}`$ of slot $`N`$): by Lemma H4, the honest PTC vote reads `has_execution_payload_envelope(store, root)` (whether the envelope was observed) and `is_data_available(beacon_block_root)` (whether the sampled blob columns arrived). Neither requires $`P`$ executed — the envelope check is a presence-of-object check, and `is_data_available` is a PeerDAS / KZG check independent of execution-layer state transition.
- **All other slot-$`\leq N`$ honest actors** (proposers and attesters at earlier slots): their duties are by hypothesis prior to $`B`$'s broadcast and therefore do not reference $`P`$ at all.

Hence no honest actor other than the builder is required to have executed $`P`$ by the end of slot $`N`$. The slot-$`N+1`$ proposer and slot-$`N+1`$ attesters, who *do* execute $`P`$ as part of their `process_block` for the slot-$`N+1`$ block, operate during slot $`N+1`$, not before it — so they are excluded from the lemma's scope. ∎

*Remark on the structural mechanism.* The lemma is a consequence of the two-phase ePBS design: the consensus-layer state transition (`process_block`) operates on the bid hash (`bid(B).block_hash`) and does not invoke `verify_execution_payload_envelope` for $`P`$. Algorithm 8 (`on_execution_payload_envelope`) is the path that locally verifies $`P`$ and imports its envelope into the fork-choice store, while the state-changing execution effects are applied later by `apply_parent_execution_payload` when a successor declares the parent FULL. The structural availability of FULL and EMPTY children in the fork-choice tree at every slot (Algorithm 2 + Lemma 1) ensures that fork-choice *also* advances past slot $`N`$ regardless of whether $`P`$ has been imported into `store.payloads` — this is the older "consensus progress" framing of P_exec; the current overview P_exec sharpens it to "no execution deadline before slot $`N+1`$".

*Remark on missed slot $`N+1`$.* If slot $`N+1`$ is missed, no actor at slot $`N+1`$ exists to execute $`P`$; Lemma 8 supplies the FULL/EMPTY resolution via slot-$`N+1`$ committee attestations at slot $`N+2`$, and the slot-$`N+2`$ proposer (or its descendants) eventually execute $`P`$ as part of processing a successor block. The "deadline is the beginning of slot $`N+1`$" property is unaffected: it constrains what honest actors at slot $`\leq N`$ are required to do, not when downstream execution must complete.
...
**Lemma 9** (Path A: builder reveals implies payment in next block). *If an honest builder reveals a valid execution payload via the cautious strategy (Assumption H7) for a block at slot N with bid.value > 0, and a subsequent block at slot M > N declares parentStatus = FULL for block N, then `process_parent_execution_payload` (Helper 12c) settles the builder payment via `settle_builder_payment` (Helper 12a), queuing a BuilderPendingWithdrawal for the proposer and clearing the pending payment entry. Furthermore, under H7, S3 plus the local-processing side condition L_verify, the strengthening hypotheses H8 ($`\mathit{parent}(B)`$ stays canonical) and H9 ($`\mathit{parent}(B).\mathit{slot} = N - 1`$), and assuming an honest slot-N+1 proposer, M = N + 1 — the FULL declaration happens at the immediately next slot.*

This is the "happy path": the builder delivered cautiously, and the next block applies the parent's execution effects, which includes settling the payment. H7 is what makes Path A *fire* (not just *would fire if some block declared FULL*) — together with S3 and L_verify it ensures that the slot-N+1 honest proposer's `get_head` selects `(r, FULL)`, that `should_build_on_full(store, head)` returns True because neither negative timeliness nor negative DA predicate holds, and that the block survives the relevant reorg attempts (Lemma 7, Part (b), with H8/H9).
...
For the second claim (M = N + 1), assume the slot-N+1 proposer is honest. By H7, B has ≥ 40% real attestation weight at payload-broadcast time. By Lemma 7 Part (b), with H8/H9 closing the split-vote and missing-slot reorg vectors, no slot-N+1 boost-only competitor displaces B. By S3 and L_verify, the evaluating honest proposer has `r ∈ store.payloads` and the honest PTC majority votes True on both payload timeliness and blob-data availability. Lemma 14c then gives both halves needed here: `should_extend_payload(store, r)` returns True through the PTC primary path, and neither `payload_data_availability(store, r, available=False)` nor `payload_timeliness(store, r, timely=False)` holds. The honest slot-N+1 proposer therefore runs `get_head`, obtains `head = (r, FULL)`, and evaluates `should_build_on_full(store, head)` (Algorithm 6b). The EMPTY guard does not fire, the head is from the immediately previous slot, and both negative checks return False, so `should_build_on_full` returns True. The proposer's `propose` handler (Section 1.2) then sets the selected bid's `parent_block_hash` to `bid(B).block_hash` and sets `body.parent_execution_requests = store.payloads[r].execution_requests`. The slot-N+1 block thus declares `parentStatus = FULL` for B, and the proof above applies with M = N + 1. ∎
...
**Lemma 11** (Unconditional payment — formal proof of P_pay). *Let $`B`$ be a block at slot $`N`$ with `bid.value > 0`, proposed in a timely fashion (the slot-$`N`$ proposer publishes $`B`$ early in slot $`N`$, without intentional delay) with **$`B`$'s slot-$`N`$ proposer not equivocating** (no second block at slot $`N`$ signed by $`B.\mathit{proposer\_index}`$ is broadcast), and satisfying both strengthening hypotheses **H8** ($`\mathit{parent}(B)`$ remains canonical at every honest validator throughout the trace) and **H9** ($`\mathit{parent}(B).\mathit{slot} = N - 1`$). Assume **S1** (synchrony $`\Delta < T_{\mathrm{att}}`$), **S2** ($`\beta < 20\%`$), P_pay's **attestation-inclusion hypothesis** from §4 of the overview (all attestations cast by honest validators at slot $`N`$ on $`B`$ are included on the canonical chain before the start of epoch $`\mathsf{epoch}(N)+2`$), and **L_withdraw**. Then the proposer's payment is **eventually** queued — via Path A (next-block FULL declaration) or via Path B (the first canonical block at slot $`\geq`$ start of epoch $`\mathsf{epoch}(N)+2`$ runs `process_epoch` and the 60% quorum is met) — and L_withdraw eventually turns that queued withdrawal into an execution-layer credit to the proposer's `fee_recipient`, regardless of whether the builder reveals the execution payload. **This lemma is the formal proof of P_pay (Unconditional payment to the proposer) from §4 of the overview, supplying the "settlement happens" guarantee. Combined with Lemma 12 (single payment), it delivers the full P_pay claim.** Lemma 12b supplies the solvency precondition that the bid is admittable only if the balance covers it.* The hypothesis `bid.value > 0` automatically excludes self-builds and non-trustless bids (`bid.execution_payment > 0, bid.value = 0`), where Helper 8 lines 17–26 are gated on `amount > 0` and no IOU is recorded — see also Lemma 12b's non-self-build scoping. The lemma is therefore vacuous for self-builds and for non-trustless bids; its content is the unconditional-payment guarantee for trustless external-builder bids only (Category C in the overview's property catalogue).

*Role of H8 and H9 in this lemma.* Both strengthenings are required: together with **S1**, **S2**, and the "timely publication" hypothesis, they let the proof derive the 60% same-slot quorum on $`B`$ that Path B presupposes (and ensure $`B`$ remains canonical so Path A can fire). The 60% is **not** assumed — it is a conclusion of the proof. **H8** closes the **split-vote-at-slot-$`N`$** gap: without H8, a Byzantine slot-$`N`$ proposer publishing $`B`$ at $`t > T_{\mathrm{att}} - \Delta`$ can fragment honest slot-$`N`$ attestations between $`B`$ and a sibling branch of $`\mathit{parent}(B)`$; if that sibling branch outweighs $`\mathit{parent}(B)`$ at slot $`N{+}1`$, $`B`$ is reorged and its IOU goes off-chain with parent($`B`$). **H9** closes the **late-prior-slot** gap: without H9, a Byzantine slot-$`N{-}1`$ proposer can withhold then late-release a sibling $`B'`$ at slot $`N{-}1`$ parented at the same ancestor as $`B`$, splitting honest slot-$`N`$ attesters between $`B`$ ($`\sim 40\%`$ under tuned delivery) and $`B'`$ ($`\sim 60\%`$). At slot $`N{+}1`$ `proposer_boost_root` resets and $`B'`$ wins fork-choice, reorging $`B`$; meanwhile $`B`$'s `payment.weight` is capped at $`\sim 40\%`$, below the 60% Path B quorum. Neither attack class is independently closed by the other: H8 fixes the parent-level competitor; H9 fixes the sibling-of-$`B`$ competitor. Both fragmentation patterns reduce $`B`$'s slot-$`N`$ honest support below 60%, breaking the derivation in Case 2 below; only with both strengthenings does $`B`$ collect $`(1-\beta) W_N > 0.8 W_N`$ near-unanimous honest weight.
...
**Case 1: A subsequent block declares FULL (Path A fires).** Case 1 is reached when the additional preconditions **H7** (builder reveals via cautious strategy), **S3** (honest-online PTC majority), **L_verify** (honest nodes complete local payload/DA/EL processing before the slot-$`N{+}1`$ decision), and an honest slot-$`N{+}1`$ proposer all hold; if any of these fails, no FULL declaration lands at slot $`N{+}1`$ and the proof falls through to Case 2 (which is sound under Lemma 11's listed hypotheses alone). Under **S1** (every honest slot-$`N`$ validator receives $`B`$ by $`T_{\mathrm{att}}`$ from the lemma's "timely" hypothesis), the lemma's explicit **no slot-$`N`$ proposer equivocation** hypothesis (equivalently, H7 condition (iii) in the honest-builder subcase), **H8** ($`\mathit{parent}(B)`$ stays canonical, so no sibling-of-$`\mathit{parent}(B)`$ pulls honest attesters off $`B`$'s branch), and **H9** (no missing slot at $`N{-}1`$, so no late-released sibling $`B'`$ competes with $`B`$ under a shared ancestor), every honest slot-$`N`$ attester runs fork-choice with $`B`$ as the unique extension of $`\mathit{parent}(B)`$ and votes for $`B`$. By **S2**, the honest weight on $`B`$ at slot $`N{+}1`$ is at least $`(1-\beta) W_N > 0.8 W_N`$. The slot-$`N{+}1`$ proposer's `get_head` walks through $`\mathit{parent}(B)`$ and selects $`B`$. By **S3** and **L_verify**, Lemma 14c gives the PTC primary path for `should_extend_payload(store, r)` and rules out both negative predicates in `should_build_on_full`; the proposer declares `parentStatus = FULL` for $`B`$. By Lemma 9 (first claim, conditional on FULL declaration), `process_parent_execution_payload` invokes `settle_builder_payment`, which queues the payment and clears the pending entry. The proposer gets paid in the slot-$`N{+}1`$ block.

**Case 2: No subsequent block declares FULL (Path A does not fire).** This includes three sub-cases: (a) the builder withholds entirely, (b) the builder reveals an invalid execution payload (rejected by `verify_execution_payload_envelope`, so `store.payloads` is never populated), or (c) the builder reveals a valid execution payload, but no subsequent block builds on B-FULL — including the PTC-negative EMPTY path analyzed in Lemma 17. In all three sub-cases, `settle_builder_payment` is never called, so the `BuilderPendingPayment` persists. Same-slot attesters accumulate `payment.weight` via `process_attestation` (Helper 15). By the same argument established in Case 1 — under S1, S2, **no slot-$`N`$ proposer equivocation** (the explicit lemma hypothesis; note H7 itself is *not* a hypothesis of this lemma, since Case 2 sub-cases (a)–(c) need not feature an honest builder), H8, H9, and "timely" publication — every honest slot-$`N`$ attester votes for $`B`$, contributing their effective balance to $`B`$'s `payment.weight` *once their attestation is included on-chain*. By the **attestation-inclusion hypothesis**, all such honest attestations are included on the canonical chain before the start of epoch $`\mathsf{epoch}(N)+2`$, so by Helper 15 their effective balances accumulate into `payment.weight`. The accumulated weight is at least $`(1-\beta) W_N > 0.8 W_N > 0.6 W_N`$, meeting the 60% quorum threshold. The first canonical block whose processing crosses the epoch-$`\mathsf{epoch}(N)+1`$ boundary (such a block exists eventually under standard Gasper liveness — itself implied by S1 + S2 + an honest proposer schedule, the same setting the inclusion hypothesis implicitly assumes) invokes `process_slots`, which runs `process_epoch`, which calls `process_builder_pending_payments`. By Lemma 10, the quorum check passes and the `BuilderPendingWithdrawal` is appended at that point. The proposer is eventually paid.

In both cases, the proposer receives a `BuilderPendingWithdrawal` with amount `bid.value`. Under **L_withdraw**, later canonical FULL blocks drain `builder_pending_withdrawals`; `apply_withdrawals` (Helper 21) debits the builder balance, and the accepted execution payload for the draining block credits the proposer's `fee_recipient` at the execution layer. ∎
...
*Remark on Path C (the third settlement path).* There is a **third settlement path**, called **Path C** in the overview's §7, hidden inside `apply_parent_execution_payload` (Helper 12b, lines 19–26): it appends a `BuilderPendingWithdrawal` directly when `parent_epoch < get_previous_epoch(state)`, bypassing both `settle_builder_payment` and the epoch-boundary quorum check. One might worry that Path C could double-fire with Path B, queuing a second withdrawal. It cannot, due to a mutual exclusion: Path C requires `state.latest_execution_payload_bid` to still point to a bid more than one epoch old, which by Helper 8 line 27 (the unconditional cache update on every `process_block`) means no intervening canonical block was processed. But Path B's quorum-met condition (`payment.weight ≥ quorum`) requires same-slot attestations to slot N's block to be **included on-chain** via subsequent blocks' `process_attestation`. With no intervening blocks, no attestation inclusions occur, `payment.weight` stays at 0, and Path B does not fire. Conversely, if any intervening block was processed, `state.latest_execution_payload_bid` is no longer slot N's bid, so Path C cannot fire for slot N's entry. The three paths (A, B, C) are reachable from disjoint chain histories; the proposer is paid at most once.
...
This is the solvency precondition for P_pay (Unconditional payment to the proposer) from §4 of the overview: a builder cannot commit to a bid it cannot pay, even across concurrent slots. The check is what makes Lemma 11 (unconditional payment) economically sound: when the proposer is paid, the builder's balance is guaranteed to cover the payment. The overview treats this as a remark in §5 Phase 1b rather than as a standalone property. Note that `can_builder_cover_bid` accounts only for trustless `value` obligations; `execution_payment` (non-trustless) is not in the sum — see Helper 6 and overview §3 for the rationale.
...
3. *(IOUs are paid before the exit sweep.) The 2-epoch payment window is strictly shorter than $`\texttt{MIN\_BUILDER\_WITHDRAWABILITY\_DELAY} = 8192`$ epochs. Therefore, by epoch $`e + 2`$, every entry of $`\mathcal{P}_i`$ that resolves into an IOU has already been written to `state.builder_pending_withdrawals`. From epoch $`e+2`$ through epoch $`e+8191`$, `process_withdrawals` (Helper 13) drains those IOUs via `get_builder_withdrawals`, which runs strictly before `get_builders_sweep_withdrawals` in `get_expected_withdrawals` (`beacon-chain.md`). Each IOU debits builder $`i`$'s balance via `apply_withdrawals` (Helper 21) using `min(withdrawal.amount, builder_balance)`. From epoch $`e + \texttt{MIN\_BUILDER\_WITHDRAWABILITY\_DELAY} = e + 8192`$ onward, `get_builders_sweep_withdrawals` becomes eligible to fire (the guard `builder.withdrawable_epoch \leq epoch` succeeds) and the residual balance is withdrawn to `builder.execution_address`.*
...
This formalises the interaction between the builder lifecycle (Helpers 26–28) and the unconditional payment mechanism (Lemmas 9–11). The spec relies on a *temporal separation* — the 2-epoch payment window vs. the 8192-epoch withdrawability delay — rather than an explicit inhibition of `initiate_builder_exit` while obligations are pending. The temporal separation is sufficient because the payment window is 4096× shorter than the withdrawability delay.
...
*Item 3.* `MIN_BUILDER_WITHDRAWABILITY_DELAY = 2^{13} = 8192` (`beacon-chain.md` line 205), which is strictly greater than 2.[^lem12c-constants] The `get_builders_sweep_withdrawals` guard at `beacon-chain.md` line 1255 checks `builder.withdrawable_epoch <= epoch and builder.balance > 0`; since `withdrawable_epoch = e + 8192`, the guard remains False through epoch $`e + 8191`$ and only succeeds from epoch $`e + 8192`$. The ordering of `get_builder_withdrawals` (which drains the IOU queue) before `get_builders_sweep_withdrawals` (which sweeps the residual) inside `get_expected_withdrawals` is a fixed property of the spec function (`beacon-chain.md`).

*Item 4.* Lemma 12b establishes the invariant `balance ≥ MIN_DEPOSIT_AMOUNT + pending` at every bid acceptance for builder $`i`$. Item 1 implies no new bids increase `pending` after epoch $`e`$. The set of obligations therefore monotonically shrinks (as IOUs are paid and discharged) over $`[e, e+8192]`$. At the moment each IOU is presented to `apply_withdrawals`, the inequality `builder_balance ≥ withdrawal.amount` holds — provided no slashing, `seq_payment_underflow`, or out-of-band balance change has occurred (the standard caveat noted in Lemma 15's proof and Helper 6's commentary). The `min(amount, balance)` clause in `apply_withdrawals` line 1325 is the spec's defensive fallback for those edge cases. ∎
...
The properties proved so far assume that the relevant actors follow the protocol. In practice, any subset of validators, PTC members, builders, or proposers may be Byzantine — deviating arbitrarily from the honest behavior specified in Section 1.2. This section establishes what the protocol guarantees *despite* such deviations. The key insight is that different actors have different attack surfaces, and the protocol's layered design limits the damage each can cause.

The interaction between the PTC and the next-slot proposer is central to understanding the adversarial landscape. The FULL/EMPTY tiebreaker for the immediately previous slot (Definition 13) is resolved by `should_extend_payload` (Algorithm 6), which first checks the hard guard (`is_payload_verified`, line 2), then the PTC primary path (line 7: True-timeliness quorum AND True-DA quorum), and then falls through to proposer-based fallbacks (lines 8–10). Independently, an honest slot-N+1 proposer additionally consults `should_build_on_full` (Algorithm 6b) before constructing its block: a negative quorum on either timeliness or blob-data availability forces the honest proposer to declare `parentStatus = EMPTY` for an immediately previous-slot FULL head. The following table provides a comprehensive analysis of the outcome-relevant combinations. The first eight rows cover the case where the builder **delivered** a valid execution payload (`r ∈ store.payloads`) — split by the PTC's two-dimensional vote on `payload_present` (timeliness) and `blob_data_available` (DA) — and the last four cover the case where the builder **withheld** (`r ∉ store.payloads`).
...
| False-timeliness quorum              | True-DA quorum                   | Honest (builds on EMPTY)   | **EMPTY** | `should_build_on_full` returns False on the negative timeliness predicate                                  |
| False-timeliness quorum              | True-DA quorum                   | Malicious (declares EMPTY) | **EMPTY** | Primary path and all fallbacks fail                                                                          |
| False-timeliness quorum              | True-DA quorum                   | Malicious (declares FULL)  | FULL            | The FULL-declaring successor passes the parent-FULL assertion under this table's premise; fallback (c) fires |
| any timeliness                       | **False-DA quorum**        | Honest (builds on EMPTY)   | **EMPTY** | `should_build_on_full` returns False on the negative DA predicate                                          |
| any timeliness                       | **False-DA quorum**        | Malicious (declares EMPTY) | **EMPTY** | Primary path and all fallbacks fail                                                                          |
| any timeliness                       | **False-DA quorum**        | Malicious (declares FULL)  | FULL            | The FULL-declaring successor passes the parent-FULL assertion under this table's premise; fallback (c) fires |
...
1. **The PTC's authority is bidirectional.** Through the PTC primary path of `should_extend_payload`, an honest PTC majority overrides a malicious proposer's false EMPTY declaration. Through `should_build_on_full`, a negative quorum on either signal overrides an honest proposer's local view and forces EMPTY — even when the proposer's `is_payload_verified` passes.
2. **A malicious PTC majority can force EMPTY against an honestly delivered payload without proposer collusion.** For an immediately previous-slot FULL head, `should_build_on_full` strips an honest proposer's discretion when either `payload_present = False` or `blob_data_available = False` has quorum. Under **S3** (honest PTC majority), neither negative quorum arises against an honest reveal, so honest builders are protected.
...
4. **`store.payloads` is the hard gate; PTC is the soft signal.** Branch existence is controlled by local payload verification (`store.payloads`), which includes local blob-data availability plus execution-engine acceptance. The PTC influences the tiebreaker (via `should_extend_payload`) and the proposer's construction step (via `should_build_on_full`), but never creates branches. This layered design separates structural integrity of the fork-choice tree from the PTC's role as DA witness.
5. **Local verification vs PTC aggregate observations.** The hard guard `is_payload_verified` reflects the local node's payload, blob-data, and execution-engine verification path inside `on_execution_payload_envelope`. The PTC's positive and negative predicates reflect the aggregate previous-slot observations of payload arrival and blob-data availability. Under asymmetric delivery, the two can disagree; `should_build_on_full` resolves honest construction in favor of the negative PTC aggregate for the immediately previous slot.
...
**Lemma 14** (True/True PTC primary path overrides a false EMPTY declaration). *Let B be a block with root r at slot N. Assume: (1) the builder reveals a valid execution payload, so that `r ∈ store.payloads`; (2) a True-timeliness predicate and a True-DA predicate both hold for r. Then `should_extend_payload(store, r)` returns True, regardless of the slot-N+1 proposer's declaration. In the previous-slot FULL/EMPTY tiebreaker, `(r, FULL)` beats `(r, EMPTY)`.*

This is the positive-authority side of the PTC. If the PTC majority reports both payload arrival and blob-data availability, a malicious slot-N+1 proposer declaring EMPTY cannot make fork choice prefer EMPTY.

*Proof.* The hard guard at Algorithm 6 line 2 passes because `r ∈ store.payloads`. By hypothesis (2), both `payload_timeliness(store, r, timely=True)` and `payload_data_availability(store, r, available=True)` return True. Therefore the PTC primary-path conjunct in Algorithm 6 returns True. The fallback conditions are irrelevant because the disjunction has already succeeded.

By Definition 13, for the immediately previous slot the EMPTY tiebreaker is 1, while the FULL tiebreaker is 2 when `should_extend_payload` returns True. Since the two nodes have the same root and the zero-return rule makes their weight equal, `get_head` selects `(r, FULL)`. ∎

---

**Lemma 14b** (Negative PTC quorum forces honest construction on EMPTY). *Let B be a block with root r at slot N. Assume: (1) the builder reveals a valid execution payload, so that `r ∈ store.payloads`; (2) at least one negative predicate holds for r — either `payload_timeliness(store, r, timely=False)` or `payload_data_availability(store, r, available=False)`; (3) the slot-N+1 proposer is honest and `get_head(store)` returns `(r, FULL)` at construction time. Then the slot-N+1 block declares `parentStatus = EMPTY` for B.*

The negative predicate overrides the honest proposer's construction choice through `should_build_on_full` (Algorithm 6b). This is an honest-construction rule, not an admission rule: a malicious proposer can still declare FULL if its block passes the parent-FULL assertion at the receiving node.

*Proof.* The honest proposer evaluates `should_build_on_full(store, head=(r, FULL))`. The head is not PENDING and not EMPTY, so the first two guards do not return. Since the head is from the immediately previous slot, the older-resolved-head branch does not return. By hypothesis (2), one of the two negative checks in Algorithm 6b returns True:

- if `payload_data_availability(store, r, available=False)` is True, the DA-side check returns False for `should_build_on_full`;
- if `payload_timeliness(store, r, timely=False)` is True, the timeliness-side check returns False for `should_build_on_full`.

Thus `should_build_on_full` returns False. In the proposer's `propose` handler, the selected bid's `parent_block_hash` is set to `state.latest_execution_payload_bid.parent_block_hash`, `parent_execution_requests` is set to `ExecutionRequests()`, and self-build payload construction uses the EMPTY parent state. The resulting block declares `parentStatus = EMPTY` for B. ∎

*Remark.* Under **S3** (honest PTC majority with honest members waiting for both payload and blob-data observations), neither negative predicate arises against an honestly delivered payload. Outside S3, a malicious PTC majority can force an honest successor to build EMPTY without proposer collusion. Conversely, because `should_build_on_full` is not an `on_block` admission rule, a malicious successor may declare FULL despite a negative PTC quorum if the parent-FULL assertion passes locally.

---

**Lemma 14c** (Honest PTC majority excludes negative quorums). *Let B be a block with root r at slot N, let an honest node evaluate the PTC helpers with `r ∈ store.payloads`, and assume the standing β < 0.5 per-PTC bound holds at slot N (i.e., the honest PTC fraction strictly exceeds `PAYLOAD_TIMELY_THRESHOLD / PTC_SIZE = 256 / 512`). Then, if every honest PTC member assigned to slot N (i) observes a `SignedExecutionPayloadEnvelope` for B strictly before `t_payload_due` and (ii) locally observes the blob columns it sampled to be available (passing `is_data_available`), then at that evaluating node:*

*(i) `payload_timeliness(store, r, timely=False)` returns False — no False-timeliness quorum exists;*

*(ii) `payload_data_availability(store, r, available=False)` returns False — no negative DA quorum exists.*

*Equivalently, because the evaluating node has `r ∈ store.payloads`, both `payload_timeliness(store, r, timely=True)` and `payload_data_availability(store, r, available=True)` return True there.*

*Proof.* By Lemma H4, an honest PTC member that observes the envelope before `t_payload_due` votes `payload_present = True`; one that locally observes its sampled blob columns votes `blob_data_available = True`. Under the hypothesis, every honest PTC member votes True on both signals. The honest fraction strictly exceeds `PAYLOAD_TIMELY_THRESHOLD / PTC_SIZE`, so at most `PTC_SIZE - (PAYLOAD_TIMELY_THRESHOLD + 1) = PTC_SIZE - 257 = 255` PTC members are Byzantine and may vote False on either signal. The local-verification gate in Helpers 4 and 5 passes at the evaluating node because `r ∈ store.payloads`. Helper 4 checks `sum(payload_timeliness_vote[r] is False) > PAYLOAD_TIMELY_THRESHOLD = 256`: at most 255 such votes exist, so the count is at most 255, which is not strictly greater than 256. Hence `payload_timeliness(store, r, timely=False)` returns False. The same arithmetic applies to Helper 5: at most 255 False votes on `blob_data_available`, below `DATA_AVAILABILITY_TIMELY_THRESHOLD = 256`. Hence `payload_data_availability(store, r, available=False)` returns False. Conversely, at least `PTC_SIZE - 255 = 257` True votes exist on each signal, strictly exceeding the threshold of 256, so `payload_timeliness(store, r, timely=True)` and `payload_data_availability(store, r, available=True)` both return True at any node whose local-verification gate passes. ∎

*Remark.* Lemma 14c is a structural counting fact used at two distinct sites: (i) Lemma 7 Part (a), where `should_extend_payload`'s PTC primary path fires because the local-verification gate passes and both True predicates hold; and (ii) Lemma 9, where `should_build_on_full(store, head=(r, FULL))` returns True because neither negative timeliness nor negative DA predicate holds. Lemma 14b's contrapositive is the same point in attack form: a negative quorum against an honestly delivered payload requires S3, local delivery, or local processing to fail.
...
**Lemma 16** (No charge below the payment quorum — P_withhold trustless core). *For a trustless bid (`bid.value > 0`) with a canonical `BuilderPendingPayment` entry, if no FULL settlement path has already cleared the entry and the entry reaches the relevant `process_builder_pending_payments` check with `payment.weight < get_builder_payment_quorum_threshold(state)`, then no `BuilderPendingWithdrawal` is queued for that bid and the builder is not debited by this slot's trustless payment.*

The 60% quorum threshold (Helper 18) is the mechanical protection used by P_withhold. The lemma is intentionally narrow: it proves the state-machine consequence of a below-quorum entry. The P_withhold subcases below explain when the overview's withholding hypotheses imply that below-quorum premise, and when they instead remove or clear the IOU by another path.

*Proof.* By the lemma premise, the entry has not been cleared by `settle_builder_payment` and is still present when `process_builder_pending_payments` (Helper 14) checks it. Helper 14 queues a withdrawal only under the branch `payment.weight >= quorum`. Since `payment.weight < quorum`, that branch is skipped: no `BuilderPendingWithdrawal` is appended. The entry is then discarded during the epoch shift (Helper 14, lines 6–8). With no queued withdrawal for this bid, `process_withdrawals` / `apply_withdrawals` has no object through which to debit the builder for this slot. ∎

*Remark on coverage of P_withhold (case-split mirroring overview §9.3).* P_withhold's trustless case splits into three sub-cases, mirroring the overview's §9.3 P_withhold proof structure:

- **Case 1 — no-weight withhold (A1a (ii) failed).** The honest builder withholds because strictly less than 40% real attestation weight is observed at reveal time, and no proposer equivocation is visible. Because Helper 15 accumulates `payment.weight` by slot rather than by attested head, the support-for-$`B`$ observation alone does not bound `payment.weight`. The needed bridge is **I_payweight**: in this subcase, the total online honest same-slot contribution that can land before the payment check is `< 40%` of the spec denominator $`W`$. By **S2**, Byzantine contribution is `< 20%` of $`W`$. Thus `payment.weight < 60%` of $`W`$, below the quorum threshold, and Lemma 16 discards the entry without a charge.
- **Case 2a — equivocation-driven withhold; equivocation reorgs $`B`$ out and no canonical IOU remains (A1a (iii) failed).** The builder withholds because proposer equivocation is visible, and the branch containing $`B`$ does not survive on the canonical chain. The IOU created while processing $`B`$ is noncanonical state. Since the canonical state never contains that bid's `BuilderPendingPayment`, there is no canonical entry to queue or debit.
- **Case 2b — equivocation-driven withhold; a canonical IOU remains (A1a (iii) failed).** The original block may still have enough same-slot support for the quorum argument to fail. The slashing-inclusion path discharges the IOU instead: **I_slash** supplies canonical inclusion and processing of the valid `ProposerSlashing` before the relevant pending-payment boundary check. Lemma 22 part (iv) then applies `process_proposer_slashing` (Helper 22), clearing the `BuilderPendingPayment` entry entirely and overriding whatever weight had accumulated.

Together, Lemma 16 (Case 1), noncanonicity of the IOU (Case 2a), and Lemma 22 part (iv) under I_slash (Case 2b) cover P_withhold's trustless A1a-driven withhold causes. The non-trustless case is degenerate: Helper 8 creates no IOU when `bid.value = 0`, so no protocol charging path exists for `bid.execution_payment > 0, bid.value = 0`.
...
**Lemma 17** (PTC-negative EMPTY path — bounded damage outside S3). *Let B be a block with root r at slot N. Assume: (1) a valid execution payload for B has been revealed and locally verified at the relevant honest nodes, so `r ∈ store.payloads`; (2) at least one negative PTC predicate holds for r — either `payload_timeliness(store, r, timely=False)` or `payload_data_availability(store, r, available=False)`; and (3) the slot-N+1 canonical successor C is a child of B that declares `parentStatus = EMPTY` for B. Condition (3) can arise honestly via Lemma 14b, because `should_build_on_full` forces an honest proposer to build EMPTY when a negative predicate holds, or maliciously by a proposer that declares EMPTY. Then:*

- *(i) After C is received, the immediately previous-slot FULL/EMPTY tiebreaker returns `(r, EMPTY)` at B's branch.*
...
*Note on hypothesis realisability.* With `r ∈ store.payloads`, a negative predicate requires a strict majority of PTC positions to vote False on the corresponding signal. Under the document's standing β < 0.2 per-committee bound, Byzantine PTC voting alone cannot produce such a majority. The lemma therefore describes the bounded outcome outside S3: for example, a locally high-β PTC sample, network delay that prevents enough honest PTC members from observing the envelope before `t_payload_due`, or asymmetric blob-data delivery that prevents enough honest PTC members from passing their sampled DA checks. Under S3 plus L_verify, Lemma 14c excludes the negative predicates.

This is the spec's negative-PTC path. A negative timeliness or DA quorum is enough to make an honest proposer build EMPTY for an immediately previous-slot FULL head. A malicious EMPTY declaration is also a possible route to the same on-chain outcome.

*Proof.*

*Part (i):* The hard guard at Algorithm 6 passes because `r ∈ store.payloads`. By assumption (2), at least one of the two positive predicates needed by the PTC primary path cannot hold: a strict False-timeliness quorum is incompatible with a strict True-timeliness quorum, and a strict False-DA quorum is incompatible with a strict True-DA quorum. Therefore the PTC primary path fails. By assumption (3), a block C exists in slot N+1 (`proposer_boost_root ≠ Root()`), it is a child of B (`C.parent_root == r`), and it declares `parentStatus = EMPTY` (`is_parent_node_full(store, C)` returns False). Fallback (a) fails because a proposer-boost root exists, fallback (b) fails because C is a child of B, and fallback (c) fails because C does not declare FULL. All conditions in `should_extend_payload` are False. By Definition 13, the tiebreaker for FULL is 0 while the tiebreaker for EMPTY is 1, so `get_head` selects `(r, EMPTY)` at B's branch.

*Part (ii):* We characterise what the protocol guarantees once the EMPTY successor has landed at slot N+1. The honest framing is **damage bounded, not damage recovered**: the protocol does not enforce that the FULL branch will be re-selected by honest proposers, but it does guarantee that the FULL branch remains structurally available, that no honest validator is slashed, and that the slot-N proposer is still paid.

**Slot N+1 state.** `get_head` returned (r, EMPTY) → (C, PENDING), where C is the colluding proposer's block built on B-EMPTY. Honest attesters at slot N+1 run `get_head` at $`T_{\text{att}}`$, see C as the head, and cast a same-slot attestation for C with `data.index = 0` (Lemma H1: same-slot voters carry no payload opinion). The vote's direct contribution to fork-choice weight is via Case 1 to $`(C.\mathit{root}, \mathrm{PENDING})`$; via Case 2 of the vote-support relation, `get_ancestor((C.root, PENDING), slot(B))` reads `parentStatus(C) = EMPTY` (from C's bid commitment, by Definition 4) and attributes the vote to $`(r, \mathrm{EMPTY})`$ at the ancestor level — a *structural* attribution from the chain's parentStatus declarations, not from the voter's payload opinion.
...
- **Proposer payment is unaffected.** The slot-N proposer is paid by one of two mutually exclusive paths. If a later block (at any slot $`\geq N + 2`$) eventually declares FULL for B, Path A fires (Lemma 9). Otherwise — including the case where the EMPTY chain remains canonical indefinitely — Path B fires at the next epoch boundary, provided slot N's beacon block reached the 60% quorum (Lemma 20 case (a)). The negative-PTC outcome does not suppress either payment path.
...
*Remark on the damage profile.* The damage from a PTC-negative EMPTY path at slot N is:
...
- **Proposer payment delivered.** Path B settles the payment if the slot-N beacon block reached the quorum (Lemma 20 case (a)); the negative PTC path cannot suppress this.
...
The negative PTC path therefore does not extract economic advantage at the proposer's expense; it produces an EMPTY decision at slot N+1 and may force the canonical chain onto the EMPTY branch. Whether this matters in practice depends on the application (e.g., whether the slot-N execution payload contained transactions that downstream applications expected to be canonical).
...
**Lemma 18a** (Chain validity — formal proof of P_valid). *Under S2's honest-supermajority validity setting, every post-Gloas non-genesis block $`C`$ on the canonical chain satisfies*

$$
\mathit{bid}(C).\mathit{parent\_block\_hash} \in \{\mathit{bid}(\mathit{parent}(C)).\mathit{block\_hash},\; \mathit{bid}(\mathit{parent}(C)).\mathit{parent\_block\_hash}\}.
$$

*Proof.* Let $`B = \mathit{parent}(C)`$. When an honest node processes $`C`$, `process_parent_execution_payload` (Helper 12c) runs before `process_execution_payload_bid` (Helper 8). At that moment `state.latest_execution_payload_bid` is `bid(B)`, cached when $`B`$ was processed. If $`C`$ declares B FULL, Helper 12c applies B's execution-payload effects and Helper 12b sets `state.latest_block_hash = bid(B).block_hash`. If $`C`$ declares B EMPTY, Helper 12c returns through the empty-parent branch and leaves `state.latest_block_hash` at the execution tip inherited by B, which Helper 8's admission of B made equal to `bid(B).parent_block_hash`. Helper 8 then asserts `bid(C).parent_block_hash == state.latest_block_hash`. Therefore any accepted block C uses one of the two hashes in B's bid. A block using a third hash fails Helper 8 and cannot be admitted by honest nodes; under S2, blocks rejected by the honest supermajority cannot become canonical. ∎

---

**Lemma 18** (Honest construction parent-hash consistency). *If an honest builder or proposer constructs a bid for a block whose parent head is `head`, then the constructed or accepted bid uses the parent execution hash selected by `should_build_on_full(store, head)`:*

- *if `should_build_on_full(store, head)` returns True, the bid uses `state.latest_execution_payload_bid.block_hash`;*
- *if it returns False, the bid uses `state.latest_execution_payload_bid.parent_block_hash`.*

*In particular, for an immediately previous-slot head `(B.root, FULL)`, local verification of B's payload is not by itself enough to make honest construction declare B FULL; a negative timeliness or DA predicate makes `should_build_on_full` return False, and honest construction uses B's EMPTY parent hash.*

*Proof.* The proposer path is explicit in the `propose` handler (Section 1.2): it computes `build_on_full = should_build_on_full(store, head)`, then sets `expected_parent_hash` to `state.latest_execution_payload_bid.block_hash` when `build_on_full` is True and to `state.latest_execution_payload_bid.parent_block_hash` otherwise. External bids are accepted only if their `parent_block_hash` equals this expected value; self-build bids are constructed with the same value through `prepare_execution_payload`.

The builder path follows the same branch in `submit_bid` (Section 1.2). The builder runs `head = get_head(store)`, reads `parent_bid = state.latest_execution_payload_bid`, and calls `should_build_on_full(store, head)`. If the helper returns True, the builder copies the state, simulates `apply_parent_execution_payload` on that copy, and uses `parent_bid.block_hash` as the payload parent. If the helper returns False, it skips the simulation and uses `parent_bid.parent_block_hash`. The later `process_block` execution agrees with this local simulation because `process_parent_execution_payload` applies the same FULL/EMPTY branch before Helper 8 checks the new bid. ∎

---

**Lemma 18b** (Data availability for chain inclusion — formal proof of P_DA). *Let $`h`$ be in the payload hash chain of the canonical beacon chain (Definition 4c). Under S2's honest-supermajority validity setting, there exists a canonical non-head block $`B^*`$ and an honest node such that:*

- *`bid(B*).block_hash = h`;*
- *the honest node successfully processed a canonical successor C of $`B^*`$ through `on_block`;*
- *C declared $`B^*`$ FULL (`bid(C).parent_block_hash == bid(B*).block_hash`);*
- *at that node, `store.payloads[B*.root]` contained an `ExecutionPayloadEnvelope` whose payload had `block_hash == h`, and that envelope reached the store only after local blob-data availability and execution-payload verification succeeded.*

*Proof.* Since $`h`$ is in the payload hash chain, Definition 4c gives a non-head canonical block $`B^*`$ such that `bid(B*).block_hash = h` and `block_status(canonical, B*) = FULL`. By Definition 4b, the canonical child/successor C of $`B^*`$ declares FULL: `bid(C).parent_block_hash == bid(B*).block_hash`.

Under S2, a block that is rejected by honest nodes cannot be the canonical successor supported by the honest supermajority. Therefore at least one honest node successfully processed C through `on_block`. During that processing, Algorithm 7's parent-FULL assertion ran because C declared its parent FULL: `is_parent_node_full(store, C)` was True, so the node asserted `is_payload_verified(store, C.parent_root)`, i.e., `B*.root ∈ store.payloads`.

The only writer of `store.payloads[B*.root]` is `on_execution_payload_envelope` (Algorithm 8). That handler writes the envelope only after `is_data_available(envelope.beacon_block_root)` succeeds and `verify_execution_payload_envelope` succeeds. Inside `verify_execution_payload_envelope`, the payload is checked against the bid commitment, including `payload.block_hash == bid(B*).block_hash`. Since `bid(B*).block_hash = h`, the envelope stored at the witness node contains a payload with `block_hash == h`, and its sampled blob data and execution payload were locally verified before C was admitted. ∎

*Remark.* This is an existential import-witness statement. It does not claim that every honest node currently stores the payload, nor does it prove the reverse direction "availability implies hash-on-chain." P_DA only needs the on-chain-to-witness direction: if a payload hash is on the canonical payload hash chain, some honest import path had the payload and the locally required blob data.
...
*(iv) Payment clearing via slashing:* If the `ProposerSlashing` evidence is included in a block within the 2-epoch `builder_pending_payments` window, `process_proposer_slashing` (Helper 22, lines 14--22) clears the `BuilderPendingPayment` for the equivocating slot. This erases the IOU entirely — even if the quorum was met, the builder is not charged. The timely canonical inclusion of that slashing evidence is the separate **I_slash** side condition used by P_withhold; it is not derived from S2 alone. ∎
...
By Lemma 1, the FULL node for root r exists if and only if `r ∈ store.payloads`. Since entries are never removed from `store.payloads`, if the FULL node exists at time t, it exists at all t' > t. The same monotonicity applies to `get_node_children` (Algorithm 2), the vote-support relation expanded in Algorithm 4, and `get_weight` (Algorithm 5), which read from but never delete store entries. ∎

*Verification note.* The proof's claim that the four listed handlers exhaust all writers to `store.{blocks, block_states, payloads, payload_timeliness_vote, payload_data_availability_vote}` should be cross-checked against the upstream spec ([`fork-choice.md`](https://github.com/ethereum/consensus-specs/blob/e17a1389e6cee2f44e6b873e324ad4cb0172e341/specs/gloas/fork-choice.md)). A useful spot-check is `grep -nE "^\\s*store\\.(blocks|block_states|payloads|payload_timeliness_vote|payload_data_availability_vote)\\[" fork-choice.md` — every write site should land inside one of the four handlers above. If a future spec revision adds a new handler that mutates these fields (e.g., for fork transition, garbage collection, or finality-driven cleanup), Lemma 23 must be revisited. As of the spec version targeted by this document (`e17a1389`), no such additional writer exists.

*Remark.* This persistence property ensures that after a PTC-negative EMPTY path resolves a slot as EMPTY, `store.payloads[r]` still exists and the FULL node remains structurally available. Whether the FULL branch actually recovers depends on whether a later proposer builds on it (Lemma 17, Part (ii)). Regardless, the proposer's payment is guaranteed via Path B if the quorum was met (Lemma 10).

---

**Lemma 24** (Liveness under β < 20%). *If (i) β < 20% (the per-committee Byzantine bound), (ii) at least one honest proposer exists in every epoch, (iii) the network delivers messages within the slot timing bounds (T_att = 25%, T_ptc = 75%) — i.e., the synchrony delay $`\Delta`$ satisfies $`\Delta < T_{\mathrm{att}}`$ — (iv) every honest validator follows the protocol per Section 1.2: in particular, every honest validator assigned to a slot's committee attests at $`T_{\mathrm{att}}`$ for the `get_head` head, every honest PTC member votes at $`T_{\mathrm{ptc}}`$ per Lemma H4, and every honest builder reveals per Assumption H7 when its preconditions are met, and (v) at every analysed slot the strengthening hypotheses **H8** (the slot's parent remains canonical) and **H9** (the slot's parent is at the immediately preceding slot) hold — used by the Step 3 invocation of Lemma 7 Part (b) below; then: the consensus chain grows by at least one block per epoch, the execution chain grows whenever the slot's builder is honest and the block is canonical, and every honest builder's payment is settled within the 2-epoch `builder_pending_payments` window.*
...
**Why β < 20% and not Gasper's 1/3 — the calibration triangle.** The tighter Byzantine bound is a consequence of ePBS's choice of proposer-boost and same-slot quorum parameters, not a free design knob. Setting `BUILDER_PAYMENT_THRESHOLD = 60%` and `PROPOSER_SCORE_BOOST = 40%`, the Byzantine budget on a committee must be strictly less than $`60\% - 40\% = 20\%`$ for the threshold to separate a withholding builder (Path B should *not* fire) from a settling builder (Path B should fire). Reading the dependency the other way: the boost calibrates **S2** (the Byzantine bound); **S2** calibrates the 60% quorum. Tightening or loosening any one of the three numbers forces the other two to move together — the triangle is closed. The overview (§9.1) frames this as: *"the boost calibrates S2; S2 calibrates the 60% quorum."*
...
**Proposer equivocation and payment suppression.** If the proposer equivocates and a `ProposerSlashing` is included on-chain (within the 2-epoch `builder_pending_payments` window), `process_proposer_slashing` (Helper 22) clears the `BuilderPendingPayment` for that slot. This protects the builder: if it detected the equivocation and withheld, it is not charged at the epoch boundary. The P_withhold proof uses **I_slash** for the timely canonical inclusion and processing of this evidence before the boundary check.
...
### 1.5 Beacon-Chain Helpers and Processing Functions

The algorithms in Section 1.3 rely on several helper functions defined in `beacon-chain.md` and `fork-choice.md`. This section provides pseudocode and Python for each, grouped by size: tiny helpers first, then the processing functions that call them.
...
This is a pedagogical helper — **not a spec function** — used to make the FULL/EMPTY check inside `process_withdrawals` readable. The spec writes the equivalent comparison inline (see *Spec reference* below). We choose the name `parent_payload_revealed_in_state` to avoid collision with the real spec helper `is_parent_node_full(store, block)` (Helper 3 below, defined at `fork-choice.md` line 331), which has a different signature and operates on the fork-choice store rather than the BeaconState.
...
This function checks whether the execution payload is considered timely, parameterized by the desired answer. With `timely=True`, it returns True only when the payload is locally verified and a strict PTC majority voted `payload_present = True`. With `timely=False`, it returns True either when the payload is not locally verified, or when a strict PTC majority voted `payload_present = False`. The PTC votes are read from the scorecard populated by `on_payload_attestation_message` (Algorithm 9). This function is called with `timely=True` from `should_extend_payload` (Algorithm 6) for the PTC primary path, and with `timely=False` from `should_build_on_full` (Algorithm 6b) as one of the honest-construction vetoes for the immediately previous slot.
...
Identical structure to `payload_timeliness` (Helper 4), but checks blob-data-availability instead of timeliness. With `available=True`, it returns True only when the payload is locally verified and a strict PTC majority voted `blob_data_available = True`. With `available=False`, it returns True either when the payload is not locally verified, or when a strict PTC majority voted `blob_data_available = False`. PTC members vote on both dimensions independently. This helper is called with `available=True` from `should_extend_payload` for the PTC primary path, and with `available=False` from `should_build_on_full` as the DA-side construction veto for the immediately previous slot.
...
**`execution_payment` is not in the sum.** The solvency check considers only the trustless `value` obligations: `builder_pending_payments` entries are written by `process_execution_payload_bid` (Helper 8) only when `bid.value > 0`, and `builder_pending_withdrawals` is populated only by the protocol's Path A / Path B / Path C settlement paths, all of which key off `value`. Non-trustless promises (`bid.execution_payment > 0`) are **not** reflected in `pending`, so a builder can issue unlimited `execution_payment` commitments across slots without the protocol noticing they have overcommitted. This is a deliberate consequence of the split established in Definition 3b: the protocol enforces only the `value` field; `execution_payment` is metadata for an off-protocol channel. The overview (§3) frames this as: *"`execution_payment` is not in that sum."*
...
This function validates PTC votes that are included on-chain in a beacon block. It is called during `process_operations` (inside `process_block`) for each `PayloadAttestation` in the block body. Two key constraints are enforced: (1) the PTC vote must reference the parent block (`data.beacon_block_root = parent_root`), and (2) the vote must be from exactly the previous slot (`data.slot + 1 = state.slot`). The second constraint means PTC votes can only be included in the immediately next block — if that slot is missed, the votes are lost on-chain (though they still influenced fork-choice via gossip, as described in the Phase 4 timeline in Section 1.2). Line 5 converts the aggregated `PayloadAttestation` (with a bitfield over PTC positions) into an `IndexedPayloadAttestation` (with explicit validator indices), and line 6 verifies the aggregate BLS signature against those indices.
...
The envelope-verification path only checks the payload and stores it in the fork-choice store; the state-changing execution effects are applied here, when a successor block declares the parent FULL.
...
**Lines 2–4:** Extract the current block's bid, the parent's cached bid (`state.latest_execution_payload_bid`), and the `parent_execution_requests` field from the block body. The `parent_execution_requests` is a new `BeaconBlockBody` field that carries the EL requests from the parent's execution payload — it is included by the proposer (see Section 1.2, Proposer).
...
def process_attestation(state, attestation):
    ...
    # see beacon-chain.md:1688 and the per-committee loop at 1693–1707). None of these
    # checks is load-bearing for the lemmas in §1.4; we elide them for clarity ...
...
**Lines 13–15: Non-same-slot attestations.** If the attester voted for a block from a previous slot (their slot had no block, so their head fell back to an older block), they can observe whether the canonical state has advanced through that block's FULL branch. `data.index` signals their observation (`0` = empty, `1` = full). Line 15 checks this against `execution_payload_availability` — a BeaconState bitvector set by `apply_parent_execution_payload` when a canonical successor applies the parent's execution payload. If the attester signals `1` (full) but the canonical state records the slot as EMPTY (availability is `0`), the signals don't match.
...
def process_proposer_slashing(state, proposer_slashing):
    ...
    # beacon-chain.md:1806–1811 iterates over both signed_headers and calls
    # bls.Verify with DOMAIN_BEACON_PROPOSER). Not load-bearing for the lemmas
    # in §1.4; elided here for readability.
...
This is the mechanism by which Lemma 12c's solvency argument holds — which itself rests on Lemma 12b (Builder solvency at bid time): an exiting builder cannot create new obligations, so the obligations tracked by `get_pending_balance_to_withdraw_for_builder` (Helper 20) are a complete picture at exit time.
...
- *(iv) PTC members for slot N + 1 do not vote (Section 1.2, PTC handler lines 2–3: "If the validator has not seen any beacon block for the assigned slot, do not submit a payload attestation"). Therefore `store.payload_timeliness_vote` and `store.payload_data_availability_vote` are not updated for any slot-N+1 root (no such root exists).*
...
The slot timeline (Definition 1) places the bid commitment at the proposer's beacon block broadcast (t = 0) and the *effective* payload reveal deadline at the **payload-arrival deadline** `t_payload_due = slot_start(N) + get_payload_due_ms()` (parameterised by `PAYLOAD_DUE_BPS`, currently 75% of the slot). After `t_payload_due` an honest PTC member voting at `t_ptc = slot_start(N) + get_payload_attestation_due_ms()` (parameterised by `PAYLOAD_ATTESTATION_DUE_BPS`, also currently 75%) will record `payload_present = False` regardless of any late arrival, so the True-timeliness PTC tally is determined by the set of envelopes observed strictly before `t_payload_due`. The interval `[t_commit, t_payload_due)` is observable by the builder, who has already committed to a bid value but has not yet decided whether to reveal the execution payload. During this interval, the builder may observe new market information (e.g., centralized-exchange price movements) and adjust its decision. This subsection states the formal definitions and lemmas that describe this strategic situation and the spec-level mechanisms that bound it. Throughout, we keep both deadlines explicit because they are *conceptually* distinct (the payload-arrival deadline gates what counts as a `payload_present = True` vote; the PTC-vote deadline gates when the member must broadcast), even though under the current spec calibration both equal 75% of the slot.

**Definition 14** (Bid commitment time). The *bid commitment time* for a slot N bid is the time `t_commit = slot_start(N)` at which the proposer's beacon block containing `signed_execution_payload_bid` is broadcast.

After `t_commit`, an honest node that processes the block records a `BuilderPendingPayment` for slot N via `process_execution_payload_bid` (Helper 8, lines 17–26). Before `t_commit`, the bid exists only as a `SignedExecutionPayloadBid` on the `execution_payload_bid` gossip topic and creates no on-chain obligation.

*Spec reference: `process_execution_payload_bid` in `beacon-chain.md`.*

**Definition 15** (Reveal deadline). The *reveal deadline* for a slot N payload is `t_payload_due = slot_start(N) + get_payload_due_ms()`, parameterised by `PAYLOAD_DUE_BPS` (currently 7500, i.e., 75% of the slot, so `t_payload_due − slot_start(N) ≈ 9s`). A reveal is *PTC-timely* for a member iff that member locally observes the `SignedExecutionPayloadEnvelope` strictly before `t_payload_due`.

An honest PTC member votes `payload_present = True` for a PTC-timely reveal and `payload_present = False` for a reveal observed at or after `t_payload_due`. The True-timeliness PTC tally for slot N is therefore determined by the set of envelopes observed before `t_payload_due`, frozen at that instant. A late reveal may still cause `on_execution_payload_envelope` (Algorithm 8) to populate `store.payloads[r]`; it just does not contribute to the True-timeliness tally.

*Spec reference: `payload_timeliness` (Helper 4); `get_payload_due_ms` and `PAYLOAD_DUE_BPS` in `fork-choice.md` / `validator.md`; the per-member "observed before `get_payload_due_ms()`" rule in the prose for "Constructing a payload attestation message" in `validator.md`.*

**Definition 16** (Option window). The *option window* for a slot N bid is the interval `[t_commit, t_payload_due)`, whose length equals `(PAYLOAD_DUE_BPS / 10000) × SLOT_DURATION` (currently 9s).

Within this window, the builder can decide to reveal by broadcasting a `SignedExecutionPayloadEnvelope`, or to withhold by broadcasting nothing. The decision is unilateral: no consensus message during the window can revoke the bid commitment recorded at `t_commit`. Reveals that arrive after `t_payload_due` but before `t_ptc` may still be locally processed, but they do not contribute to the True-timeliness PTC tally and therefore do not influence the slot-N+1 fork-choice tiebreaker via the PTC primary path; Lemma 30 records that consequence.

**Definition 17** (Withhold strategy). A builder *withholds* if, despite having a `BuilderPendingPayment` recorded for its bid at `t_commit`, it does not broadcast a valid `SignedExecutionPayloadEnvelope` before `t_payload_due`.

By Algorithm 8, a withheld payload creates no `store.payloads[r]` entry at any honest node by `t_payload_due`, hence no FULL node is created in the fork-choice tree (Definition 6).
...
**Lemma 30** (Option window upper bound — PTC tally is frozen at the payload-arrival deadline). *The same-slot True-timeliness PTC tally for B (i.e., the second conjunct of `payload_timeliness(store, B.root, timely=True)`, `sum(payload_timeliness_vote[B.root] is True) > PAYLOAD_TIMELY_THRESHOLD`) is determined entirely by the set of envelopes observed by honest PTC members strictly before `t_payload_due = slot_start(N) + get_payload_due_ms()`. A reveal arriving at `t ≥ t_payload_due` does not contribute to the True-timeliness tally, even if it arrives in time for the PTC to broadcast at `t_ptc = slot_start(N) + get_payload_attestation_due_ms()` (under current spec parameters `t_payload_due = t_ptc ≈ 9s`, but `t_payload_due ≤ t_ptc` in general).*

*Proof.* By the honest PTC member handler (Section 1.2), each PTC member decides its vote value as `data.payload_present = has_execution_payload_envelope(store, block_root)` — which by the notation-table semantics is True iff the node previously saw an envelope referencing the block **strictly before `t_payload_due`**. The vote is broadcast no later than `t_ptc`. By the standing synchrony assumption ($`\Delta < T_{\mathrm{att}}`$), all honest PTC votes for slot N reach all honest nodes by `slot_start(N+1)`. The `payload_timeliness_vote[B.root]` array is written only by `on_payload_attestation_message` (Algorithm 9) applied to these votes. An envelope arriving at any node at `t ≥ t_payload_due` is processed by `on_execution_payload_envelope` (Algorithm 8) and may populate `store.payloads[B.root]` at the recipient — but it does **not** trigger any further write to `payload_timeliness_vote[B.root]`, and it did not change the value any honest PTC member had already committed to vote (their `payload_present` was decided based on their pre-`t_payload_due` view). Therefore the second conjunct of `payload_timeliness` is determined by the pre-`t_payload_due` set of arrivals and is independent of any later arrival. ∎

*Remark on `payload_timeliness` flipping.* `payload_timeliness(store, B.root, timely=True)` (Helper 4) is the conjunction of two conjuncts: (a) `is_payload_verified(store, B.root)`, and (b) the True-timeliness PTC tally above threshold. Lemma 30 establishes that the tally (b) is frozen at `t_payload_due`. The verification gate (a), however, CAN flip from `False` to `True` when a late payload arrives. So `payload_timeliness(..., timely=True)` can flip from `False` to `True` precisely in the case where the tally was already $`>`$ threshold at `t_payload_due` but a local observer hadn't yet validated the payload. In the typical "builder withholds, PTC votes False" scenario this case does not arise (the tally is False), and `payload_timeliness(..., timely=True)` stays False even after a late payload.

*Remark on the fork-choice outcome.* `should_extend_payload` (Algorithm 6) has two inputs a late payload can affect: the hard guard `is_payload_verified(store, B.root)` (line 2, can flip True via late payload) and the True-timeliness conjunct of `payload_timeliness` (just analysed). Even when the PTC tally was False at `t_payload_due`, a flipped hard guard can enable the proposer-based fallbacks (lines 8–10): if no slot-N+1 block has been proposed yet (fallback (a)) or if the slot-N+1 block declares FULL for B (fallback (c)), the tiebreaker can flip in B's favour. The PTC-primary-path option window therefore closes strictly at `t_payload_due`, but the broader proposer-based recovery window remains open until the slot-N+1 proposer commits. Both are subject to the builder's strategic withholding decision — the formal "free option" exercise cost (Lemma 29) applies regardless.
...
3. **Bounded option window.** Lemma 30 caps the duration during which the builder can observe market information without consequence at the **payload-arrival deadline** `t_payload_due` (= 9s under current `PAYLOAD_DUE_BPS = 7500`), which is what gates the True-timeliness PTC tally. Tightening the window (e.g., reducing `PAYLOAD_DUE_BPS`) would shorten the option but risks PTC members voting on an incomplete view of execution payload arrival; the spec balances these concerns at 75% of the slot. The PTC-vote deadline (`PAYLOAD_ATTESTATION_DUE_BPS`) is conceptually distinct from the payload-arrival deadline (`PAYLOAD_DUE_BPS`) — they happen to coincide today but may diverge in future spec revisions, and the bound that matters for the option window is the payload-arrival deadline.
...
*Remark on the trustless / non-trustless asymmetry.* The free-option analysis above applies only to the **trustless case** (`bid.value > 0`). In the **non-trustless case** (`bid.execution_payment > 0, bid.value = 0`), there is no analogous on-chain exercise price: Helper 8 creates no `BuilderPendingPayment` for non-trustless bids, so Lemma 29's "builder pays `bid.value` regardless of reveal/withhold" does not apply. A non-trustless builder can withhold freely without protocol-side cost — the option is *literally* free at the consensus layer. Any disincentive to withhold must come from the off-protocol channel (reputation, relay-style escrow, side-contract penalties); the consensus layer is silent. This asymmetry is one of the structural reasons the overview (§3) characterises the non-trustless field as "the protocol does not enforce `execution_payment` at all": absent on-chain enforcement, there is also no on-chain option premium. The strategic concern this raises (a builder strictly preferring `execution_payment` for option-exercise reasons) is intentional design slack, not a positive property — see overview §7's *free option (trustless case)* note.

[^lem12c-constants]: The Item 3 argument relies on the temporal separation between **two distinct constants**: the 2-epoch payment-settlement window (Lemma 21, fixed by the size of `state.builder_pending_payments`) and the 8192-epoch withdrawability delay (`MIN_BUILDER_WITHDRAWABILITY_DELAY`). If either constant is changed in a future spec revision, Item 3 of this proof must be revisited: the lemma requires the payment-settlement window to be **strictly shorter** than the withdrawability delay so that every IOU resolves before the exit sweep activates. The current 4096× margin is well-padded, but the proof is not a function of the gap — it is a function of the ordering.
