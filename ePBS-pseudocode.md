...
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A1a (honest builder reveals cautiously — trustless case `bid.value > 0`)** | **Assumption H7** — trustless cautious-reveal strategy: $t_{\mathrm{rev}} \geq T_{\mathrm{att}}$ AND ≥ 40% real attestation weight observed AND no proposer equivocation, before broadcast. Used by Lemmas 7, 9, 11, 17 to deliver concrete anti-reorg / Path-A-fires guarantees under β < 20%. |
...
**Lemma 8b** (Payload execution deadline is the beginning of the next slot — formal proof of P2). *Let $B$ be a block proposed in slot $N$ and let $P$ be the payload associated with $B$ (i.e., `bid(B).block_hash = hash(P)`). With the only possible exception of builders, the protocol does not require any other honest actor to complete the execution of $P$ before the beginning of slot $N+1$.*
...
- **Case 2b — equivocation-driven withhold; $B$ retains canonicity (A1a (iii) failed).** The original block may still have $\geq 40\%$ honest weight, so Case 1's quorum argument does not apply directly. The slashing-inclusion path discharges the IOU instead, via Lemma 22 part (iv): by **S1**, the equivocation evidence (observed by the honest builder at reveal time) is gossipped network-wide within $\Delta$; the `builder_pending_payments` settlement window spans 2 epochs = 64 slots; under **S2** (β < 20% per balance-weighted committee, which governs the proposer schedule), at least one of those 64 slots has an honest proposer who includes the `ProposerSlashing`. (The probability that all 64 consecutive proposers are Byzantine under **S2** is at most $\beta^{64} \approx 10^{-45}$ — of the same order as the per-slot **S3** bound and treated the same way: the proof reads the existence of an honest proposer within the window off **S2** as a deterministic claim at the named window, on the same overwhelming-probability footing that **S3** is justified on; see §1.4 "Adversarial model" / overview §9.1.) `process_proposer_slashing` (Helper 22) then clears the `BuilderPendingPayment` entry entirely, overriding whatever weight had accumulated.
