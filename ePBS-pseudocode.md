...
- If `s = PENDING`: the children are `(r, EMPTY)` (always) and `(r, FULL)` (only if `r ∈ store.payloads`, i.e., the payload for r was locally received and validated — see Definition 8).
...
**Definition 11** (Payload timeliness). The payload for block with root r is *timely* if both:

1. `r ∈ store.payloads` (the node locally has the validated payload envelope), and
...
`should_extend_payload(r)` first checks the hard guard: if `is_payload_verified(store, r)` is False, it returns False immediately. Otherwise, it returns True if any of: (a) the payload is timely and blob data is available (Definition 11), (b) `proposer_boost_root` is the zero root, (c) `parent(B_{proposer\_boost\_root}) ≠ B_r`, (d) `is_parent_node_full(store, B_{proposer\_boost\_root})`.
...
- The **consensus chain** is a sequence of beacon blocks, linked by `parent_root`, one per slot (or with gaps for missed slots). This chain always advances slot by slot: Block(98) → Block(99) → Block(100). Every beacon block exists regardless of whether its builder reveals a payload.
- The **execution chain** is a sequence of execution payloads, linked by `parent_hash` (`payload.parent_hash = state.latest_block_hash`). This chain only advances when a builder actually reveals a valid payload. If a builder withholds, the execution chain has a gap at that slot.

The two chains can therefore have different shapes. For example, if slot 98 has a payload, slot 99's builder withholds, and slot 100's builder reveals:
...
The mechanism that tracks where the execution chain is at any given time is `state.latest_block_hash`. This field is only updated by `apply_parent_execution_payload` (Helper 12b, line 29), which is called inside `process_parent_execution_payload` (Helper 12c) at the beginning of the next block's `process_block`. If a builder withholds, no subsequent block declares FULL for that slot, `latest_block_hash` is not updated, and still points to the last slot that had a revealed payload. The next builder reads this value and builds on that older execution state.
...
**Spec timing semantics.** The `@Upon(time_in_slot == T_PTC ...)` clause above models the worst-case (most conservative) firing time. The spec ([`validator.md`](https://github.com/ethereum/consensus-specs/blob/master/specs/gloas/validator.md)) actually specifies a *deadline*, not an exact firing time: a PTC member must broadcast the `PayloadAttestationMessage` *within the first* `PAYLOAD_ATTESTATION_DUE_BPS` (= 75%, i.e., 9 seconds) of the slot. An honest member is therefore allowed to vote as soon as it has decided its vote (e.g., as soon as the envelope is observed), but a *cautious* member waits close to the deadline to maximise information — voting earlier risks emitting `payload_present = False` and then having the envelope arrive later. The proofs in Section 12.4 depend only on (a) the value the member commits to and (b) the deadline being met; they do not depend on when within `(0, T_PTC]` the broadcast actually happens. We therefore keep the simpler firing-at-deadline form in the handler above and rely on the fact that all proof obligations follow from the worst case.

The `@Upon` guard fires for validators who are PTC members for this slot. A PTC member casts a **witness statement** — a report of what they observed, not a judgment on validity. The PTC vote handler checks envelope arrival (`has_execution_payload_envelope`), not execution validity (`verify_execution_payload_envelope`). Since PTC members are validators running full nodes, they do validate the payload as part of normal node operation (via `on_execution_payload_envelope`) — but this validation is independent of their PTC duty and is not a precondition for the vote.
...
**Lines 9–12: Payload observation.** This is the core PTC duty. The member checks whether a `SignedExecutionPayloadEnvelope` referencing this block has been seen. If yes: `payload_present = True`. If not: `payload_present = False`. The member does NOT verify the payload's execution validity — they only check whether the envelope arrived on the gossip network and passed the gossip-level validation rules (correct builder index, correct block hash, valid signature). The execution engine validation happens separately in `on_execution_payload_envelope` (Algorithm 8).
...
- **Lines 9–14: Envelope construction.** The builder wraps its stored payload and execution requests into an `ExecutionPayloadEnvelope`, referencing the beacon block root and the parent beacon block root. The `builder_index` must match the bid's `builder_index`.
- **Lines 14–15: Signing.** The envelope is signed using `get_execution_payload_envelope_signature`. Every node that later processes this envelope will verify this signature via `verify_execution_payload_envelope_signature` (Helper 12, line 5).
...
*Spec reference: bid construction in `builder.md`; envelope construction in `builder.md`; honest payload withholding in `builder.md`.*
...
The reason the function asks the *descendant* rather than the target block itself is that a block does not know its own payload status at creation time. Block B is proposed at `t = 0`, but the builder reveals the payload later (up to `t = 9s`). B cannot declare "I am FULL" — that has not happened yet when B is created. But the next block in the chain (say C, one or more slots later) does know: C's builder read `state.latest_block_hash` and set `bid.parent_block_hash` accordingly. If B's payload was revealed, C's hash matches B's committed hash → FULL. If not → EMPTY. The function simply reads this declaration.
...
**Line 2–3: Hard guard.** Before any PTC or fallback logic, the function checks `is_payload_verified(store, root)` — i.e., whether `root ∈ store.payloads`. If the payload envelope was never received or failed verification, the function returns False immediately and the tiebreaker favors EMPTY. This hard guard is the outermost gate: no amount of PTC votes or fallback conditions can override it. It ensures that `should_extend_payload` never returns True for a payload that the local node has not verified.

**Line 6: The PTC primary path.** If the PTC voted that the payload is timely (more than half of PTC members voted `payload_present = True`) and the blob data is available (`is_payload_data_available`), the function returns True — the payload was delivered on time, so FULL should win.
...
These fallbacks limit the damage a malicious PTC can cause (see the adversarial table in Section 12.4): even if every PTC member lies and votes against a valid payload, the FULL path often still wins through conditions 7–9.
...
**Lines 3–5: Parent payload verification.** The function asserts that the parent block's state exists in the store (line 3). If the block declares its parent was FULL (`is_parent_node_full` returns True), line 5 asserts that the parent's execution payload envelope has been verified and stored in `store.payloads` — i.e., `is_payload_verified(store, block.parent_root)` must hold. If the parent's payload was never received or was invalid, this assertion fails and the block is rejected. This is the mechanism that enforces the two-world model: a block cannot claim FULL ancestry without the payload actually existing.
...
This function is called when the builder's execution payload envelope arrives over the network (constructed and broadcast as described in Section 12.2, Builder Phase 2), separately from the beacon block. It verifies the envelope and stores it — but does not apply execution-layer state effects. Those effects are applied later, when the next block's `process_parent_execution_payload` runs inside `process_block`.

**Line 3:** Asserts that the beacon block this payload belongs to has already been processed by `on_block`. The payload cannot be verified in isolation — it needs the consensus-only state for reference.

**Line 4:** Asserts that the blob data (EIP-4844 data columns) associated with this payload is available. Without blob data, the payload cannot be fully validated.

**Line 5:** Reads the state `block_states[root]` (no copy needed — verification does not mutate state).

**Line 6:** Verifies the execution payload envelope against the EL: checks bid consistency (block hash, RANDAO, gas limit, builder index), verifies withdrawals, validates via the execution engine. No state mutation or payment processing happens here.

**Line 7:** Stores the verified envelope in `store.payloads`. This single write is the gate that controls whether the FULL node exists: Algorithm 2 (line 5) checks `is_payload_verified(store, r)` (i.e., `r ∈ store.payloads`) to decide whether the FULL child node is created. If `verify_execution_payload_envelope` at line 6 fails, line 7 is never reached, `store.payloads` is never populated, and the FULL node never appears in the fork-choice tree.
...
**Lemma H3** (Builder bid-reveal consistency). *If an honest builder submits a bid with block_hash = H and later reveals a payload for that bid, the revealed payload has block_hash = H.*
...
**Lemma H4** (PTC votes reflect observation, not validity). *An honest PTC member sets payload_present = True if and only if it has locally observed a SignedExecutionPayloadEnvelope referencing the block. The PTC vote is conditioned on envelope arrival, not on execution validity. (The member's node does validate the payload via `on_execution_payload_envelope` as part of normal operation, but this is independent of the PTC vote handler.)*

This property formalizes the "witness statement" design: PTC votes are observational reports, not validity judgments. It is the basis for the separation between PTC influence (tiebreaker) and execution validity (store.payloads gate, Lemma 1): even if all PTC members vote True, the FULL node only exists if the execution engine accepts the payload.

*Proof.* By the `ptc_vote` handler (Section 12.2, PTC), lines 9–12: `if has_execution_payload_envelope(store, block_root): data.payload_present = True; else: data.payload_present = False`. The function `has_execution_payload_envelope` checks whether the envelope was received on the gossip network — it does not invoke the execution engine or call `verify_execution_payload_envelope`. The handler contains no call to any execution validation function. ∎

---

**Lemma H5** (Honest builder withholding is conditional). *An honest builder withholds the payload (does not broadcast the envelope) only if the beacon block containing its bid is not the head of the builder's chain.*
...
The spec (`builder.md`) does not strictly mandate this strategy — it only says an honest builder *may* withhold when the block "is not the head of the builder's chain". H5 captures that minimal rule. H7 is a strictly stronger behavioural assumption that the proofs in this section rely on whenever they need a concrete anti-reorg or Path-A-fires guarantee. Without H7, the proofs only deliver the weaker Lemma H5 conclusion (envelope is broadcast iff block is locally head), which is insufficient to argue that the FULL outcome survives into the canonical chain.

*Proof.* H7 is a **strengthened behavioural assumption**, not a derivation from the spec. Its content is precisely the commitment that an honest builder runs, before the broadcast at `reveal_payload` lines 8–21, two extra checks beyond the spec-mandated `is_head_of_chain(block, store)` (Lemma H5):
...
This lemma establishes that the FULL branch is gated entirely by local execution validation. If the builder withholds the payload, or reveals an invalid payload that fails `verify_execution_payload_envelope`, then `store.payloads[r]` is never populated (Algorithm 8 line 7 is never reached), and the FULL child does not exist. This is the fundamental enforcement mechanism of ePBS: it is not PTC votes or attestation weight that prevents an invalid payload from being treated as delivered — it is the absence of the FULL branch in the tree.
...
This is the most basic safety property of ePBS: if the builder does not deliver (or honestly withholds — see Section 12.2, Builder Phase 2 lines 5–6), the protocol does not pretend the payload exists. The proof is a direct consequence of Lemma 1.

*Proof.* If the builder does not reveal a valid payload, `on_execution_payload_envelope` (Algorithm 8) is never called for root r (either the envelope never arrives, or `verify_execution_payload_envelope` at line 6 fails). In either case, line 7 is never reached — the envelope is never stored in `store.payloads` — so `r ∉ store.payloads`. By Lemma 1, (r, FULL) is not a child of (r, PENDING) in the fork-choice tree. By Lemma 3, the only way to reach a node (r, FULL) in the tree is as a child of (r, PENDING). Since this child does not exist, `get_head` (Algorithm 1) can never visit (r, FULL): the traversal at line 5 calls `get_node_children`, which does not include (r, FULL) in its output, so the `max` at lines 8–10 cannot select it, and if the traversal reaches (r, PENDING) as a leaf candidate, its only child is (r, EMPTY). Therefore, `get_head` cannot return (r, FULL). ∎
...
**Lemma 7** (Honest builder and honest PTC imply FULL). *Let B be a block with root r at slot N. Assume: (1) the builder reveals a valid execution payload for B via the cautious strategy of Lemma H7, so that r ∈ store.payloads and B has accumulated at least 40% of a slot committee's effective balance in real attestations by reveal time; (2) a majority of PTC members are honest and observe the payload, so that more than PTC_SIZE / 2 entries in payload_timeliness_vote[r] are True and more than PTC_SIZE / 2 entries in payload_data_availability_vote[r] are True. Then, when get_head is executed at slot N + 1 (i.e., current_slot = N + 1), the traversal selects (r, FULL) over (r, EMPTY), and B-FULL is canonical for every honest validator running get_head at slot N + 1 — no slot-N+1 proposer can reorg slot N via proposer boost alone (under β < 20%).*

This is the honest-case correctness property: when the builder delivers via cautious reveal (Lemma H7) and the PTC honestly reports it (Section 12.2, PTC lines 9–10), the fork-choice rule correctly picks the FULL path for the most recent block, AND the FULL outcome is robust against a malicious slot-N+1 proposer trying to reorg slot N with their own boost.
...
- Honest slot-N+1 attesters run `get_head`, observe (per Step 3) that $(B.\mathit{root}, \mathrm{PENDING})$ wins and traverse into B's branch, and attest on B's chain (most likely for $B$ itself if no slot-N+1 block extends it, or for an extending honest block). By Algorithm 4 Case 2 (descendant attribution), their votes propagate to $(B.\mathit{root}, \mathrm{PENDING})$ as ancestor (PENDING is supported via Case 2 line 13's `s == PENDING` disjunct, regardless of payload-status of the descendant chain). This adds at least $(1 - \beta)\, W_{N+1}$ to $\mathit{weight}((B.\mathit{root}, \mathrm{PENDING}))$, layered on top of the slot-N contribution $(1-\beta)\, W_N$ from Step 1.
...
- **Path A (builder reveals):** In the *next* block's `process_block`, `process_parent_execution_payload` (Helper 12c) calls `apply_parent_execution_payload` (Helper 12b), which calls `settle_builder_payment` (Helper 12a) to queue the payment and clear the pending entry. Payment happens in the next block, not during envelope receipt.
...
**Lemma 11** (Unconditional payment). *For any block at slot N with bid.value > 0 that is attested by honest validators with total effective balance meeting the quorum threshold: the proposer's payment is queued regardless of whether the builder reveals the execution payload.*

This combines Lemmas 9 and 10 into the central economic guarantee of ePBS. The hypothesis `bid.value > 0` automatically excludes self-builds, where Helper 8 line 6 forces `bid.value = 0` (so the proposer would only ever "owe itself" zero, and no IOU is recorded by Helper 8 lines 17–26 — see also Lemma 12b's non-self-build scoping). The lemma is therefore vacuous for self-builds; its content is the unconditional-payment guarantee for external-builder bids only.
...
**Case 1: A subsequent block declares FULL (Path A fires).** This happens when the builder reveals via the cautious strategy of Lemma H7 and an honest proposer builds on B-FULL. By Lemma 9 (second claim), under H7 and an honest slot-N+1 proposer, the FULL declaration happens at slot N + 1, Path A fires, `settle_builder_payment` queues the payment, and the pending entry is cleared. The proposer gets paid in the slot-N+1 block.

**Case 2: No subsequent block declares FULL (Path A does not fire).** This includes three sub-cases: (a) the builder withholds entirely, (b) the builder reveals an invalid payload (rejected by `verify_execution_payload_envelope`, so `store.payloads` is never populated), or (c) the builder reveals a valid payload, but no subsequent block builds on B-FULL — this last sub-case is the PTC + proposer collusion scenario analyzed in detail by Lemma 17 below. In all three sub-cases, `settle_builder_payment` is never called, so the `BuilderPendingPayment` persists. Same-slot attesters accumulate `payment.weight` via `process_attestation` (Helper 15). By the lemma's assumption, the accumulated weight meets the quorum threshold. By Lemma 10, `process_builder_pending_payments` queues the payment at the epoch boundary. The proposer gets paid.
...
*Remark.* The quorum threshold (60% of per-slot active balance, Helper 18) exists to protect the builder: if the beacon block was not widely attested (e.g., it was equivocating, arrived late, or was on a minority fork), the builder should not be forced to pay for a block that the network did not accept. Without the quorum, a malicious proposer could broadcast an invalid beacon block, have it attested by a small minority, and still extract payment from the builder.
...
*Proof.* By Helper 12a (`settle_builder_payment`), line 5: after queuing the payment (lines 3–4), `settle_builder_payment` replaces the pending payment entry with a fresh `BuilderPendingPayment()`. This entry has `weight = 0` and `withdrawal.amount = 0`. Two consequences prevent double payment:

1. **No further weight accumulation.** When `process_attestation` (Helper 15) later processes attestations for this slot, it reads `payment.withdrawal.amount` (line 31). Since `amount = 0`, the guard at line 31 (`payment.withdrawal.amount > 0`) fails, and no weight is added.
2. **Epoch boundary check fails.** When `process_builder_pending_payments` (Helper 14) runs at the epoch boundary, the entry has `weight = 0`. The quorum check `payment.weight >= quorum` (line 4) fails since `0 < quorum`. The entry is discarded during the epoch shift (lines 6–8) without producing a second payment. ∎
...
This formalises P3: a builder cannot commit to a bid it cannot pay, even across concurrent slots. The check is what makes Lemma 11 (unconditional payment) economically sound: when the proposer is paid, the builder's balance is guaranteed to cover the payment.
...
| PTC                    | Next-slot proposer        | Outcome | Mechanism                                                                                                                                                    |
| ---------------------- | ------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Honest (votes False)   | Honest (declares EMPTY)   | EMPTY   | Correct outcome — payload wasn't delivered, everyone agrees                                                                                                 |
...
3. **PTC cannot create FULL nodes.** Regardless of PTC votes, the FULL node exists only if `store.payloads` is populated, which requires the execution engine to validate the payload (Lemma 1). A malicious PTC voting True for a withheld or invalid payload has zero effect — the bottom four rows all resolve to EMPTY.
...
**Lemma 13** (Malicious PTC cannot force FULL for an invalid payload). *Let B be a block with root r. If the execution payload for B is invalid (rejected by the execution engine) or never revealed, then get_head cannot return (r, FULL), regardless of PTC votes.*
...
*Proof.* By Lemma 1, (r, FULL) is a child of (r, PENDING) if and only if `r ∈ store.payloads`. By Algorithm 8 (on_execution_payload_envelope), `store.payloads[r]` is written only at line 7, which is reached only after `verify_execution_payload_envelope` completes successfully at line 6. If the payload is invalid, `verify_execution_payload_envelope` fails at one of its assertions (beacon block consistency, bid consistency, execution engine validation) and line 7 of Algorithm 8 is never reached. If the payload is never revealed, Algorithm 8 is never called at all. In either case, `r ∉ store.payloads`, and by Lemma 1 the FULL child does not exist.
...
A builder cannot commit to a bid and then avoid paying. The unconditional payment mechanism ensures that the builder pays whether it reveals (Path A — settled by `settle_builder_payment` in the next block), withholds (Path B — settled at epoch boundary), or reveals an invalid payload (also Path B, since the invalid payload fails `verify_execution_payload_envelope`, the envelope is never stored, and `settle_builder_payment` is never called).

*Proof.* There are three cases for the builder's behavior after the bid is included:

**Case 1: Builder reveals a valid payload.** By Lemma 9 (Path A), the next block's `process_parent_execution_payload` calls `settle_builder_payment`, which queues the payment and clears the pending entry. The builder pays.

**Case 2: Builder withholds the payload entirely.** By Lemma 10 (Path B), the pending payment remains, attester weight accumulates, and `process_builder_pending_payments` queues the payment at the epoch boundary if the quorum is met. By the lemma's assumption, the quorum is met. The builder pays.

**Case 3: Builder reveals an invalid payload.** `verify_execution_payload_envelope` (Helper 12) fails at one of its assertions. The envelope is never stored in `store.payloads`, so no subsequent block can declare `parentStatus = FULL` for this slot (`on_block` Algorithm 7, line 5 would fail). Therefore, `settle_builder_payment` is never called for this slot. The pending payment entry created by `process_execution_payload_bid` (Helper 8, lines 17–26) is never cleared. This is identical to Case 2 from the payment mechanism's perspective: the entry remains, weight accumulates, and Path B queues the payment at the epoch boundary. The builder pays.
...
**Lemma 16** (Malicious proposer cannot extract payment without network support). *If a proposer includes a builder's bid in a beacon block, but the beacon block does not accumulate enough same-slot attestation weight to meet the quorum threshold, and the builder does not reveal the payload, then the builder is not charged.*
...
*Part (i):* The hard guard at Algorithm 6 line 2 passes because the builder revealed a valid payload (`is_payload_verified(store, r)` is True). By assumption (2), more than half of PTC members voted False, so `sum(store.payload_timeliness_vote[r]) ≤ PTC_SIZE / 2` and `is_payload_timely` returns False (Helper 4). The PTC primary path at line 6 fails. By assumption (3), a block exists in slot N+1 (`proposer_boost_root ≠ Root()`, so line 7 fails), it is a child of B (`boost_block.parent_root == r`, so line 8 fails), and it declares `parentStatus = EMPTY` (`is_parent_node_full` returns False, so line 9 fails). All conditions in the disjunction are False, and `should_extend_payload` returns False. By Definition 13, tiebreaker for FULL = 0, tiebreaker for EMPTY = 1. `get_head` selects (r, EMPTY).

*Part (ii):* We trace the state at slot N+2 and show that recovery is possible but not instantaneous.

**Slot N+1 state.** `get_head` returned (r, EMPTY) → (C, PENDING), where C is the colluding proposer's block built on B-EMPTY. Honest attesters at slot N+1 run `get_head` at $T_{\text{att}}$, see C as the head, and vote for C with `data.index = 0` (same-slot, no payload opinion). By Lemma 5, `get_ancestor(C.root, slot(B))` walks from C to B's slot and reads `parentStatus(C) = EMPTY`, so all slot N+1 attestation weight is attributed to (r, EMPTY).

**Slot N+2 fork-choice.** The zero-return rule does not apply to B (`slot(B) + 1 = N+1 ≠ N+2`), so `get_weight` computes full scores. At this point:

- `get_weight(r, EMPTY)` includes slot N+1's honest attestation weight (attributed via C's EMPTY declaration)
- `get_weight(r, FULL)` has no attestation weight (no block has been built on B-FULL yet)

An honest proposer at slot N+2 runs `get_head` and sees B-EMPTY → C as the winning chain. The proposer follows `get_head` honestly — it does not unilaterally override the fork-choice to build on B-FULL. It extends C.

**Multi-slot recovery.** Recovery requires an honest proposer to build a block D on B-FULL. This can happen when:

- At some slot $M > N+1$, an honest proposer observes that `store.payloads[r]` exists and that building on B-FULL is viable (e.g., because the EMPTY branch has stalled, or because an alternative fork-choice heuristic favors FULL).
- Once D exists, attesters at slot $M$ run `get_head` and may see D competing with C. By Lemma 5, votes for D's descendants are attributed to (r, FULL). Over subsequent slots, the FULL branch accumulates honest attestation weight.
- Under $\beta < 20\%$, honest validators control $\geq 80\%$ of the weight. Once the FULL branch has a proposer and one slot of attestations, it outweighs the EMPTY branch.

Recovery is **not guaranteed in one slot** — it depends on whether an honest proposer chooses to build on B-FULL rather than extending the current head. The fork-choice rule itself does not force this; it follows the heaviest chain. The key guarantee is that the FULL branch **remains available** (Lemma 23: `store.payloads[r]` is never removed), so recovery is always possible.

In summary: the colluders succeed in (i) suppressing FULL for slot N+1, and (ii) attracting slot N+1 honest attestation weight to the EMPTY branch. The FULL branch eventually prevails if an honest proposer extends it and honest attesters provide majority weight, but this may take multiple slots. ∎

*Remark.* The damage from PTC + proposer collusion is:

- **At least one slot of incorrect fork-choice** — the FULL/EMPTY decision at slot N+1 is wrong, and the EMPTY branch attracts honest attestation weight because the colluding block C is the fork-choice head.
- **A fork between B-FULL and B-EMPTY branches** — the colluding proposer's block C creates a competing branch that honest attesters initially follow. Recovery requires an honest proposer to build on B-FULL and enough subsequent attestation weight to overtake the EMPTY branch.
- **No permanent chain damage** — `store.payloads[r]` exists and cannot be removed, so the FULL node remains structurally available. If an honest proposer eventually builds on B-FULL, the FULL branch can recover with honest majority weight.
- **Proposer still gets paid** — if a subsequent block builds on B-FULL, Path A settles the payment (Lemma 9). If the EMPTY chain prevails and no block ever builds on B-FULL, Path A does not fire — but the proposer is still paid via Path B at the epoch boundary (Lemma 10), since the beacon block was widely attested and the 60% quorum is met (Lemma 20). Either way, the colluders cannot prevent the proposer's payment.
- **No slashing risk for honest validators** — honest attesters who followed C at slot N+1 are not penalized, since they followed the fork-choice rule honestly. The protocol does not slash for following `get_head`.
...
**Lemma 18** (Honest builder parentStatus consistency). *If an honest builder constructs a bid for slot $N+1$ on top of block $B$ at slot $N$, and $B$'s builder revealed a valid payload (i.e., `B.root ∈ store.payloads` at the bid-construction time), then the honest builder's bid has `bid.parent_block_hash = bid(B).block_hash`, and therefore `parentStatus(B') = FULL` (Definition 4) for the resulting block $B'$. Conversely, if $B$'s payload was not revealed (i.e., `B.root ∉ store.payloads`), the honest builder's bid has `parentStatus(B') = EMPTY`.*

This ensures that honest builders' FULL/EMPTY declarations match reality: if the parent's payload was delivered, the honest builder declares FULL.

*Proof.* The honest builder constructs its bid by invoking the `submit_bid` handler (Section 12.2). Line 3 reads `parent_block_hash ← state.latest_block_hash`. The subtlety: at the time the slot-$N+1$ builder constructs its bid, no block has yet executed `apply_parent_execution_payload` for $B$'s payload — that helper runs *inside* `process_block` of the slot-$N+1$ block, which doesn't exist yet (the builder is constructing the bid for it). So `state.latest_block_hash` as held in `store.block_states[B.root]` has *not* been updated to `bid(B).block_hash` yet.
...
*Proof.* By the `propose` handler (Section 12.2, Proposer), lines 27–30: if `should_extend_payload(store, parent_root)` is true (the proposer is building on B-FULL), the proposer sets `body.parent_execution_requests = store.payloads[parent_root].execution_requests`. The envelope in `store.payloads[parent_root]` was stored by `on_execution_payload_envelope` (Algorithm 8, line 7) only after `verify_execution_payload_envelope` (Helper 12) verified the envelope. During that verification, Helper 12 line 17 asserted `hash_tree_root(envelope.execution_requests) == bid.execution_requests_root`. Therefore, the execution requests in the stored envelope satisfy the root commitment. When `process_parent_execution_payload` (Helper 12c) runs for B', line 9 checks `hash_tree_root(requests) == parent_bid.execution_requests_root` — this holds because the requests are the same object whose root was verified at envelope time.
...
**Lemma 21** (Payment index correctness). *For a block at slot N with `bid.value > 0`: the index written by `process_execution_payload_bid` (Helper 8) and the index read by `settle_builder_payment` (inside `apply_parent_execution_payload`, Helper 12b) refer to the same entry in `builder_pending_payments`, regardless of whether the payment is settled in the same epoch or the next.*

This ensures that Path A settles the correct IOU — not some other slot's payment.

*Proof.* Helper 8 (line 26) writes to index `SLOTS_PER_EPOCH + bid.slot % SLOTS_PER_EPOCH`. This places the entry in the "current epoch" half of the 2-epoch `builder_pending_payments` array (indices `SLOTS_PER_EPOCH` through `2 × SLOTS_PER_EPOCH - 1`).

When the next block processes the parent's execution payload via Helper 12b, it computes the payment index based on the parent's epoch relative to the current state:

- **Case 1 (same epoch):** `parent_epoch == get_current_epoch(state)`. Helper 12b line 14 computes `payment_index = SLOTS_PER_EPOCH + parent_slot % SLOTS_PER_EPOCH`. Since `parent_slot = bid.slot`, this is the same index that Helper 8 wrote to. ✓
- **Case 2 (previous epoch — epoch boundary crossed):** `parent_epoch == get_previous_epoch(state)`. At the epoch boundary, `process_builder_pending_payments` (Helper 14, lines 6–8) shifts the array: the "current epoch" half (where Helper 8 wrote) becomes the "previous epoch" half (indices `0` through `SLOTS_PER_EPOCH - 1`). Helper 12b line 17 computes `payment_index = parent_slot % SLOTS_PER_EPOCH`, which is the shifted position of the same entry. ✓
- **Case 3 (older than previous epoch):** Helper 12b lines 19–26 queue the payment directly (bypassing the array), handling the edge case where more than one epoch elapsed. ✓

In all cases, `settle_builder_payment` operates on the same entry that `process_execution_payload_bid` created. ∎
...
This ensures that the protocol's structural guarantees are permanent — a validated payload cannot be "unvalidated" by later events.
...
2. The builder sees B as the head (boost applied) and reveals the payload.
...
| $20\% \leq \beta < 30\%$ | ≥ 20%    | No                           | Yes            | **No** — $\beta + 40 < 100 - \beta$ holds |
| $\beta \geq 30\%$        | ≥ 20%    | No                           | Yes            | **Yes** — $\beta + 40 > 100 - \beta$      |

The attack succeeds only when $\beta \geq 30\%$. The 20% threshold is where the spec's protection mechanism provides the first line of defense; the mathematical bound ($\beta < 30\%$) provides the second.
...
The spec (`builder.md`) prescribes only the minimal honest rule: reveal when the block is "timely and the head of the builder's chain". This document strengthens that rule into the **cautious-reveal strategy** formalised as Lemma H7: reveal only after observing ≥ 40% real attestation weight AND no proposer equivocation. Lemma H7 contains the full statement, proof, and remarks; Lemma 7 Part (b) and Lemma 9 (second claim) show how the 40% threshold yields concrete anti-reorg and Path-A-fires guarantees under $\beta < 20\%$. The fork-choice mechanism (`should_apply_proposer_boost`, `is_head_weak`, dual timeliness tracking in `record_block_timeliness`) is calibrated to support this strategy.
...
This is the execution payload verification function — the second phase of the two-phase model. It is called by `on_execution_payload_envelope` (Algorithm 8, line 6) when the builder's payload envelope arrives separately from the beacon block. This function **only verifies** the envelope — it does not mutate state, process EL requests, settle payments, or update `latest_block_hash`. Those effects are deferred to the *next* block's `process_parent_execution_payload` (Helper 12c). If this function completes successfully, Algorithm 8 stores the envelope in `store.payloads`, which enables the FULL branch in the fork-choice tree. If it fails at any assertion, `store.payloads` is never populated and the FULL branch never exists.

**Lines 4–5: Signature verification.** The envelope signature is verified against the builder's public key.

**Lines 6–10: Verification against the beacon block.** A copy of the block header is created with the state root filled in (lines 7–8), and the envelope must reference this exact block (line 9). The envelope's `parent_beacon_block_root` must match the block header's `parent_root` (line 10), ensuring the envelope is tied to the correct chain position.

**Lines 11–17: Verification against the committed bid.** The function reads the bid that was cached by `process_execution_payload_bid` (Helper 8, line 27) during beacon block processing. It verifies that the revealed payload matches what the builder committed to: same builder (line 13), same RANDAO (line 14), same gas limit (line 15), same block hash (line 16 — the core commitment check), and same execution requests root (line 17).
...
This function applies the execution-layer effects of the parent block's payload. It reads `parent_bid` from `state.latest_execution_payload_bid` internally (line 2), rather than receiving it as a parameter. It has two callers:
...
2. **During block production:** called by `prepare_execution_payload` (spec: `validator.md`) on a **copy** of the state by builders and self-building proposers *before* the block is proposed. This local simulation determines the correct execution chain head (`latest_block_hash`) and withdrawals for the new payload. The simulation and the real execution produce identical results because both operate on the same inputs deterministically.
...
**Lines 5–10: Execution-layer requests.** Processes deposits, withdrawals, and consolidation requests embedded in the parent's payload. These are the EL-originated state changes that modify the validator set and pending queues.
...
**Lines 28–29: State updates.** Marks the parent's slot as having a revealed payload in `execution_payload_availability` (line 28), and advances `latest_block_hash` to the parent's `block_hash` (line 29). The `latest_block_hash` update is critical: it is read by the next bid's consistency check in `process_execution_payload_bid` (Helper 8, line 14).
...
**Lines 2–4:** Extract the current block's bid, the parent's cached bid (`state.latest_execution_payload_bid`), and the `parent_execution_requests` field from the block body. The `parent_execution_requests` is a new `BeaconBlockBody` field that carries the EL requests from the parent's payload — it is included by the proposer (see Section 12.2, Proposer).
...
**Lemma 25** (Balance-weighted sampling is unbiased). *Each validator in the candidate set is selected with probability proportional to its effective balance, conditional on being reached during the sampling loop.*

*Proof sketch.* Within `compute_balance_weighted_selection` (Helper 30), each candidate `i` is accepted at iteration `j` (when `next_index` resolves to `i`) with probability $P_{\text{accept}}(i) = \text{effective\_balance}(i) / \text{MAX\_EFFECTIVE\_BALANCE\_ELECTRA}$. The `random_value` is a uniform 16-bit value, so the acceptance threshold is uniform over $[0, \text{MAX\_EFFECTIVE\_BALANCE\_ELECTRA} \times \text{MAX\_RANDOM\_VALUE}]$. The acceptance test `weight >= threshold` therefore succeeds with probability $\text{weight} / (\text{MAX\_EFFECTIVE\_BALANCE\_ELECTRA} \times \text{MAX\_RANDOM\_VALUE}) = \text{effective\_balance}(i) / \text{MAX\_EFFECTIVE\_BALANCE\_ELECTRA}$. The conditional probability of selection given that the candidate is reached equals $P_{\text{accept}}(i)$. ∎
...
- For **payment safety** (the binding constraint), the required margin is small in pp but tight in concentration: $\beta'$ within ~1pp of $0.20$ degrades per-slot reliability below $99\%$, while $\beta'$ at $0.18$ or below gives essentially perfect reliability.
...
**Lemma 27** (PTC vote idempotence across gossip and block inclusion). *Let v be a PTC member's vote for a beacon block with root r. If v reaches the store via gossip (Algorithm 9, `is_from_block=False`) and is later re-delivered via block inclusion (Helper 31), or vice versa, the final state of `store.payload_timeliness_vote[r]` and `store.payload_data_availability_vote[r]` is the same as if only one delivery occurred.*

*Proof.* By Algorithm 9 (`on_payload_attestation_message`), the writes at lines 11–12 are:

```
store.payload_timeliness_vote[r][ptc_index] = msg.data.payload_present
store.payload_data_availability_vote[r][ptc_index] = msg.data.blob_data_available
```

Both writes are unconditional assignments to a fixed index `ptc_index` (the voter's position in the PTC, computed deterministically from the validator index). The values written (`payload_present`, `blob_data_available`) are fields of `msg.data` — the same `PayloadAttestationData` object regardless of which path the vote took.

Two paths can deliver the same vote:

1. **Gossip first, then block inclusion.** The gossip-received vote writes `(payload_present, blob_data_available)` to position `ptc_index`. The later block-included vote (via Helper 31) writes the same values to the same position. The final state is unchanged after the second write.
2. **Block inclusion first, then gossip.** Symmetric: the block-included write occurs first, then the gossip-received vote re-writes the same values to the same position. Again, the final state is unchanged.

The two paths differ only in the validation performed before the write (signature check in gossip, skipped for block-included since the proposer aggregated and signed the bundle). Both paths reach the same write site with the same values. ∎
...
- *(i) The fork-choice store is unchanged from the start of slot N + 1, except: `proposer_boost_root` was reset to `Root()` by `on_tick_per_slot` at the slot N + 1 → N + 2 boundary (and was not set by any `on_block` during slot N + 1, since none arrived).*
...
*(iv)* By the PTC handler in Section 12.2 (line 4), a PTC member checks `has_beacon_block_for_slot(store, slot)`. With no slot N + 1 block, this returns False, and the handler returns without broadcasting. The vote stores indexed by slot N + 1 roots do not exist (since no slot N + 1 root entered the store at all). ∎
...
**Lemma 30** (Option window upper bound). *The option window has length `T_ptc` (= 9 seconds with current spec parameters). For any reveal that occurs strictly later than `slot_start(N) + T_ptc`, the same-slot PTC tally for B determines the FULL/EMPTY tiebreaker as if the reveal had not occurred.*

*Proof.* By the honest PTC member behavior (Section 12.2, PTC handler), at time `slot_start(N) + T_ptc` each honest PTC member assigned to slot N broadcasts a `PayloadAttestationMessage` with `payload_present = is_payload_verified(store, B.root)` evaluated at that time. By the standing network-model synchrony assumption (every honest node receives any honestly broadcast message within delay $\Delta < T_{\mathrm{att}}$; see the use in Lemma 7 Part (b)), all honest PTC votes for slot N reach all honest nodes by `slot_start(N+1)`. By Helper 4 (`is_payload_timely`), the predicate evaluates the `payload_timeliness_vote[B.root]` array as written by these votes. A late envelope arriving at `t > slot_start(N) + T_ptc` is processed by `on_execution_payload_envelope` (Algorithm 8) and may populate `store.payloads[B.root]`, but it does not retroactively change `payload_timeliness_vote[B.root]`. Therefore, when `should_extend_payload` (Algorithm 6) is evaluated at the start of slot N+1 to decide the tiebreaker for B, the PTC primary path (line 6) reads the same vote tally regardless of whether the late envelope arrived. ∎

*Remark.* Lemma 30 captures the *protocol-level* boundary on the option: any reveal after `T_ptc` cannot earn the builder a same-slot FULL outcome via the PTC primary path. Recovery via the proposer-based fallbacks (Algorithm 6 lines 7–9) is still possible but requires an honest next-slot proposer to declare FULL, which is outside the builder's unilateral control. From the builder's strategic perspective, `T_ptc` is the deadline at which the option's value crystallizes.
