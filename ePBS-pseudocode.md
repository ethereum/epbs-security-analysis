...
| "$B'$ will remain canonical forever"                                                             | Strengthening canonicity hypothesis embedded in P1, P4 (trustless case), and P6's statements (overview §4), where $B'$ is $B$'s parent at slot $N{-}1$. Formalised as Assumption H8 + H9 in §1.4. Rules out reorg configurations against $B$ that would otherwise contaminate the trustless proofs. P4's non-trustless case substitutes the confirmation rule's $\Sigma_R$ for this hypothesis.                                                                                                                                                                                                                                                                                                                                              |
...
The [overview document](https://github.com/ethereum/epbs-security-analysis/blob/master/README.md) (Section 4) lists **seven** externally observable properties, P1–P6 plus P_valid — claims an observer with full visibility of network messages and on-chain state can verify without inspecting any node's internal state — organised in three categories: **Category A** (payment-trustlessness-independent: P1, P2, P3, P_valid), **Category B** (payment-trustlessness-dependent, each with two cases: P4, P5), **Category C** (trustless-payment-only: P6). The following table maps each overview property to the formal lemmas proved in this section.

| Cat. | Property                                  | Statement                                                                                                                                                                                                                                                                                                    | Proved by                                                                                                                                                                                                                                                                                                                             |
| ---- | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A    | P1†: Block safety                         | If $B$ at slot $N$ extends a block $B'$ at slot $N{-}1$, $B'$ remains canonical forever, $B$ is timely, $B$'s slot-$N$ proposer does not equivocate, and $B$ includes a valid bid, then $B$ remains canonical at every later time                                                                            | **Lemma 7c** (standalone statement) + Lemma 7 Part (b) (anti-reorg argument under β < 20% and PROPOSER_SCORE_BOOST = 40%, with strengthenings H8 + H9 closing the split-vote and late-prior-slot reorg vectors)                                                                                                                                                              |
| A    | P2: Payload execution deadline is the beginning of the next slot | Let $B$ be a block proposed in slot $N$ and $P$ the payload with `bid(B).block_hash = P.block_hash`. With the only possible exception of builders, the protocol does not require any other honest actor to complete the execution of $P$ before the beginning of slot $N+1$.                                          | **Lemma 8b** (standalone statement, enumerating slot-$\leq N$ honest actors — attesters, PTC, earlier-slot proposers — and verifying none requires $P$ executed) + structural support from Lemma H1 (same-slot attesters signal zero), Lemma H4 (PTC reports observation, not validity), Algorithm 8 (`on_execution_payload_envelope` is the local payload-import path into `store.payloads`, while state-changing execution effects are applied later by `apply_parent_execution_payload`)                                                                                                                          |
| A    | P3: Data availability for chain inclusion | If a payload hash $h$ is in the payload hash chain (Definition 4c) of the canonical beacon chain — equivalently, $h = \mathit{bid}(B).\mathit{block\_hash}$ for some FULL block $B$ — then some honest node successfully processed a canonical FULL-declaring successor through `on_block`, and at that node the corresponding execution payload was verified and its blob data was locally available | **Lemma 18b** (P3 import witness), under S2. The statement is existential: hash-on-chain implies an honest-node import witness with `store.payloads[B.root]`, not that every honest node currently stores the payload |
...
| B    | P4†: Builder revealing protection (trustless case `bid.value > 0`) | Honest reveal under A1a ⇒$\mathit{bid}(B).\mathit{block\_hash}$ is in the payload hash chain (Definition 4c) of the canonical beacon chain — equivalently, $\mathit{block\_status}(\mathit{canonical}, B) = \text{FULL}$ — under honest PTC majority and successor liveness | **Canonicity step**: Lemma 7c (≡ P1), equivalently Lemma 7 Part (b), under H8 + H9. **FULL-on-chain step**: Lemma 7 Part (a) (PTC primary path selects FULL) + Lemma 9 second claim (honest successor's `should_build_on_full(store, head)` returns True under honest PTC because no negative timeliness or DA quorum exists — Lemma 14c) + Lemma 18 + Lemma 19 + the successor-liveness condition that turns the FULL fork-choice decision into a canonical child |
| B    | P4: Builder revealing protection (non-trustless case `bid.execution_payment > 0, bid.value = 0`) | Honest reveal under A1b ⇒$\mathit{bid}(B).\mathit{block\_hash}$ is in the payload hash chain — canonicity routed through the confirmation rule $R$ and its assumption set $\Sigma_R$ instead of P1 | **Canonicity step**: Lemma 17b (parametric in $\Sigma_R$ via A1b ≡ H7b's $R$-confirmation precondition). **FULL-on-chain step**: the same value-independent FULL-declaration machinery as the trustless row, with H7b replacing H7 and with honest PTC majority plus successor liveness |
| B    | P5: Builder withholding protection (trustless case `bid.value > 0`)        | Honest withhold under A1a in the covered no-weight or equivocation-driven subcases ⇒ the protocol does not queue or debit that bid's trustless payment | **Case 1** (no-weight withhold): Lemma 16's no-charge lemma, using the proof-specific **I_payweight** bridge because `process_attestation` is slot-keyed, not head-keyed. **Case 2a** (equivocation reorgs $B$ out and no canonical IOU remains): no canonical IOU exists to charge. **Case 2b** (canonical IOU remains): Lemma 22 part (iv), under **I_slash**, clears the IOU before the boundary check |
| B    | P5: Builder withholding protection (non-trustless case `bid.execution_payment > 0, bid.value = 0`) | Trivially true — no IOU is recorded for non-trustless bids, so the protocol's charging path is structurally absent                                                                                                                                                                                            | Helper 8 (`process_execution_payload_bid`) lines 17–26 are gated on `bid.value > 0`; for `bid.value = 0` no `BuilderPendingPayment` is created, and `process_attestation` (Helper 15) skips zero-`amount` entries via the precondition `payment.withdrawal.amount > 0`                                                              |
| C    | P6†: Unconditional payment to the proposer (trustless-only) | For a block at slot $N$ with `bid.value > 0` extending $B'$ at slot $N{-}1$ ($B'$ canonical forever), with $B$ timely, $B$'s slot-$N$ proposer not equivocating, P6's **attestation-inclusion hypothesis**, and **L_withdraw**, the proposer's `fee_recipient` is eventually credited with the bid amount; payment is single (no double-charge) | Lemma 11 (a `BuilderPendingWithdrawal` is queued via Path A or Path B) + Lemma 12 (single queueing) + Lemma 12b (solvency at admission) + Lemma 21 (index correctness) + Helper 13/Helper 21 under **L_withdraw** (the queued withdrawal is eventually drained by a canonical FULL block and credited at the EL) |

† P1, P4 (trustless), and P6 all rely on **both** strengthening hypotheses **H8** ($\mathit{parent}(B)$ remains canonical at every honest validator throughout the trace) and **H9** ($\mathit{parent}(B).\mathit{slot} = N - 1$ — no missing block immediately before $B$, with the assumption failing whenever any slot between $\mathit{parent}(B)$ and $B$ is missing). The 60% same-slot quorum on $B$ that Path B presupposes is **derived** (not assumed) in Lemma 11's proof from "timely publication" + S1 + S2 + H8 + H9 + A1a (i); H8 closes the split-vote-reorg gap (a Byzantine slot-$N$ proposer fragmenting honest attestations between $B$ and a sibling branch of $\mathit{parent}(B)$); H9 closes the late-prior-slot gap (a colluding slot-$(N{-}1)$/slot-$N$ Byzantine pair seating a competing sibling $B'$ at the missing slot $N{-}1$). Without either strengthening, the 60% derivation fails — Path B's quorum is never met — AND $B$ may be reorged so Path A doesn't fire either. P2, P3, P5 do not depend on either strengthening — they reason about structural fork-choice progress (P2), post-admission data availability (P3), and quorum failure (P5) respectively, all independent of $B$'s canonicity and structural-slot relationships. P4 (non-trustless) substitutes the confirmation rule's $\Sigma_R$ for the H8/H9 strengthenings.
...
| ------------------------------------------------------- | ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PTC bounds proposer power                               | PTC True/True overrides a single malicious proposer's EMPTY; negative PTC quorums constrain honest construction | Lemma 14 + Lemma 14b + adversarial table below — feeds the proof of P4, and explains the PTC-negative EMPTY path outside S3 |
| Builder solvency at bid time                            | Builder cannot overbid across concurrent slots          | Lemma 12b — precondition for P6 (unconditional payment); without solvency, the on-chain withdrawal could not be honoured                                                                           |
| Bid commitments are binding                             | Revealed execution payload must match bid hash          | Lemma H3 — a behavioural fact about honest builders that the overview reads off Phase 0 / Phase 3 code by construction (the same `stored_payload` object is set at bid time and read at reveal time); proved here as a lemma. Feeds the proof of P4.                                                                  |
...
| Execution validity gates FULL-node existence (per node) | FULL node exists iff payload validated                  | Lemma 1 — a per-node invariant that supports P3 (data availability for chain inclusion) externally                                                                                                 |
...
| (strengthening, P1, P4-trustless, P6) "$B'$ at slot $N{-}1$ will remain canonical forever" — overview §4 | Formalised as **H8 + H9** in §1.4, with $B' = \mathit{parent}(B)$: H8 says the parent remains canonical throughout the trace; H9 says the parent is exactly the slot-$N{-}1$ block, so the missing-slot configuration is excluded. Used as an explicit hypothesis in Lemma 7, Lemma 9 (second claim), and Lemma 11. P4's non-trustless case routes canonicity through $\Sigma_R$ instead and does **not** carry H8/H9. |
...
| ----------------------- | ----------------------------- |
| I_payweight             | Used by Lemma 16's P5 no-weight subcase. It supplies the bridge from "A1a's support-for-$B$ weight is < 40%" to "`payment.weight` cannot reach 60%," because Helper 15 accumulates payment weight by slot rather than by attested head. |
| I_slash                 | Used by Lemma 16's equivocation-driven P5 subcase and Lemma 22 part (iv). It supplies canonical inclusion and processing of a valid `ProposerSlashing` before the relevant `builder_pending_payments` boundary check. |
| L_verify                | Used by Lemma 7 / Lemma 9 for P4. It supplies local payload/DA/EL processing before the PTC and successor decisions after an honest reveal; S1 supplies delivery, not local verification time. |
| L_successor             | Used by P4's FULL-on-chain step. It turns the honest fork-choice preference for FULL into an actual canonical successor block declaring FULL. |
| L_withdraw              | Used by P6 after Lemma 11 queues a `BuilderPendingWithdrawal`. It supplies canonical FULL blocks with accepted execution payloads until the queued withdrawal is drained and credited on the execution layer. |
...
*Parametricity in $\Sigma_R$.* The non-trustless P4 (Lemma 17b below) holds parametrically in $R$ and its $\Sigma_R$. Choosing the [Fast Confirmation Rule (FCR)](https://fastconfirm.it) pulls in $\Sigma_{\mathrm{FCR}}$ (synchrony + a Byzantine bound + a quorum predicate); choosing a more conservative rule pulls in stronger assumptions but slows confirmation down. The overview (§4) writes the P4 non-trustless case statement parametrically in $\Sigma_R$ for this reason.
...
This is a **strengthening hypothesis** used alongside H8 by **Lemma 7 (the P4-trustless proof), Lemma 9's second claim, and Lemma 11 (the P6 proof)** to close a residual gap orthogonal to the split-vote-at-slot-$N$ gap H8 addresses. Lemma 9's second claim — that the FULL declaration happens at slot $N{+}1$ under A1a's 40% reveal threshold — depends on H9 through Lemma 7's Part (a) chain. For P6 in particular, the 60% same-slot quorum on $B$ that Path B presupposes is **derived** in Lemma 11's proof from "timely publication" + S1 + S2 + H8 + H9 — it is not assumed. Without H9, the derivation fails: the late-prior-slot attack splits slot-$N$ honest attestations between $B$ and a late-released sibling $B'$, capping $B$'s `payment.weight` below 60% AND reorging $B$ at slot $N{+}1$. Concretely:
...
**Lemma 8b** (Payload execution deadline is the beginning of the next slot — formal proof of P2). *Let $B$ be a block proposed in slot $N$ and let $P$ be the payload associated with $B$ (i.e., `bid(B).block_hash = P.block_hash`). With the only possible exception of builders, the protocol does not require any other honest actor to complete the execution of $P$ before the beginning of slot $N+1$.*

This is the formal statement of **property P2 (Payload execution deadline is the beginning of the next slot) from §4 of the overview**.
...
*Remark on coverage of P5 (case-split mirroring overview §9.3).* P5's trustless case splits into three sub-cases, mirroring the overview's §9.3 P5 proof structure:
...
Together, Lemma 16 (Case 1), noncanonicity of the IOU (Case 2a), and Lemma 22 part (iv) under I_slash (Case 2b) cover P5's trustless A1a-driven withhold causes. The non-trustless case is degenerate: Helper 8 creates no IOU when `bid.value = 0`, so no protocol charging path exists for `bid.execution_payment > 0, bid.value = 0`.
...
**Lemma 18b** (Data availability for chain inclusion — formal proof of P3). *Let $h$ be in the payload hash chain of the canonical beacon chain (Definition 4c). Under S2's honest-supermajority validity setting, there exists a canonical non-head block $B^*$ and an honest node such that:*
...
*Verification note.* The proof's claim that the four listed handlers exhaust all writers to `store.{blocks, block_states, payloads, payload_timeliness_vote, payload_data_availability_vote}` should be cross-checked against the upstream spec ([`fork-choice.md`](https://github.com/ethereum/consensus-specs/blob/ec1c01f5f10fb60636a022ae8944c912b6da35f8/specs/gloas/fork-choice.md)). A useful spot-check is `grep -nE "^\\s*store\\.(blocks|block_states|payloads|payload_timeliness_vote|payload_data_availability_vote)\\[" fork-choice.md` — every write site should land inside one of the four handlers above. If a future spec revision adds a new handler that mutates these fields (e.g., for fork transition, garbage collection, or finality-driven cleanup), Lemma 23 must be revisited. As of the spec version targeted by this document (`ec1c01f5`), no such additional writer exists.
...
**Proposer equivocation and payment suppression.** If the proposer equivocates and a `ProposerSlashing` is included on-chain (within the 2-epoch `builder_pending_payments` window), `process_proposer_slashing` (Helper 22) clears the `BuilderPendingPayment` for that slot. This protects the builder: if it detected the equivocation and withheld, it is not charged at the epoch boundary. The P5 proof uses **I_slash** for the timely canonical inclusion and processing of this evidence before the boundary check.
