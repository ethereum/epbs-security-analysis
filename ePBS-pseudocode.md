...
† P1 and P2 both rely on **both** strengthening hypotheses **H8** ($\mathit{parent}(B)$ remains canonical at every honest validator throughout the trace) and **H9** ($\mathit{parent}(B).\mathit{slot} = N - 1$ — no missing block immediately before $B$, with the assumption failing whenever any slot between $\mathit{parent}(B)$ and $B$ is missing). The 60% same-slot quorum on $B$ is **derived** (not assumed) in Lemma 11's proof from "timely publication" + S1 + S2 + H8 + H9 + H7 (i); H8 closes the split-vote-reorg gap (a Byzantine slot-$N$ proposer fragmenting honest attestations between $B$ and a sibling branch of $\mathit{parent}(B)$); H9 closes the late-prior-slot gap (a colluding slot-$(N{-}1)$/slot-$N$ Byzantine pair seating a competing sibling $B'$ at the missing slot $N{-}1$). Without either strengthening, the 60% derivation fails — Path B's quorum is never met — AND $B$ may be reorged so Path A doesn't fire either. P3 and P4 do not depend on either strengthening — they reason about quorum failure and post-admission data availability respectively, both independent of $B$'s canonicity and structural-slot relationships.
...
| Bid commitments are binding                             | Revealed execution payload must match bid hash          | Lemma H3 — a behavioural fact about honest builders that the overview reads off Phase 0 / Phase 3 code by construction (the same `stored_payload` object is set at bid time and read at reveal time); proved here as a lemma. Feeds the proof of P2.                                                                  |
...
| (strengthening, P1 and P2)`parent_remains_canonical(B)` — overview §4 | **Assumption H8** in §1.4 — $\mathit{parent}(B)$ remains canonical at every honest validator throughout the trace. Used as an explicit hypothesis in Lemma 7, Lemma 9, Lemma 11, Lemma 17. Closes the split-vote-reorg gap left by H7's 40% threshold; open work is to weaken it (see H8 statement).                                                                                                                                                                                                                                                              |
| (strengthening, P1 and P2)`parent_in_previous_slot(B)` — overview §4  | **Assumption H9** in §1.4 — $\mathit{parent}(B).\mathit{slot} = N - 1$ (no missing block immediately before $B$; covers all multi-missing configurations between $\mathit{parent}(B)$ and $B$). Used as an explicit hypothesis in Lemma 7, Lemma 9 (second claim), Lemma 11, and Lemma 17. Closes the late-prior-slot reorg gap exploitable by a colluding slot-$(N{-}1)$/slot-$N$ Byzantine pair — without H9, $B$'s slot-$N$ attestation weight can be split between $B$ and a late-released sibling at slot $N{-}1$, so the 60% derivation in Lemma 11's Case 2 fails AND $B$ is reorged at slot $N{+}1$. Open work is to weaken it (see H9 statement). |

**Behavioral assumption** — the overview names only one (A1, cautious-reveal). It is discharged by Assumption H7 below; the formal treatment also proves additional honest-behaviour lemmas (H1–H6) that the overview reads directly off its §5 Phase handlers (the overview removed the corresponding named A-assumptions because they are restatements of phase code rather than non-derivable strengthenings):

| Overview assumption / honest-behaviour fact      | Discharged by                                                                                                                                                                                                                                                                         |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A1 (honest builder reveals cautiously)** — the overview's sole named behavioural assumption | **Assumption H7** — cautious-reveal strategy: $t_{\mathrm{rev}} \geq T_{\mathrm{att}}$ AND ≥ 40% real attestation weight observed AND no proposer equivocation, before broadcast. Used by Lemmas 7, 9, 11, 17 to deliver concrete anti-reorg / Path-A-fires guarantees under β < 20%. |
...
This is a **strengthening hypothesis** used alongside H8 by **Lemma 7 (the P2 proof), Lemma 11 (the P1 proof), and Lemma 17 (the §8 "only successful attack" remark)** to close a residual gap orthogonal to the split-vote-at-slot-$N$ gap H8 addresses. Lemma 9's second claim — that the FULL declaration happens at slot $N{+}1$ under H7's 40% reveal threshold — also depends on H9 (it is used by Lemma 7's Part (a) chain). For P1 in particular, the 60% same-slot quorum on $B$ that Path B presupposes is **derived** in Lemma 11's proof from "timely publication" + S1 + S2 + H8 + H9 — it is not assumed. Without H9, the derivation fails: the late-prior-slot attack splits slot-$N$ honest attestations between $B$ and a late-released sibling $B'$, capping $B$'s `payment.weight` below 60% AND reorging $B$ at slot $N{+}1$. Concretely:
...
**Case 2: No subsequent block declares FULL (Path A does not fire).** This includes three sub-cases: (a) the builder withholds entirely, (b) the builder reveals an invalid execution payload (rejected by `verify_execution_payload_envelope`, so `store.payloads` is never populated), or (c) the builder reveals a valid execution payload, but no subsequent block builds on B-FULL — this last sub-case is the PTC + proposer collusion scenario analyzed in detail by Lemma 17 below. In all three sub-cases, `settle_builder_payment` is never called, so the `BuilderPendingPayment` persists. Same-slot attesters accumulate `payment.weight` via `process_attestation` (Helper 15). By the same argument established in Case 1 — under S1, S2, H7 (i), H8, H9, and "timely" publication — every honest slot-$N$ attester votes for $B$, contributing their effective balance to $B$'s `payment.weight`. The accumulated weight is at least $(1-\beta) W_N > 0.8 W_N > 0.6 W_N$. By Lemma 20 (quorum reachability), assuming attestations are included on-chain by the next-epoch boundary, the accumulated `payment.weight` meets the 60% quorum. By Lemma 10, `process_builder_pending_payments` queues the payment at the epoch boundary. The proposer gets paid.
