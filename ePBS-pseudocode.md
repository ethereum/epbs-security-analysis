...
The [overview document](https://github.com/ethereum/epbs-security-analysis/blob/master/README.md) (Section 9) states explicit assumptions — structural (S-prefix), behavioral (A-prefix), and algorithmic (G-prefix) — that its proof sketches depend on. This document provides the code and proofs that discharge every assumption. Note that the overview's set of named assumptions is deliberately small: it carries **two** behavioural assumptions (A1a, the trustless cautious-reveal strengthening; A1b, the non-trustless cautious-reveal strengthening, parametric in a confirmation rule $`R`$ and its assumption set $`\Sigma_R`$) and **six** algorithmic G-assumptions (payment + slashing machinery), because every other honest-actor fact and every fork-choice helper is read off code that the overview itself shows in full (Phase handlers in overview §5 and §6, payment helpers in overview §6/§7). The tables below show the correspondence for both the named overview assumptions and the formal-treatment lemmas that prove the additional honest-behaviour facts the overview cites by phase-code reference.
...
| S4 (per-property honesty hypotheses)                                                                                        | Stated as explicit lemma hypotheses where used (e.g., "honest slot-N+1 proposer" in Lemma 7 and Lemma 9, or "honest node successfully processed the block" in Lemma 18b's import witness)                                                                                                                                                                                                                                                         |
