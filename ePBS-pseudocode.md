...
| P2: Builder revealing protection          | Honest reveal under H7 ⇒ payload included in the canonical chain (slot becomes FULL on chain) — under honest PTC majority and honest slot-N+1 proposer                                                                              | Lemma 7 Part (a) (FULL/EMPTY tiebreaker selects FULL via PTC primary path) + Lemma 7 Part (b) (anti-reorg argument under β < 20% and PROPOSER_SCORE_BOOST = 40%) + Lemma 18 (honest builder's bid declares FULL when local store has the parent's execution payload) — combined, deliver the "payload on chain" claim                          |
...
| P4: Data availability for chain inclusion | A beacon block's execution payload is part of the canonical chain *only if* both the payload wrapping it and the associated blob data have been delivered (one-directional necessary condition)                                                                                | Lemma 18b forward direction (DA missing ⇒ no canonical successor declares FULL), under honest super-majority + honest slot-N+1 proposer. The converse direction is also proved in Lemma 18b but is not claimed by P4                                                                                                                |
...
**Lemma 18b** (Data availability for chain inclusion). *Let $B$ be a beacon block with root $r$ at slot $N$ that is canonical in the consensus chain. Under (i) the per-committee Byzantine bound $\beta < 20\%$, (ii) network synchrony with delay $\Delta < T_{\mathrm{att}}$, and (iii) an honest super-majority of slot-$N+1$ proposers and attesters (in particular, an honest slot-$N+1$ proposer): $B$ is FULL on chain* — in the §3 sense of the overview, i.e., some subsequent canonical block $C$ has $\mathit{bid}(C).\mathit{parent\_block\_hash} = \mathit{bid}(B).\mathit{block\_hash}$ — *if and only if both the payload wrapping $B$'s execution payload and the blob data associated with $B$ were delivered to the honest super-majority (so that $r \in \mathit{store.payloads}$ at every honest node, including the slot-$N+1$ proposer).*

**The forward direction of this lemma is the formal proof of P4 (Data availability for chain inclusion) from §4 of the overview** — P4 itself only claims the one-directional "*only if* data is available" condition. The reverse direction is proved here as a strictly stronger biconditional fact: under the same hypotheses, the data-availability gate is *equivalent* to chain inclusion, not merely necessary for it. This is the biconditional between the on-chain FULL/EMPTY notion (which compares $\mathit{bid}(C).\mathit{parent\_block\_hash}$ against $\mathit{bid}(B).\mathit{block\_hash}$, §3 of overview) and the data-availability gate (which requires both payload and blob data to populate `store.payloads`).
