...
**Definition 4b** (Block status — chain-relative). Given a canonical beacon chain $\mathit{chain}$ and a non-head block $B$ in $\mathit{chain}$ (i.e., $B \neq \mathit{head}(\mathit{chain})$), let $C$ denote $B$'s child in $\mathit{chain}$ (the unique block in $\mathit{chain}$ with $\mathit{parent}(C) = B$). Then:

$$
\mathit{block\_status}(\mathit{chain}, B) = \begin{cases} \text{FULL} & \text{if } \mathit{bid}(C).\mathit{parent\_block\_hash} = \mathit{bid}(B).\mathit{block\_hash} \\ \text{EMPTY} & \text{otherwise} \end{cases}
$$

When $B = \mathit{head}(\mathit{chain})$, $\mathit{block\_status}$ is **undefined**: $B$'s `bid.block_hash` has been committed but no successor has yet referenced it.

This is the chain-relative companion to Definition 4: for non-head $B$, $\mathit{block\_status}(\mathit{chain}, B) = \mathrm{parentStatus}(C)$ where $C$ is $B$'s child in $\mathit{chain}$. Definition 4 reads the FULL/EMPTY declaration from the child's perspective (intrinsic to $B$'s child); Definition 4b reads the same fact relative to a named chain.
...
**Definition 4c** (Payload hash chain). Given a head beacon block $B$ on the canonical chain, the *payload hash chain* is the sequence of payload hashes $h^{(0)}, h^{(1)}, h^{(2)}, \ldots$ defined by:
...
**Equivalent characterization.** A payload hash $h$ belongs to the payload hash chain if and only if there exists a non-head canonical block $B^*$ such that $\mathit{bid}(B^*).\mathit{block\_hash} = h$ AND $\mathit{block\_status}(\mathit{canonical}, B^*) = \text{FULL}$. Equivalently: the payload hash chain is the sequence of `bid.block_hash` values of every FULL (in the sense of Definition 4b) block on the canonical chain, ordered from head back to genesis.
